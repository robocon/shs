...
include_once 'fpdf_thai/shspdf.php';
...
$drugReactTxt = '';
// แพ้ยา
$sqlReact="SELECT GROUP_CONCAT(`tradname`) AS `react_name` FROM `drugreact` WHERE `hn`='$hn' AND `advreact`<>'' LIMIT 1";
$reactQ = $dbi->query($sqlReact);
if($reactQ->num_rows>0){
    $re = $reactQ->fetch_assoc();
    $drugReactTxt .= $re['react_name'];
}

// อาการข้างเคียง
$sqlEffect="SELECT GROUP_CONCAT(`sideeffects`) AS `sideeffects` FROM `drugreact` WHERE `hn`='$hn' AND `sideeffects` <> '' LIMIT 1";
$qEffect = $dbi->query($sqlEffect);
$effectRows = $qEffect->num_rows;
if($effectRows > 0){
    $resEff = $qEffect->fetch_assoc();
    $drugReactTxt .= ' S/E: '.toUTF($resEff['sideeffects']);
}
...
$pdf->Cell(198, 7, 'Medication Reconciliation Form and Doctor order sheet [ใบ M.R.]', 1, 1, 'L',true);
...
$pdf->SetXY(142.5, 50.5);
$pdf->Cell(60, 5, $drugReactTxt,0,1);
