...
$sqlReact="SELECT GROUP_CONCAT(`tradname`) AS `react_name` FROM `drugreact` WHERE `hn`='$hn' AND `advreact`<>'' LIMIT 3";
...
$sqlEffect="SELECT GROUP_CONCAT(`sideeffects`) AS `sideeffects` FROM `drugreact` WHERE `hn`='$hn' AND `sideeffects` <> '' LIMIT 2";
$qEffect = $dbi->query($sqlEffect);
$eff = '';
if($qEffect->num_rows > 0){
    $resEff = $qEffect->fetch_assoc();
    $eff = '  S/E: '.$resEff['sideeffects'];
}
...
$pdf->Cell(60, 5, $re['react_name'].$eff,0,1);
