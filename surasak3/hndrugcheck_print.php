...
$pdf->SetXY(70, 25.5);
$pdf->Write(5, '[  ] ไม่มีประวัติยาเดิมใน ร.พ. (ส่วนที่ 2)');
$pdf->SetXY(75, 30.5);
...
$pdf->SetXY(135, 45.5);
$pdf->Cell(68, 5, 'อาการนำ:_____________________________________',0,1);
$pdf->SetXY(135, 50.5);
...
if(!empty($drug_items)){
    ...
    foreach ($drug_items as $key => $di) {
        ...
        // หาความสูงโดยประมาณของ genname tradname
        $drugHeight = toUTF("\n".$di['tradname']."\n".$di['genname']);
...
$pdf->SetTextColor(255,255,255);
$pdf->SetXY(5, $lastY);
$pdf->Cell(198, 6.5, $pdf->conv('ส่วนที่ 2 ยาเดิมจากสถานพยาบาลอื่น'), 1, 1, 'L',true);
