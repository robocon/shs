...
<form action="report_listicd10mount.php" method="post" name="form1">
<input name="act" type="hidden" value="show" />
แพทย์ : 
<?
$sqldr="select name from inputm where status='Y' AND level='dr'";
$querydr=mysql_query($sqldr);
?>
<select name="doctor" class="forntsarabun" id="doctor">
  <option value="all">-------------------- เลือกทั้งหมด --------------------</option>
<? while($rowsdr=mysql_fetch_array($querydr)){ ?>  
  <option value="<?=$rowsdr["name"];?>"><?=$rowsdr["name"];?></option>
<? } ?>
</select>
เดือน : 
...
ประจำเดือน <?=$showdate;?>  แพทย์ : <? if($doctor=="all"){ echo "แพทย์ทั้งหมด";}else{ echo $doctor;}?></strong></p>
