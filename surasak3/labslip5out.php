...
/*$query3 = "SELECT * FROM hiv_vp WHERE hn = '$cHn' limit 1";
$result3 = mysql_query($query3);
if(mysql_num_rows($result3)>0){
	$row3 = mysql_fetch_array($result3);
}else{
	$query3 = "SELECT * FROM hiv_vo WHERE hn = '$cHn' limit 1";
	$result3 = mysql_query($query3);
	if(mysql_num_rows($result3)>0){
		$row3 = mysql_fetch_array($result3);
	}else{
		$query3 = "SELECT * FROM hiv_nhso WHERE hn = '$cHn' limit 1";
		$result3 = mysql_query($query3);
		if(mysql_num_rows($result3)>0){
			$row3 = mysql_fetch_array($result3);
		}
...
$query3 = "SELECT * FROM hiv WHERE hn = '$cHn' limit 1";
$result3 = mysql_query($query3);
$row3 = mysql_fetch_array($result3);

$pos = strpos($row3['address'],"D");
if($pos!=null){
$nap = substr($row3['address'],$pos,14);
print "<DIV style='left:70PX;top:75PX;width:500PX;height:30PX;'><span class='fc1-1'><b>$nap</b></span></DIV>";
} 
