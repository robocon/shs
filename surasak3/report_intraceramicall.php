...
<span class="tet1">พิมพ์ใบตรวจสุขภาพอินทราเซรามิค</span> <br />
  <br />
  <span class="tet1">  <input type="submit" name="ok" value="ตกลง" style="width:60px; height:40px; font:'TH SarabunPSK'; font-size:20px;"></span>
  <br />
  <br />
...
if(isset($_POST['ok'])){		
	include("connect.inc");
	$sql="SELECT  a.course, a.branch, b.hn, b.ptname, b.weight, b.height, b.bp1, b.bp2, b.p FROM opcardchk AS a INNER  JOIN out_result_chkup AS b ON a.HN = b.hn WHERE a.part ='อินทราเซรามิค' order by a.row";
	//echo $sql;
...
mmHg. </u></strong><span class="text3"><strong>P: </strong> <u>
  ...
  <tr>
    <td width="100%"  valign="top"><table width="100%" height="111" border="1" cellpadding="3" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse">
      <tr>
...
<? $sql="SELECT * FROM resulthead WHERE profilecode='UA' and hn='".$result['hn']."' and clinicalinfo ='ตรวจสุขภาพอินทราเซรามิค' order by autonumber desc limit 0,1";
//echo "===>".$sql."<br>";
...
list($authorisename,$authorisedate)=mysql_fetch_array($objQuery1);
		...
		?>
          ...
          <tr>             
			...
			<? //echo $bloodvalue;  
			if($bloodvalue=="Negative" && $provalue=="Negative" && $gluvalue=="Negative" && $ketuvalue=="Negative"){ echo "ปกติ";}else{ echo "ผิดปกติ ควรตรวจซ้ำหรือพบแพทย์เพื่อหาสาเหตุ";}?></td>
            </tr>
        </table></td>
      </tr>
    </table></td>
...
<?
$sql="SELECT * FROM resulthead WHERE profilecode != 'UA' and hn='".$result['hn']."' and clinicalinfo ='ตรวจสุขภาพอินทราเซรามิค' order by autonumber desc";
//echo $sql;
$query = mysql_query($sql);
...
if($objResult["labcode"]=='CHOL'){
	if($objResult["result"]<=200){
		...
		$app="ระดับไขมันในเลือดมีค่าสูงมากผิดปกติ สมควรพบแพทย์เพื่อรับการประเมินและให้การรักษา";	
	}
}
...
if($objResult["labcode"]=='ALP'){  //ALK
	...
	?>
         ...
         <?
		$sqlchk="SELECT * FROM `out_result_chkup` WHERE hn='".$result['hn']."' and row_id between 3981 and 4210";
		//echo $sqlchk;
...
<table width="100%" border="0" class="text4">
  <tr>
    <td  width="50%" align="center"><strong>Authorise  LAB : </strong><?=$authorisename?> <strong> (<?=$authorisedate?>) <span><strong> CXR : </strong>ร.อ.วริทธิ์ พสุธาดล (ว.38228) รังสีแพทย์<strong> (15-11-2015)</strong></span></strong><strong> Doctor : พ.ท.เลอปรัชญ์ มังกรกนกพงศ์ (ว.32166) (20-11-2015)</strong></td>
