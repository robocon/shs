...
//insert data into pocompany
       ...
       $result = mysql_query($query) or 
                die("**เตือน !ได้มีการบันทึกข้อมูลเรียบร้อยแล้ว<br>
                -------- รายการจ่าย ---------<br> 
                $Thidate<br>
                ($cComcode)$cComname  จำนวนรายการที่สั่ง $item รวมเงินทั้งสิ้น $nNetprice บาท<br><BR><BR><BR><BR><a target=_BLANK href='podgbill.php'>พิมพ์รายการสั่งซื้อ</a>");
...
for ($n=1; $n<=$x; $n++){
 ...
 include("unconnect.inc");
  ...
  /*print "<font face='Angsana New' size='2'>&nbsp;&nbsp;&nbsp;<B>$Thidate,รายการสั่งซื้อ</B><BR>";
  ...
  print "  <th>รหัส</th>";
  print "  <th>รายการ</th>";
  print "  <th>หน่วยนับ</th>";
  print "  <th>ขนาดบรรจุ</th>";
  print "  <th>ราคา(ไม่รวมVAT)</th>";
  print "  <th>จำนวน</th>";
  print "  <th>รวมเงิน(ไม่รวมVAT)</th>";
  print " </tr>";
      for ($n=1; $n<=$x; $n++){
            print (" <tr>\n".
                     "  <td>$n</td>\n".
    ...
    print "</table>";
    print "รวมเงินทั้งสิ้น(ไม่รวมVAT)  $nNetprice  บาท<br>";
    print "<font face='Angsana New' size='2'>เจ้าหน้าที่ :$sOfficer<BR>";*/
	/*if($result){
	?>
	<script>
...
print"<DIV style='left:49PX;top:304PX;width:250PX;height:22PX;'><span class='fc1-4'> $aSnspec[2]</span></DIV>";
...
print"<DIV style='left:360PX;top:816PX;width:263PX;height:27PX;'><span class='fc1-0'>ส่งของภายใน 5 วัน นับจากวันที่ที่ลงในใบสั่งซื้อ</span></DIV>";
