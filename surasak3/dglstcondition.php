...
if( $user_code !== 'ADM' ){
	...
	$check_level = in_array($user_code, array('ADMPHA'));
    $check_user = in_array($user_id, array('อนุพงศ์'));
...
?>
        ...
        <p>เพื่อทำการแก้ไขสิทธิ์ในการเข้าถึงเมนูระบุเงื่อนไขการจ่ายยา</p>
...
function searchSuggest(str,len,getto) {
		...
		if(str.length >= len){
			...
			url = 'dglstcondition.php?action=drugcode&search1=' + str+'&getto=' + getto;
...
If (!empty($drugcode)){
    ...
    while (list ($drugcode, $tradname,$genname, $salepri,$part,$stock,$mainstk,$totalstk,$pack,$packpri_vat, $comcode, $comname, $unitpri,$code24, $edpri,$spec) = mysql_fetch_row ($result)) {
        print (" <tr>\n".
           "  <td BGCOLOR=66CDAA><a   href=\"dgconditionedit.php?Dgcode=$drugcode\">$drugcode</a></td>\n".
