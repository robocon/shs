...
if($page=='reprint')
{
    ...
    $date = $_GET['date'];
    $hn = $_GET['hn'];
    $sql = "SELECT * FROM `appoint` WHERE `date` LIKE '$date%' AND `detail2` LIKE '%cef-3%' AND `hn` = '$hn' ";
    $q = $dbi->query($sql);
...
$sql = "SELECT `hn`,`ptname`,`date`,`appdate`,`apptime`,`detail2`,`depcode`,`remark`,`room` FROM `appoint`WHERE `appdate_en` >= '$endate' AND `detail2`LIKE '%cef-3%' GROUP BY `date`,`hn`";
$q = $dbi->query($sql);
?>
<div>
