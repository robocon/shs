...
<table width="85%">
	<tr>
		<td align="center" class="textcash1"><strong>เอกสารแสดงค่าใช้จ่ายในการรักษาพยาบาลประเภทผู้ป่วยนอก</strong></td>
	</tr>
	<tr>
	...
	<tr>
		<td class="textcash1">
			<span class="textcash">
			<strong>ชื่อผู้ป่วย :</strong><?= $results['yot'] . " " . $results['name'] . " " . $results['surname'] ?>
			<strong>HN :</strong> <?= $hn ?>
			<strong>VN :</strong> <?= $vn ?>
			<strong>สิทธิ :</strong> <?= $ptright; ?>
			<strong>บัตร ปปช. :</strong> <?= $results['idcard'] ?>
...
<table width="85%" border="1" cellpadding="0" cellspacing="0" class="textcash" style="border-collapse:collapse">
	...
	<?php
	$query = "CREATE TEMPORARY TABLE drugrx01 SELECT * FROM phardep WHERE date like '$date%' AND hn = '$hn' and price >0";
	$result = mysql_query($query) or die("Query failed,warphar");
	$query = "CREATE TEMPORARY TABLE drugrx02 SELECT * FROM drugrx WHERE date like '$date%' AND hn = '$hn' and price >0 and status ='Y' ";
	$result = mysql_query($query) or die("Query failed,warphar");
	...
	?>
		...
		while (list($drugcode, $tradname, $amount, $price, $part) = mysql_fetch_row($result13)) {
			$dsql = mysql_query("select code24 from druglst where drugcode='$drugcode'");
			list($code24) = mysql_fetch_row($dsql);
	...
	$result10 = mysql_query($query10)
		or die("Query failed");
	while ($fetch = mysql_fetch_array($result10)) {
		$query13 = "SELECT drugcode,tradname,amount,price,part FROM drugrx02 WHERE idno = '" . $fetch['row_id'] . "' and (part = 'DDY' or part = 'DDN')";
		$result13 = mysql_query($query13)
			or die("Query failed");
		$nn = @mysql_num_rows($result13);
		if ($nn == "0") {
	...
	$query = "CREATE TEMPORARY TABLE depart01 SELECT * FROM `depart` WHERE `date` LIKE '$date%' AND `status` = 'y' AND `price` > 0 ";
	$result = mysql_query($query) or die("Query failed,warphar");
	...
	while ($rowid = mysql_fetch_array($result)) {
		...
		while (list($code, $detail, $amount, $price, $yprice, $nprice) = mysql_fetch_row($result10)) {
			...
			print(" <tr bordercolor='#FFFFFF'>\n" .
				...
				"  <td align='center'>$sum</td>\n" .
				" </tr>\n");
			if ($yprice == "0.00") $price = 0;
			else $price = $yprice;
			if ($nprice == "0.00") $price1 = 0;
			else $price1 = $nprice;
			$payyes += $price;
			$payno += $price1;
			switch ($code) {
	...
	////////////////////////////////////////////////////////////
	$query = "SELECT  * FROM depart01 
	WHERE depart NOT IN (  'EMER',  'HEMO',  'WARD',  'PATHO',  'XRAY',  'SURG',  'DENTA',  'PHYSI',  'NID') 
	AND hn =  '$hn' 
	AND date LIKE  '$date%'";
	...
	<tr bordercolor="#333333">
		...
		<td align="center"><strong><?= number_format($payyes, 2) ?></strong></td>
		<td align="center"><strong><?= number_format($payno, 2) ?></strong></td>
