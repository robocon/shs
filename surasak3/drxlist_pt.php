...
print ("<table>
 <tr>
  ...
  $query = "SELECT tvn, date,ptname,hn,price,row_id,accno,ptright,doctor, stkcutdate,kew,kewphar,pharin,idname FROM dphardep_pt WHERE  whokey='DR' and date LIKE '$today%' AND dr_cancle is null  ORDER BY stkcutdate, hn  DESC ";
    ...
    while (list ($tvn,$date,$ptname,$hn,$price,$row_id,$accno,$ptright,$doctor, $stkcutdate,$kew,$kewphar,$pharin,$idname) = mysql_fetch_row ($result)) {
		...
		if($tvn==""){
			print "  <td BGCOLOR='".$bgcolor."'><font face='Angsana New'>$ptname</td>\n";
       	 	...
       	 	print   "  <td BGCOLOR='".$bgcolor."'><font face='Angsana New'><a target=_BLANK  href=\"drxdetail_pt.php? sDate=$date&nRow_id=$row_id&nAccno=$accno&sPtright=$ptright\">$ptname</a></td>\n";
		}
...
function searchSuggest() {
			...
			url = 'drxlist_pt.php?action=refresh&d=<?php echo $_GET["d"];?>&m=<?php echo $_GET["m"];?>&yr=<?php echo $_GET["yr"];?>';
			xmlhttp = newXmlHttp();
...
<FORM METHOD=GET ACTION="drxlist3_pt.php" target="_blank">
<TABLE>
<TR>
	<TD>VN : </TD>
