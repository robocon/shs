<html>
<head>
<title>Fort Surasakmontri Hospital</title>
<style type="text/css"> 
@media print
{
...
<?php
	...
	print  "<font face='Angsana New'>(ดูความเคลื่อนไหวของยาแต่ละเดือนในห้องจ่ายยา)";
...
//         print "$today<br>";
  //        print "$drugcode<br>";
   ...
   print "<b>ข้อมูลปัจจุบัน ($Thaidate)</b><br>"; 
         ...
         print "<br><b>รายการเบิกจากคลังยาของเดือน $m-$yr</b> ";
///////////
    $query = "SELECT getdate,billno,drugcode,lotno,department,unitpri,amount,stkcut,netlotno,mainstk,stock,totalstk         FROM stktranx  WHERE drugcode = '$drugcode'  and department='ห้องจ่ายยา' and getdate LIKE '$today%' ORDER BY getdate ";
	...
	$mainstkpri =$unitpri*$mainstk;
        ...
        print "</table>";
    $query = "SELECT date,hn,amount,price,idno FROM drugrx  WHERE drugcode = '$dcode' and date LIKE '$thday%'  ORDER BY date ";
     $result = mysql_query($query)
        or die("Query failed");
...
//   echo "<br>";
  	$nTotal=0;
    $nTotalpri=0;
    while (list($date,$hn,$amount,$price,$idno ) = mysql_fetch_row ($result)) {
    ...
    print "<br><b>รายงานการจ่ายจากคลังยาของเดือน $m-$yr</b>";
	print "<table>";
 	print "<tr>";
