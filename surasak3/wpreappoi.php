...
if (isset($_GET["action"])  && $_GET["action"] == "viewlist") {
	...
	<TABLE bgcolor='#FFFFD2' style='font-size: 24px;'>
	<TR>
		<TD>";
	for ($i = 0; $i < $count; $i++) {
	...
	if (Mysql_num_rows($result) > 0) {
		echo "<Div style=\"position: absolute;text-align: left; width:520px; height:230px; overflow:auto; border: 1px solid #000000; box-shadow: 4px 4px 4px #747474;background-color:#F2F3F4;\">";

		echo "<table bgcolor=\"#FFFFCC\" width=\"520\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
...
?>
<p class="forntsarabun">
	<strong>ชื่อ</strong>: <?= $ptname ?> <br>
	<strong>HN</strong>: <?=$hn;?> <strong>AN</strong>: <?=$an;?> <strong>อายุ</strong>: <?=$age;?> <br>
	<strong>สิทธิ</strong>: <?=$ptright;?><br>
	<strong>แพทย์</strong>: <?= $doctor; ?>
</p>
<?php
...
<TABLE border="0" class="forntsarabun">
	<TR valign="top">
		<TD width="50%">
			<form method="POST" action="wappinsert1.php?an=<?= $an; ?>&cBed=<?= $bed; ?>&cBedcode=<?= $bedcode; ?>&cbedname=<?= $_GET['cbedname'] ?>">

				<div id="list_patho">
					<font class="forntsarabun"><A HREF="javascript:show_bock();">คลิกเลือกรายการเจาะเลือด</A></font>
				</div>
		...
		<TD>
			...
			<TABLE id="bock_lab" width="100%" border="1" bordercolor='#000000' cellpadding="3" cellspacing="0" style="display:none; font-size: 28px;">
				<TR valign="top">
					<TD width="500">
						<div align="center"><B>รายการตรวจทางพยาธิ</B></div>
						<TABLE width="100%" align="left" border="0" cellpadding="5" style="font-size: 20px;">
							<TR valign="top">
							...
							<TR>
							<TR>
								<TD><strong>รายการที่ใช้บ่อย</strong></TD>
							</TR>
							<?php
							for ($i = 1; $i <= $count; $i++) {
