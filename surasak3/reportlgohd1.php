...
$sql = "SELECT x.*, y.`hd_room`
FROM ( 
	Select a.date,a.txdate, a.hn, CONCAT(b.yot,' ',b.name,' ',b.surname) as full_name, a.depart, sum(a.price),b.idcard,
	b.note,sum(a.paidcscd) 
	From opacc as a, 
	opcard as b 
	where a.hn=b.hn 
	AND a.txdate like '".$date1."%' 
	AND a.credit ='จ่ายตรง อปท. (HD)' 
	group by a.hn, a.depart 
	ORDER by a.date 
) AS x LEFT JOIN ( 
	SELECT `hn` AS `hn2`,SUBSTRING(`detail`,1,4) AS `hd_room` 
	FROM `appoint` 
...
while(list($date,$txdate, $hn, $full_name, $depart, $price,$idcard,$note,$paidcscd, $hd_room) = Mysql_fetch_row($result)){
 ...
 $d=substr($date,8,2);
  ...
  $note=substr($note,0,25);
	...
	$list2[$hn] = $d."".$m."".$y."/".$full_name."/".$idcard."/".$note.'/'.$hd_room;
switch($depart){
...
$num='0';
$i='0';
echo "<font face='Angsana New' size ='4'><center> <b>ลูกหนี้ผู้ป่วยนอก สิทธิจ่ายตรง อปท. (HD) ประจำวันที่ $date2 <br></b> ";
...
<td><font face='Angsana New' size ='1'><center> <b>&nbsp;&nbsp;ห้อง&nbsp;&nbsp;</td>
...
$datetime=$day.' '.$timeth;
	...
	<td><font face='Angsana New' size ='1'>&nbsp;&nbsp;".($xx[4]=='FU18' ? 'ไตเทียม 1' : 'ไตเทียม 2')."&nbsp;</td>
...
echo "<font face='Angsana New' size ='4'><center> <b>ลูกหนี้จ่ายตรง อปท. (HD) ประจำวันที่ $date2 <br></b> ";
echo "<font face='Angsana New' size ='3'> โรงพยาบาลค่ายสุรศักดิ์มนตรี ลำปาง </center>";
