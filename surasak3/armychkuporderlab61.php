...
<input name="Submit" type="submit" value="คลิกที่นี่ เพื่อนำเข้า Lab ตรวจสุขภาพทหาร รพ.ค่ายฯ" />
</form>
<p align="center" style="color:#FF0000; font-size:28px; font-weight:bold;">!!! คลิกแค่ครั้งเดียว เฉพาะวันที่ 6/12/60 เท่านั้น</p>
</p>
<?
if($_POST["act"]=="add"){
	include("connect.inc"); 
	//$sql="SELECT * FROM chkup_solider where (yearchkup='61' and active='' and camp = 'D01 รพ.ค่ายสุรศักดิ์มนตรี') order by  row_id asc";
	$sql="SELECT * 
FROM chkup_solider
WHERE (
yearchkup =  '61'
) AND (
hn =  '47-3885' OR hn =  '47-6296'
)
ORDER BY row_id ASC";
	//echo $sql;
