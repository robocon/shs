...
$filePath = $dirPath.'/'.$date_start.'_'.$date_end.'_drug.sql';

unlink($filePath);

$sql = "SELECT `row_id`,`date`,`hn`,`drugcode`,`amount`,CONCAT(SUBSTRING(`date`,1,10),`hn`) AS `date_hn` 
FROM `drugrx` 
...
$sql_header = "INSERT INTO `drugrx` ( `id`,`row_id`,`date`,`hn`,`drugcode`,`amount`,`date_hn`,`date_generate`,`quarter`) VALUES ";
$sql_data = '';
...
while ( $item = mysql_fetch_assoc($q) ) {
    ...
    $sql_data = $sql_header."( NULL,'$row_id','$date','$hn','$drugcode','$amount','$date_hn',NOW(),'$quarter');\n";
