...
if($_POST["submit_update"] == "ล็อกยา" && $_POST["text_alert"] == ""){
	echo "<CENTER>ถ้าต้องการล็อกยากรุณาพิมพ์ข้อความแจ้งแตือนแพทย์ด้วยครับ</CENTER>";
	echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"5;URL=lock_drug_dr.php\">";
exit();
}
...
if($_POST["submit_update"] == "ล็อกยา"){
	...
	$status = "Y";
		...
		if(count($_POST["row_id"]) > 0){
			...
			$sql = "Update druglst set `lock_dr` = '{$status}' Where row_id in ('".implode("','",$_POST["row_id"])."') ";
			$result = Mysql_query($sql);

		}		
		echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0;URL=lock_drug_dr.php\">";
		exit();
...
<TR bgcolor="<?php echo $bgcolor;?>">
	...
	<TD><?php if($arr["lock_dr"]=="Y") echo ""; else{echo $arr["lock_dr"];}?></TD>
...
?>
<TR>
	<TD colspan="1" align="center"><INPUT TYPE="text" NAME="text_alert" size="10" maxlength="15" value="<?php if($rea=="Y") echo ""; else{echo $rea;}?>"></TD>
