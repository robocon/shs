...
<?
		...
		$nPrefix2="25".$nPrefix;
?>
<p align="center" style="margin-top: 20px;"><strong>รายงานผลการตรวจสุขภาพประจำปีกองทัพบก กลุ่มที่เป็นโรคอ้วน (BMI > 25)</strong></p>	
<form name="form1" method="post" action="report_chkuparmy_risk_bmi.php" >
...
if($_POST["act"]=="show"){
	if($_POST["camp"]=="all"){
		$sql="select * from condxofyear_so where yearcheck='".$_POST["year1"]."' and bmi > 25 group by hn order by camp, age, row_id desc";
	}else{
		$sql="select * from condxofyear_so where yearcheck='".$_POST["year1"]."' and bmi >25 and camp='".$_POST["camp"]."' group by hn order by camp, age, row_id desc";
	}
	//echo $sql;
	$query=mysql_query($sql);	
?>	
<p align="center" style="margin-top: 20px;"><strong>รายงานผลการตรวจสุขภาพประจำปีกองทัพบก กลุ่มที่เป็นโรคอ้วน (BMI > 25) ปี <?=$nPrefix2;?></strong></p>
<div align="center">
