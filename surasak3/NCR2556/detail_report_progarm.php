...
<TR>
	...
	<?php
	if(isset($_GET['risk']) && isset($_GET['until'])){  /// GET risk และ until มีค่าส่งมา
	$risk=$_GET['risk'];
	$until=$_GET['until'];
	$where="AND  $risk = '1' AND until='$until' ";
	
	}elseif(isset($_GET['risk']) && isset($_GET['clinic'])){/// GET risk และ clinic มีค่าส่งมา
	
	$where="AND  $risk = '1' AND clinic='".$_GET['clinic']."' ";
	}elseif(isset($_GET['risk'])){
	$where="AND  $risk = '1'";
	}elseif(isset($_GET['until'])){
	$where="AND  until = '".$_GET['until']."' ";	
	}else{
	$where="";
	}

		$sql = "Select date_format(nonconf_date,'%d/%m/%Y')as nonconf_date, left(nonconf_time,5)as nonconf_time ,a.*  From  ncr2556 as a  where nonconf_date like '".$_GET["y"]."-".$_GET["m"]."%'  $where  Order by nonconf_id DESC ";
