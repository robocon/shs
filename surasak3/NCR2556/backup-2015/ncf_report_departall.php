...
$selectsql = "SELECT COUNT(*)as count   FROM    ncr2556  WHERE until ='".$arruntil['newuntil']."' and nonconf_date  like '".$date1."-".$m."-%'";
		$result = mysql_query($selectsql);
		$arr01 = mysql_fetch_array($result);
		
 ?>
<? if ($arr01['count']!=0){?>
  <td align="center" class="forntsarabun"><a href="detail_report_progarm.php?y=<?=$date1;?>&m=<?=$m;?>&until=<?=$arruntil['newuntil']?>" target="_blank"><?=$arr01['count'];?></a></td>
 <? 
	}else{
?>
...
$selectsql = "SELECT COUNT(*)as count   FROM    ncr2556  WHERE nonconf_date  like '".$date1."-".$m."-%'"; 	
$result = mysql_query($selectsql);
$arr01 = mysql_fetch_array($result);	
$sum2=0;
		for($a=0;$a<=11;$a++){
