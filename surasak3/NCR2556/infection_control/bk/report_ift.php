...
$mysql="Select * From ic_infection where an = '".$_POST["an"]."' limit 1 ";
$myresult = mysql_query($mysql);

	$myrow=mysql_num_rows($myresult);
	if($myrow){
	$myarr=mysql_fetch_array($myresult);
	$action = "report_ift.php?do=edit";
	}else{
	$action = "report_ift.php?do=add";
	}
}else{
...
`initial_diag` =  '".$_POST['initial_diag']."' WHERE  `row_id` =  '".$_POST['row_id']."' LIMIT 1 ;";
	...
	//echo "<BR><A HREF=\"report_ift.php\">บันทึกเพิ่ม</A>";
