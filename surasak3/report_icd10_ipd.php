...
HN</td>
    <td > *เลือกการค้นหา จากวันที่จำหน่าย หรือ HN    <a target=_self  href='../nindex.htm'><----ไปเมนู</a></td>
  </tr>
    <tr>
...
<table>
 <tr class="fonthead">
  <th bgcolor=CD853F>วันadmit</th>
  <th bgcolor=CD853F>วัน dc</th>
...
if($_POST['btnSubmit']){
 ...
 $thisdate=$_POST['y_start'].'-'.$_POST['m_start'].'-'.$_POST['d_start'];
  ...
  if($_POST['rdo4']=="Y"){
	$query = "SELECT * FROM ipcard WHERE  dcdate  like  '$thisdate%' ORDER BY  dcdate  DESC   ";

  }else{
	  
	  $query = "SELECT *  FROM ipcard WHERE  hn = '$hn' ORDER BY date DESC   ";
  }
...
<tr class="fontlist">
   <td BGCOLOR=F5DEB3  valign="top"><?=$arr['date'];?></a></td>
   <td BGCOLOR=F5DEB3  valign="top"><?=$arr['dcdate'];?></a></td>
  <td BGCOLOR=F5DEB3  valign="top"><?=$arr['hn'];?></td>
  <td BGCOLOR=F5DEB3  valign="top"><?=$arr['an'];?></td>
  <td BGCOLOR=F5DEB3 valign="top"><?=$arr['ptname'];?></td>
  <td BGCOLOR=F5DEB3 valign="top"><?=$arr['ptright'];?></td>
  <td BGCOLOR=F5DEB3 valign="top"><?=$arr['diag'];?></td>
  <td BGCOLOR=F5DEB3  valign="top">
  <?   $sql1="select * from diag where hn ='".$arr['hn']."'   and an = '".$arr['an']."' and  type='PRINCIPLE' ";
  ...
  <td BGCOLOR=F5DEB3  valign="top">
  <?
  		 $sql2 = "SELECT * FROM diag where hn ='".$arr['hn']."'   and an = '".$arr['an']."' and  type='CO-MORBIDITY' ";
  ...
  <td BGCOLOR=F5DEB3 valign="top">
  <?
  		 $sql3 = "SELECT * FROM diag where hn ='".$arr['hn']."'   and an = '".$arr['an']."' and  type='COMPLICATION' ";
  ...
  <td BGCOLOR=F5DEB3  valign="top">
   <?
  		 $sql4 = "SELECT * FROM diag where hn ='".$arr['hn']."'   and an = '".$arr['an']."' and  type='OTHER' ";
  ...
  <td BGCOLOR=F5DEB3  valign="top">
   <?
  		 $sql5 = "SELECT * FROM diag where hn ='".$arr['hn']."'   and an = '".$arr['an']."' and  type='EXTERNAL CAUSE' ";
  ...
  <?
  		$sql6="select * from ipicd9cm where an = '".$arr['an']."'  ";
