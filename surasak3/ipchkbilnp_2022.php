...
if ($row->part=="BFN"){
			...
			if(($row->price-$row->paid)>0){
			array_push($rowN,$row->row_id);
			}
...
////
$query = "SELECT date,depart,code,detail,amount,price,paid,part,idname,billno,startdatetime,enddatetime FROM ipacc WHERE an = '$cAn'  ORDER BY date ASC ";
    $result = mysql_query($query)
        or die("Query failed ipacc");
    $num=0;
    while (list ($date,$depart,$code,$detail,$amount,$price,$paid,$part,$idname,$billno,$startdatetime, $enddatetime) = mysql_fetch_row ($result)) {
	    $num++;
...
$np=number_format($nNprice,2,'.','');


print "<b>ส่วนเกินทั้งหมด $np บาท</b><br>";
//print "จนท. $sOfficer วันที่ $Thaidate<br>";

    print "<form name='f1'  method='POST'  action='ipbillnp.php' Onsubmit='return checkformf1()' >";
    print "เก็บเงินทั้งหมด&nbsp;&nbsp;&nbsp; <input type='text' name='paid' size='10' value=$np>&nbsp;&nbsp;บาท&nbsp;&nbsp;ใบเสร็จเลขที่&nbsp;<input type='text' name='billno' size='10'><br>";
