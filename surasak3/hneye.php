...
if(!empty($hn)){
	...
	//$cHn=$hn;
    include("connect.inc");
    $vnlab = 'EX94 ออก VN โดย คลีนิกตา';   
    $today = date("d-m-Y");   
    $d=substr($today,0,2);
    $m=substr($today,3,2);
...
$result = mysql_query($query)or die("Query failed,opday");
	...
	$sql="select * from cliniceye where date_time like '$subtoday%' && hn='$cHn' && vn='$tvn'";
	$result=mysql_query($sql);
	$num=mysql_num_rows($result);
		if($num < 1){
		$add="insert into cliniceye set date_time='$today', hn='$cHn', vn='$tvn'";
