...
/**
 * - หา drugcode จาก hn มาก่อน จากนั้นเอารายการยาไปหาใน drugreact_group_list อีกทีว่าจริงๆแล้วรายการยาทีแพ้ จัดอยู่ในกลุ่มไหนบ้าง
 * - เสร็จแล้วค่อยเอามา join กับ drugreact_gorup อีกทีเพื่อแสดงชื่อของกลุ่มที่แพ้ทั้งหมด
 */
$sql = "SELECT b.* FROM 
( 
	SELECT `drugcode`,`drugreact_group`
	FROM `drugreact_group_list` 
	WHERE `drugcode` IN (SELECT `drugcode` FROM `drugreact` WHERE hn = '$my_hn' GROUP BY `drugcode`) 
	GROUP BY `drugreact_group`
) AS a 
LEFT JOIN `drugreact_group` AS b ON a.`drugreact_group` = b.`id`";
dump($sql);
$res = $dbi->query($sql);
$groupnameList = array();
if ($res->num_rows>0) { 
...
$sql1 = "SELECT y.* FROM ( 
	...
	SELECT c.* FROM ( 
		SELECT `row_id`,`hn`,`drugcode`,`tradname` FROM `drugreact` WHERE `hn` = '$my_hn' AND `drugcode` != '' GROUP BY `drugcode`
	) AS a 
	LEFT JOIN `drugreact_group_list` AS b ON a.`drugcode` = b.`drugcode` 
	LEFT JOIN `drugreact_group` AS c ON b.`drugreact_group` = c.`id`
	WHERE c.`name` IS NOT NULL 
	GROUP BY c.`name` 

) AS x 
LEFT JOIN `drugreact_group_list` AS y ON x.`id` = y.`drugreact_group`";
$res = $dbi->query($sql1);
 ...
 */
if($_GET["action"] == "drugcode" && !empty($_GET['search'])){
...
<TR>
	<TD>
		<?php 
			// แสดงรายการยาที่แพ้
			$sql = "SELECT `drugcode`, `tradname`, `advreact`, `groupname` FROM `drugreact` WHERE `hn` = '".$arr["hn"]."' ";
			$result = Mysql_Query($sql);
			$rows = Mysql_num_rows($result);
			if($rows> 0){
	...
	<td valign="top">
		...
		if (count($groupnameList)>0) {
			...
			foreach ($groupnameList as $key => $value) {
				echo "<tr><td colspan='6'><font color=\"red\">$i) ".$value['name']."</font></td></tr>";
				$i++;
			}
		}
		?>
...
<TABLE width="55%" align="center" cellpadding="6" cellspacing="3">
	...
	<TR>
		<TD style="position: relative;">
			<strong>จำนวน : </strong>
			<div id="listdrugcode" style="position: absolute; top:0; left:0; width:800px; height:auto; overflow:auto;"></div>
		</TD>
