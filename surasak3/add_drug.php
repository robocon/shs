...
$sqlGroup = "SELECT `groupname` FROM `drugreact` WHERE hn = '$my_hn' AND groupname <> '' AND sideeffects='' GROUP BY `groupname`";
...
<TR>
	<TD>
		<?php 
			// แสดงรายการยาที่แพ้
			$sql = "SELECT `drugcode`, `tradname`, `advreact`, `groupname` FROM `drugreact` WHERE `hn` = '".$arr["hn"]."' AND advreact != '' AND g6pd <> '' ";
			$result = Mysql_Query($sql);
			$rows = Mysql_num_rows($result);
			if($rows> 0){
