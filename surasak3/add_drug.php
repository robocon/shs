 ...
 */
$ThidateTime = (date("Y")+543).date("-m-d H:i:s");
...
if(isset($_POST["Save_dgprofile"]) && $_POST["Save_dgprofile"] == "บันทึกข้อมูลใน DrugProfile" ){
	...
	if($result == true || $_SESSION["num_list"] > 0){
		...
		<div style="text-align:center;">
			...
			<p>
				<A HREF="phardividedrug.php?an=<?= $_GET["an"]."&bed=".$_GET["bed"]."&bedcode=".$_GET["bedcode"] ?>" class="button">ตัดจ่ายยา</A>&nbsp;
				&nbsp;<A HREF="enddrugprofile.php" class="button">กลับหน้าward</A>&nbsp;
				&nbsp; <a href="add_drug.php?an=<?= $_GET["an"] ?>&bed=<?= $_GET["bed"] ?>&bedcode=<?= $_GET["bedcode"] ?>&date=<?= $_GET["date"] ?>" class="button">กลับหน้า เพิ่ม/แก้ไข/OFF ยา AN <?= $_GET["an"] ?></a> &nbsp;
			</p>
		</div>
		<?php
	}else{
...
<TABLE width="55%" align="center" cellpadding="6" cellspacing="3">
	...
	<TR valign="top">
		<TD style="position: relative;" align="right">
			<strong>จำนวน : </strong>
			<div id="listdrugcode" style="position: absolute; top:0; left:0; width:960px; height:auto; overflow:auto; background-color:#ffffff; text-align:left;"></div>
		</TD>
...
<div class="drug-off-container drug-group">
	<div><h3 style="text-align: left;">รายการยาที่ OFF [ <a href="javascript:void(0);" onclick="hideShowDrugOff()">ซ่อน/แสดง</a> ]</h3></div>
	<div id="drug-off-container" style="display:none;">
	<table width="100%">
	...
	<script>
		function hideShowDrugOff(){
			var x = document.getElementById("drug-off-container");
...
<div class="drug-stat drug-group">
	<h3 style="text-align: left;">รายการยา STAT ที่เคยจ่าย [ <a href="javascript:void(0);" onclick="hideShow()">ซ่อน/แสดง</a> ]</h3>
	<div id="drug-stat-container" style="display:none;">
		<table width="100%">
...
<script>
	function hideShow(){
		var x = document.getElementById("drug-stat-container");
		if (x.style.display === "none") {
			x.style.display = "block";
			x.style.transitionDelay = "0s";
		} else {
			x.style.display = "none";
			x.style.transition = "visibility 0s linear 0.33s, opacity 0.33s linear";
		}
	}
</script>

<div class="drug-old drug-group">
	<h3 style="text-align: left;">รายการยาเดิม [ <a href="javascript:void(0);" onclick="hideShowDrugOld()">ซ่อน/แสดง</a> ]</h3>
	<div id="drug-old-container" style="display:none;">
	<table width="100%">
...
<script>
	function hideShowDrugOld(){
		var x = document.getElementById("drug-old-container");
		if (x.style.display === "none") {
			x.style.display = "block";
		} else {
			x.style.display = "none";
