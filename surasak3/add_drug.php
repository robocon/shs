...
$sqlGroup = "SELECT `groupname` FROM `drugreact` WHERE hn = '$my_hn' AND groupname <> '' GROUP BY `groupname`";
...
// หากลุ่มที่แพ้ใน drugreactก่อน จากนั้นถ้ามีการบันทึกกลุ่ม ก็เอาไปหาใน drugreact_group_list แสดงเป็นรายการที่มีโอกาสแพ้ตามกลุ่ม
$sql1="SELECT b.* FROM ( 
...
if($_GET["action"] == "drugcode" && !empty($search_txt)){
						...
						while($arr = $res->fetch_assoc()){
							...
							if(in_array($mydrugcode, $drugreact_list)===true){
								...
								if(in_array($mydrugcode, $drugreact_groups)===true){
									$relative_react_txt = '<span style="background-color: #ffeb3b; font-weight: bold;padding: 0 8px;">แพ้ยาในกลุ่ม</span>';

									if ($opcard_g6pd===true && in_array($mydrugcode, $drugreactGroup10)===true) {
	...
	exit;
}elseif ($_GET["action"] == "drugcode" && empty($search_txt)) {
	?>
	<p style="background-color:#ffffff;padding:8px;margin:0;border:2px solid #000000;"><b>ไม่พบข้อมูล</b></p>
	<?php
	exit;
	...
	<td valign="top">
		...
		if (count($groupnameList)>0) {
			...
			foreach ($groupnameList as $key => $value) {
				echo "<tr><td colspan='6'><font color=\"red\">$i) ".$value['groupname']."</font></td></tr>";
				$i++;
			}
		}
		?>
...
<TABLE width="55%" align="center" cellpadding="6" cellspacing="3">
	...
	<TR>
		<TD style="position: relative;">
			<strong>จำนวน : </strong>
			<div id="listdrugcode" style="position: absolute; top:0; left:0; width:800px; height:auto; overflow:auto; background-color:#ffffff;"></div>
		</TD>
