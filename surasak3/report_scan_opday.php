...
<?php
	...
	if(!empty($_POST["hn"])){
		...
		$query = "SELECT row_id,thidate,thdatehn,ptname,hn,ptright,doctor,vn,clinic,toborow,an,icd10,diag,dxgroup,goup,okopd,officer2,age FROM opday WHERE thidate like '$date%' ORDER BY toborow,thidate ASC ";
...
if($opday_id==0){  //เป็นใบ summary หรือ ไม่ได้สแกน
	$num_noscan++;
	...
	$statusicd10="#EAEDED";	
        ...
        print (" <tr BGCOLOR='$statusicd10'>\n".
		   ...
		   "  <td align='center' style='color:blue;font-size:28px;'><strong>$countfile</strong></td>\n".
		   "  <td>$last_update</td>\n".
		  /* "  <td>$regisdate</td>\n".	*/	   
		   " </tr>\n");
       }
...
<?
	...
	$numother=$numscan_today-$num_scan;
	//echo "==>".$_POST["case"];
...
echo "<div align='center' style='margin-top:20px; color:black; font-weight:bold; font-size:28px;'> เป็นเอกสารของผู้ป่วยวันนี้ จำนวน $num_scan ราย เป็นใบ summary หรือ เอกสารของวันอื่นๆ จำนวน $numother ราย</div>";	
...
echo "<div style='margin-left: 150px; margin-top:20px; color: green; font-weight:bold; font-size:28px;'>- จำนวนที่สแกนเข้าระบบ $num_scan ราย  <a href='report_digital_scan.php?getdate=$date' target='_blank'>ดูรายชื่อ <img src='images/detail-green.png' height='28px' width='28px' /></a></div>";
echo "<div style='margin-left: 150px; margin-top:20px; margin-bottom: 50px;  color: red; font-weight:bold; font-size:28px;'>- จำนวนที่เป็นใบ summary หรือ ยังไม่ได้สแกนเข้าระบบ $num_noscan ราย  <a href='report_digital_noscan.php?getdate=$date' target='_blank'>ดูรายชื่อ <img src='images/detail-red.png' height='28px' width='28px' /></a></div>";
