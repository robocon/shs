...
return $ageY;
	...
	$_SESSION["cDoctor"]=$_POST['cDoctor'];
	$_SESSION["Netprice"]=$_POST['cPrice'];
	$aSumYprice=array_sum($aYprice);
	$aSumNprice=$_POST['cPrice'];
   ...
   for ($n=1; $n<=$x; $n++){
        If (!empty($aDgcode[$n])){
	    	$aNprice[$n]=$_POST['cPrice'];
			$aMoney[$n]=$_POST['cPrice'];
            $item++;
	}
            };
	...
	for ($n=1; $n<=$x; $n++){
		if(!empty($aDgcode[$n])){
			if($aPart[$n]=="LAB"||$aPart[$n]=="NCARE"||$aPart[$n]=="TOOL"||$aPart[$n]=="XRAY"||$aPart[$n]=="MC"||$aPart[$n]=="SINV"||$aPart[$n]=="SURG"||$aPart[$n]=="PT"||$aPart[$n]=="DENTA"||$aPart[$n]=="OTHER"||$aPart[$n]=="BLOOD"||$aPart[$n]=="STX"){
				$query = "INSERT INTO ipacc(date,an,code,depart,detail,amount,price,yprice,nprice,idname,part,accno,idno) VALUES('$Thidate','$cAn','$aDgcode[$n]','$cDepart','$aTrade[$n]','$aAmount[$n]','$aYprice[$n]','$aYprice[$n]','0','$sOfficer','$aPart[$n]Y','$cAccno','$idno');";
				$result = mysql_query($query) or die("Query failed,cannot insert into ipacc1");//เบิกได้
				if($aNprice[$n]>0){
					$query = "INSERT INTO ipacc(date,an,code,depart,detail,amount,price,yprice,nprice,idname,part,accno,idno)VALUES('$Thidate','$cAn','$aDgcode[$n]','$cDepart','$aTrade[$n](ส่วนเกิน)','$aAmount[$n]','$aNprice[$n]','0','$aNprice[$n]','$sOfficer','$aPart[$n]N','$cAccno','$idno');";
					$result = mysql_query($query) or die("Query failed,cannot insert into ipacc2");//เบิกไม่ได้
				}
			}else{
				$query = "INSERT INTO ipacc(date,an,code,depart,detail,amount,price,yprice,nprice,idname,part,accno,idno) VALUES('$Thidate','$cAn','$aDgcode[$n]','$cDepart','$aTrade[$n]','$aAmount[$n]','$aMoney[$n]','$aYprice[$n]','$aNprice[$n]','$sOfficer','$aPart[$n]','$cAccno','$idno');";
