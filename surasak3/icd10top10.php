...
<form method="POST" action="icd10top10.php">
	...
	<table>
        ...
        <tr>
        <td><fieldset>
					<legend><span class="forntsarabun">สิทธิการรักษา</span></legend>
                    <div>
                        <select name="ptright" class="forntsarabun">
                         <option value="0">ทุกสิทธิ</option>
                         <option value="R07">ประกันสังคม</option>
                         <option value="R09">ประกันสุขภาพถ้วนหน้า</option>
                       </select>
					</div>
         		</fieldset>                           
         </td>
...
if(isset($_POST['B1'])){
	...
	if($_POST['ptright']=="0"){
		$ptright="ทุกสิทธิ";
	}else if($_POST['ptright']=="R07"){
		$ptright="ประกันสังคม";
	}else if($_POST['ptright']=="R09"){
		$ptright="ประกันสุขภาพถ้วนหน้า";
	}

	if( $_SESSION['smenucode'] === 'ADMOPD' ){
		print "<font class=\"forntsarabun\"><b>รายชื่อผู้ป่วยตาม ICD10 ประจำเดือน</b> $yrmonth <strong>สิทธิการรักษา</strong> $ptright";
		...
		print "<font class=\"forntsarabun\"><b>รายชื่อผู้ป่วยตาม ICD10 ช่วงเดือน</b> $yrmonth <b>ถึง</b> $end_date <strong>สิทธิการรักษา</strong> $ptright";
	}
	...
	WHERE $opday_where 
	AND `icd10` !='' 
	AND (`an` ='' OR `an` IS NULL) AND ptright LIKE '".$_POST['ptright']."%'";
	$resultdb1 = mysql_query($querydb1) or die(mysql_error());
	...
	FROM `ipcard` 
	WHERE $ipcard_where 
	AND `icd10` !=''  AND ptright LIKE '".$_POST['ptright']."%'";
	$resultdb2 = mysql_query($querydb2) or die(mysql_error());	
	}
