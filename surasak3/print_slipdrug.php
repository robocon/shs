...
if($_GET["type"]=="SI"){
	...
	$result = Mysql_Query($sql) or die(Mysql_Error());
		...
		$sql2 = "Select thdatehn,status_day1,status_day2 From opselfisolation where hn = '".$_GET["hn"]."' order by row_id desc limit 1";
		//echo $sql2."<br>";
	...
	$sql ="UPDATE opday SET opdtype='".$_GET["type"]."',opdcolor='".$_GET["color"]."'  WHERE  thdatehn='".$thdatehn."'";   // แก้ไขข้อมูลตาราง opday
	$result = Mysql_Query($sql) or die(Mysql_Error());	
}else if($_GET["type"]=="OP"){
	$type="ผู้ป่วยทั่วไป";
	$sql ="UPDATE opday SET opdtype='".$_GET["type"]."',opdcolor=''  WHERE  thdatehn='".$thdatehn."'";   // แก้ไขข้อมูลตาราง opday
	$result = Mysql_Query($sql) or die(Mysql_Error());	
	
	$del ="DELETE FROM opselfisolation WHERE thdatehn='".$thdatehn."'";   // ลบข้อมูลในตาราง opselfisolation
	//echo $del;
