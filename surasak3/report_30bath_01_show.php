...
$i = 1; 
//-----> sql
/*$sql = "SELECT a.* , b.idcard, b.phone,  c.`authen_code`, c.`certifier`, c.`authen_method`
FROM 
( 
...
( 
	...
	AND `part` = 'LAB' 
	AND `status` = 'Y' 
	AND (`code`='SWAB' OR `code`='AgCG3' OR `code`='Covid19' OR `code`='AGCG3_UC') 
) AS a 
LEFT JOIN `opcard` AS b ON b.`hn` = a.`hn` 
LEFT JOIN ( 
	SELECT * 
...
<table border="1" width="90%" id="table1">
	...
	<td align="center" style="font-size:15"><b>Authen Code</b></td>
	<td align="center" style="font-size:15"><b>ผู้รับรอง</b></td>
	<td align="center" style="font-size:15"><b>การเข้ารับบริการ</b></td>
	</tr>
<?
$tmp_hn = "";
...
if($tmp_hn != $hn){
  ...
  echo '
	...
	<td align="left" style="font-size:15">'.$authen_code.'</td>
	<td align="left" style="font-size:15">'.$certifier.'</td>
	<td align="left" style="font-size:15">'.$authen_method.'</td>
	</tr>
  ';
