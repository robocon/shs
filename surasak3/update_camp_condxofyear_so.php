...
<form name="form1" method="post" action="update_camp_condxofyear_so.php" >
  ...
  <table width="50%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	&nbsp;&nbsp;หน่วย :
        ...
        <select name="camp" id="camp" class="txtsarabun">
		 ...
		 <?
		 $sql="select distinct(camp) as camp from condxofyear_so where `camp` like 'M03%'";
...
$nPrefix=$_POST["year1"];
	...
	if($_POST["camp"]=="all"){
		$sql1="SELECT * FROM `condxofyear_so` WHERE `camp` like 'M03%'
		GROUP BY hn 
		ORDER BY row_id ASC";
	}else{
		$sql1="SELECT * FROM `condxofyear_so` WHERE `camp` like 'M03%'
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;" class="pdxpro">
  ...
  <?php
	...
	$opsql="select hn,yot,name,surname,idcard,sex, DAY(dbirth) as bday,MONTH(dbirth) as bmonth,(YEAR(dbirth)) as byear,camp from opcard where hn='$arr1[hn]'";
	//echo $opsql."<br>";
	$opquery=mysql_query($opsql);
	list($hn,$yot,$name,$surname,$idcard,$gender,$bday,$bmonth,$byear,$camp)=mysql_fetch_row($opquery);	
	
	$edit="update condxofyear_so SET camp ='$camp' WHERE hn='$hn'";
	//echo $edit."<br>";
	mysql_query($edit);
