...
if($today11==$dVndate){
		...
		//print "<font face='TH SarabunPSK' size=5>$today11</font><br>";
}
...
if($sumamount > 1){  //ถ้าเดือนนี้รับยาไปมากกว่า 1 หลอดให้ไปจ่ายเงิน แล้วออกคิว
	echo "<body>";  
	echo "<font face='TH SarabunPSK' size='3'><center>HN:$hn VN:$tvn<br>$ptname<br>ผู้ป่วยต้องชำระเงิน<BR>ให้รอคิวที่ห้องจ่ายเงิน<br>หลังจากชำระเงินเสร็จสิ้น<br>ดูคิวรับยาที่ใบเสร็จรับเงิน</center>"; 
	...
	$result = mysql_query($query) or die("Query failed,update thaywin");
		...
		$idguard=substr($idguard,0,4);
			...
			//if($item<='4'){$pharinx="pharin_m";}else{$pharinx="pharin_l";};
				...
				echo "<body>";
				echo "<font   style='line-height:18px;'  face='TH SarabunPSK' size='3'><center>ใบรอรับยา รพ.ค่ายสุรศักดิ์มนตรี<br></FONT>";
				 //  echo "<font face='TH SarabunPSK' size='3'>โรงพยาบาลค่ายสุรศักดิ์มนตรี ลำปาง<br>";
	   	   		echo " <font style='line-height:20px;'  face='TH SarabunPSK' size='3'>เวลายื่นใบยา&nbsp;$today&nbsp;$todaytime<br>"; 
	       		echo "<font style='line-height:20px;'  face='TH SarabunPSK' size='4'><b>ชื่อ &nbsp;$ptname <br> <font face='TH SarabunPSK' size='2'>&nbsp;HN:$hn&nbsp;VN:$tvn</b><br>";
	   	   		echo " <font style='line-height:35px;'  face='TH SarabunPSK' size='7'>$kew<BR>";
		   		echo " <font style='line-height:20px;'  face='TH SarabunPSK' size='3'>**กรุณารอเรียกชื่อและคิว**</center>";
		   		echo " <font style='line-height:20px;'  face='TH SarabunPSK' size='3'><center>**ในกรณีที่เรียกผ่านแล้ว</center>";
		    	echo " <font style='line-height:20px;'  face='TH SarabunPSK' size='3'><center>ให้ติดต่อช่องหมายเลข 6**</center>";
	   		}else{  //else ถ้าคิวไม่ว่าง
				echo "<body>";
				echo "<font style='line-height:18px;'  face='TH SarabunPSK' size='5'><center>ได้ให้คิวไปเรียบร้อยแล้ว<br>"; 
				echo "<font style='line-height:25px;'  face='TH SarabunPSK' size='7'><b>ชื่อ &nbsp;$ptname <br> <font face='TH SarabunPSK' size='2'>&nbsp;HN:$hn&nbsp;VN:$tvn</b><br>";
        		echo "<font style='line-height:25px;'  face='TH SarabunPSK' size='10'>คิวที่ $kewphar</center>";						
			}  //close เช็คคิว
	...
	if($pay =='y' || ($pay =='x' and $m_pay >0)){  //เช็คว่าต้องชำระเงินหรือไม่ ถ้าชำระเงินให้ออกคิวที่จัดเก็บรายได้		
		...
		if($ptright111=="R02" || $ptright111=="R03"){  //ถ้าเป็นสิทธิเบิกจ่ายตรง
			echo "<body>";  
			echo "<font face='TH SarabunPSK' size='4'><center>HN:$hn VN:$tvn<br>$ptname<br>ผู้ป่วยต้องใช้บัตรประชาชน<br>เพื่อยืนยันสิทธิการรักษา<BR><u>กรุณารอคิวรับยาที่ห้องจ่ายเงิน</u><br>หลังทำธุรกรรมเสร็จสิ้น</center>"; 
			$query ="update dphardep SET pharin ='".date("H:i:s")."' WHERE hn = '".$cTdatehn1."' AND  date LIKE '$today1%'  ";
			$result = mysql_query($query) or die("Query failed,update thaywin");		
		}else{
			if(empty($kewphar)){
...
//    echo "<BR>  <a target=_self  href='../nindex.htm'>&lt;&lt;ไปเมนู............</a><br> ";
     echo "<BR><font size='5' color='red'>ลงเวลารับใบสั่งยาผู้ป่วยด้วย  HN ผู้ป่วย</font>&nbsp;&nbsp; <a target=bank  href='phakewmanual.php'>ให้คิว Manual</a>&nbsp;&nbsp; <a target=bank  href='phas.php'>เปิดเสียงพูด</a>&nbsp;&nbsp; <a target=bank  href='kewpharedit.php'>RESET Q</a>&nbsp;&nbsp;  <a target=_self  href='../nindex.htm'>&lt;&lt;ไปเมนู............</a><br> ";
...
<FORM METHOD=POST ACTION="<?php echo $_SERVER['PHP_SELF'];?>">
	<TABLE>
		<TR>
			<TD>HN&nbsp;:&nbsp;<INPUT ID="cTdatehn" TYPE="text" NAME="cTdatehn" class="txt"></TD>
			<TD></TD>
		</TR>
	</TABLE>
</FORM>
...
$kewl=$prefixl.$runnol;
       ...
       echo " <center style='font-size:32px;'><strong>คิวต่อไป &nbsp;&nbsp;บุคคลทั่วไป : $kewl <span style='margin-left:50px;'>ทหาร/ครอบครัว : $kewm</span></strong></center> ";
	...
	if(Mysql_num_rows($result) > 0){
?>
<table  align="center" style="font-family: TH SarabunPSK; font-size: 28px;">
 <tr>
  </tr>
 <tr>
...
$starttime = $pharin;
	...
	if($lasttime!=""and $starttime!=""){
        ...
        print (
			...
			"  <td BGCOLOR=ffffff><font face='TH SarabunPSK' size ='5'><b>$kewphar</b></td>\n".
			"  <td BGCOLOR=fffffff><font face='TH SarabunPSK' ></td>\n".
					"  <td BGCOLOR=fffffff><font face='TH SarabunPSK' >$tvn</td>\n".
						"  <td BGCOLOR=fffffff><font face='TH SarabunPSK'  color='red' ><b>$hn</b></td>\n".
							"  <td BGCOLOR=fffffff><font face='TH SarabunPSK' size ='4'  >$kew</td>\n".
					"  <td BGCOLOR=fffffff><font face='TH SarabunPSK' size ='5' ><b>$ptname</b></td>\n".
					
						"  <td BGCOLOR=fffffff><font face='TH SarabunPSK' size ='4' >$ptright</td>\n".
		"  <td BGCOLOR=fffffff><font face='TH SarabunPSK' size='4'>$idguard</td>\n".
					"  <td BGCOLOR=fffffff><font face='TH SarabunPSK' size='4'><b>$time3</b></td>\n".
					" </tr>\n");
       }
