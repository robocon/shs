...
$status="Y";
    ...
    $result = mysql_query($query) or die("Query failed,warphar0");
	 ...
	 $query="CREATE TEMPORARY TABLE opday3 SELECT clinic,vn,thdatehn,an FROM opday WHERE  thidate  LIKE '$yrmonth%'  ";
    $result = mysql_query($query) or die("Query failed,warphar1");
//   echo mysql_errno() . ": " . mysql_error(). "\n";
 //   echo "<br>";
  print "1. รายงามาตรฐานแฟ้มข้อมูลแฟ้มที่09 DRUG ประจำเดือน $yrmonth <a target=_self  href='../nindex.htm'><<ไปเมนู</a><br> ";
    print "<table>";
    print " <tr>";
...
$clinicall="$c1$clinic1$num00";
        print (
   "11512|$hn|$date1$vn|$date1|$drugcode|$amount|$salepri|$unitpri|$tradname|$code24|$subunit|$packing|$dtimerx|$clinicall|$idcard<BR>");
