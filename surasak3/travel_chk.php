...
<div align="center">
<p style="font-size:36px;"><b>ตรวจสอบสิทธิการรักษาพยาบาลเจ้าหน้าที่การท่องเที่ยวแห่งประเทศไทย (TRAVELEMPLOYEE20220301)</b></p>
<form action="travel_chk.php" method="post">
...
if ($action==='search'){
    ...
    $sql = "SELECT * FROM traveldata WHERE emp_idcard = '$idcard' ";
	//echo $sql;
    $result = mysql_query($sql) or die("ไม่สามารถค้นหาข้อมูลได้");
    $num = mysql_num_rows($result);
	$rows = mysql_fetch_array($result);
	$ptname = $rows["emp_name"];
	$status = $rows["emp_relation"];
	$subposition = $rows["emp_subposition"];
	$position = $rows["emp_position"];
	$idcard = $rows["emp_idcard"];
        if(empty($num)){
          echo "<h3 class='w3-pink' align='center' style='color:red; height: 50px;'>ไม่พบสิทธิ์! ในรายชื่อเจ้าหน้าที่การท่องเที่ยวแห่งประเทศไทย</h3>";
        }else{
	  echo "<h3 class='w3-green' align='center' style='color:green; height: 50px;'>พบสิทธิ์! ในรายชื่อเจ้าหน้าที่การท่องเที่ยวแห่งประเทศไทย</h3>";
	  ...
	  echo "<h4 align='center' style='color:black'>ชื่อ - นามสกุล : $ptname</h4>";
	   ...
	   echo "<h4 align='center' style='color:black'>ตำแหน่งสายงาน : $subposition</h4>";
	   echo "<h4 align='center' style='color:black'>ตำแหน่งบริหาร : $position</h4>";
	}//end if result
