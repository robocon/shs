...
$sql = "Select  a.depart, sum(a.paidcscd) From opacc as a where  ( a.date between '".($start_year)."-".$start_month."-".$start_day." 00:00:00' AND '".($end_year)."-".$end_month."-".$end_day." 23:59:59' )  AND a.credit ='กสทช' group by  a.depart   ORDER by a.txdate";  //ปรับวันที่ 11/09/64
...
while(list( $depart, $paidcscd) = Mysql_fetch_row($result)){
...
switch($depart){
	...
	case "SURG" : $list["SURG"] = $list["SURG"] + $paidcscd; break;
...
echo "</table>";
			...
			*/
echo "<font face='Angsana New' size ='5'><br><br><center> <b>สรุปลูกหนี้โครงการจ่ายตรง กสทช. ประจำวันที่ ".$start_day."-".$start_month."-".($start_year)." 00:00:00 ถึง  ".$end_day."-".$end_month."-".($end_year)." 23:59:59 <br></b> ";
...
$sql = "Select date_format( a.date, '%d-%m-%Y' ) AS date2, a.depart, sum(a.paidcscd) From opacc as a where  ( a.date between '".$start_year."-".$start_month."-".$start_day." 00:00:00' AND '".$end_year."-".$end_month."-".$end_day." 23:59:59' )  AND a.credit ='กสทช' group by date2, a.depart   ORDER by date";
...
while(list($date, $depart, $paidcscd) = Mysql_fetch_row($result)){
//echo $depart,"<BR>";
	switch($depart){
