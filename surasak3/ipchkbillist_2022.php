...
function checkptring(opt){
		...
		var pt2 = '<?php echo substr($sPtright,3);?>';
			...
			if( (pt == "R01" || pt == "R02" || pt == "R04" || pt == "R05" || pt == "R06" || pt == "R16" || pt == "R20" || pt == "R021" || pt == "R15" ) && opt (!= "เงินสด" || != "เงินโอน")){
...
print "  <th bgcolor=73C6B6>เลขที่</th>";
print "  <th bgcolor=73C6B6>ประเภท</th>";
print "  <th bgcolor=73C6B6>จนท.</th>";
print "  <th bgcolor=73C6B6>เวลาเข้า</th>";
print "  <th bgcolor=73C6B6>เวลาออก</th>";
print " </tr>";

////
for($k=1;$k<=$_POST['cout'];$k++){
	if(isset($_POST['ch'.$k])){
		array_push($_SESSION['idacc'],$_POST['ch'.$k]);
	}
}
	$name_select = implode(' or row_id=',$_SESSION['idacc']);
	include("connect.inc");
    $query = "SELECT * FROM ipacc WHERE row_id = $name_select  ";
...
for($k=0;$k<count($_SESSION['idacc']);$k++){
	$p++;
	$query = "SELECT date,depart,detail,amount,price,paid,part,idname,billno,startdatetime,enddatetime FROM ipacc WHERE row_id = '".$_SESSION['idacc'][$k]."'  ORDER BY date ASC ";

    $result = mysql_query($query) or die("Query failed ipacc");
    list ($date,$depart,$detail,$amount,$price,$paid,$part,$idname,$billno,$startdatetime, $enddatetime,$row_id) = mysql_fetch_row ($result);
	...
	print("<tr>\n".
                "<td bgcolor=FCF3CF><font face='Angsana New'>$p</td>\n".
			    ...
			    "<td bgcolor=FCF3CF><font face='Angsana New'>$billno</td>\n".  
...
//print "จนท. $sOfficer วันที่ $Thaidate<br>";
    ...
    print "<form name='f1'  method='POST'  action='ipbill5_2022.php' Onsubmit='return checkformf1()' >";
	...
	print "ระหว่างวันที่ &nbsp;<input type='text' name='dd' size='2' value='00'>-<input type='text' name='mm' size='2' value='00'>-<input type='text' name='yy' size='2' value='2555'>&nbsp;&nbsp;บาท&nbsp;&nbsp;ถึงวันที่ &nbsp;<input type='text' name='dd2' size='2' value='00'>-<input type='text' name='mm2' size='2' value='00'>-<input type='text' name='yy2' size='2' value='2555'><br>";
...
list($pricedc,$status) = Mysql_fetch_row($result2);
 include("unconnect.inc");
	print("<tr><td colspan='13' align='center'><input type='hidden' name='cout' value='$num'><input name='okbtn' type='submit' value='ตกลง'></td></tr>");
print "</table>";
?>
</form>
<?
