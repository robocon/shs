...
//runno to make unique for dgexplot
    ...
    $query = "SELECT title,prefix,runno FROM runno WHERE title = 'combill1'";
...
$query = "Select docno From combill_pt where dgexplot = '$drugcode$expdate$lotno$nRunno' ";
$result = Mysql_Query($query);
...
//insert data into combill
   $query = "INSERT INTO combill_pt(`date`,docno,billno,billdate,comcode,comname,drugcode,
...
//
if($result){
$query ="update runno SET runno = $nRunno WHERE title='combill1'";
...
print "$cComcode, $cComname<br>";
        $query ="update druglst_pt SET  comcode  = '$cComcode',
			           comname = '$cComname',
					   tradname = '$tradname',
...
//        echo mysql_errno() . ": " . mysql_error(). "\n";
//        echo "<br>";
print "<br><br>บันทึกข้อมูลเรียบร้อย <A HREF=\"dgprocure_print_pt.php?id=".$idno."\" target=\"_blank\">พิมพ์ข้อมูลการซื้อ</A><br>";
print "<br><a href='newitem_pt.php'>บันทึกรายการต่อไป (ใบส่งของเดิม)</a><br>";
print "<br><a href='stkok_pt.php'>หมดรายการบันทึก</a><br>";
