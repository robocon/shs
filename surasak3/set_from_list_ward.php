...
//$bedcode=$_GET['bedcode'];

if($_GET['bedcode']=='42'){
	
	$where="and ward ='หอผู้ป่วยรวม' and status !='N' ";
}
if($_GET['bedcode']=='43'){
	
	$where="and ward ='หอผู้ป่วยสูติ' and status !='N' ";	
}
if($_GET['bedcode']=='44'){
	
	$where="and ward ='หอผู้ป่วยหนัก' and status !='N'";	
}
if($_GET['bedcode']=='45'){
	
	$where="and ward ='หอผู้ป่วยพิเศษ' and status !='N' ";
}
...
$sqlnow="SELECT * FROM `set_or` WHERE date_surg='$tomorow'  ".$where." ";
...
<table border="1" style="border-collapse:collapse; border-color:#000;font-family:'Angsana New'; font-size:16pt;" cellpadding="0" cellspacing="0" class="f1" align="center">
  ...
  <tr>
    ...
    <td align="center"><a href="JavaScript:if(confirm('ยืนยันการยกเลิก?')==true){ javascript:MM_openBrWindow('set_from_cancel.php?row_id=<?=$arr['row_id'];?>&status=N','','width=200,height=200');}">ยกเลิก</a></td>
  </tr>
...
while($arrG=mysql_fetch_array($queryGroup)){
 ...
 $dateshow=$printmonth." ".$sub1[0];
	...
	$sql1="SELECT * FROM `set_or` WHERE date_surg >='".$datenow."'  and  date_surg like '$arrG[subdate]%' ".$where." order by date_surg asc";
...
<table border="1" style="border-collapse:collapse; border-color:#000;font-family:'Angsana New'; font-size:16pt;" cellpadding="0" cellspacing="0" class="f1" align="center">
  ...
  <tr>
    ...
    <td align="center"><a href="JavaScript:if(confirm('ยืนยันการยกเลิก?')==true){ javascript:MM_openBrWindow('set_from_cancel.php?row_id=<?=$arr1['row_id'];?>&status=N','','width=200,height=200');}">ยกเลิก</a></td>
  </tr>
