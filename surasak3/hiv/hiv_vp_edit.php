...
<div class="font_title" align="left">แก้ไขข้อมูล ผู้ป่วย HIV สิทธิประกันสังคม <a href ="../../nindex.htm"  class="forntsarabun1"><----- ไปหน้าแรกสุด</a> || <a href="hiv_index.php?do=edit" class="forntsarabun1">&lt;-----เมนู</a></div>
...
if(!empty($_POST["p_hn"]) != ""){
	  ...
	  $sqlhiv="select * from hiv_vp where hn='$hn' ";
...
//ค้นหา hn จาก hiv_vp ****************************************************************************************
	$sql = "Select  * From hiv_vp where  hn = '".$_POST["p_hn"]."' limit 0,1";
...
////////////////////////////////////////
	...
	$datenow=date("Y-m-d");
	  ...
	  $arrvp=mysql_fetch_array($queryvp);
	  $vp=$arrvo['vp_id']+1;
	  $vp_no=$vo;*/
...
<FORM METHOD="post" ACTION="hiv_vp_edit.php?do=save" name="F1">
...
<TR>
	...
	<table border="0">
		...
		<tr>
		  ...
		  <td><span class="data_show">
		    <input name="vp_no" type="text" class="forntsarabun1" id="vp_no"  value="<?=$arr_view['vp_id'];?>" readonly="readonly"/>
		  </span></td>
  ...
  <tr>
    ...
    <?
//   $year=date("Y");
	$sql = "CREATE TEMPORARY TABLE lab01 SELECT  *  FROM  hiv_lab_vp  WHERE  ref_id='".$arr_view['row_id']."' ";
	$result = mysql_query($sql) or die (mysql_error());
...
//  6 lipid profile  //
$sqladd61 = "UPDATE hiv_lab_vp  SET ";
...
$sqladd62 = "UPDATE hiv_lab_vp  SET ";
...
$sqladd63 = "UPDATE hiv_lab_vp  SET ";
...
$sqladd64 = "UPDATE hiv_lab_vp  SET ";
...
$objadd9 = mysql_query($sqladd9);
	...
	print "<META HTTP-EQUIV='Refresh' CONTENT='2;URL=hiv_vp_edit.php'>";
}
else
{
