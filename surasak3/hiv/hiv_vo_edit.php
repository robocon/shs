...
<div class="font_title" align="left">แก้ไขข้อมูล ผู้ป่วย HIV สิทธิเบิกทั่วไป <a href ="../../nindex.htm"  class="forntsarabun1"><----- ไปหน้าแรกสุด</a> || <a href="hiv_index.php?do=edit" class="forntsarabun1">&lt;-----เมนู</a></div>
...
if(!empty($_POST["p_hn"]) != ""){
	  ...
	  $sqlhiv="select * from hiv_vo where hn='$hn' ";
...
//ค้นหา hn จาก hiv_vo ****************************************************************************************
	$sql = "Select  * From hiv_vo where  hn = '".$_POST["p_hn"]."' limit 0,1";
...
<FORM METHOD="post" ACTION="hiv_vo_edit.php?do=save" name="F1">
...
<TR>
	...
	<table border="0">
		...
		<tr>
		  ...
		  <td><span class="data_show">
		    <input name="vo_no" type="text" class="forntsarabun1" id="vo_no"  value="<?=$arr_view['vo_id'];?>" readonly="readonly"/>
		  </span></td>
  ...
  <tr>
    ...
    <?
//   $year=date("Y");
	$sql = "CREATE TEMPORARY TABLE lab01 SELECT  *  FROM  hiv_lab  WHERE  ref_id='".$arr_view['row_id']."' ";
	$result = mysql_query($sql) or die (mysql_error());
...
if($objQuery)
{
	...
	print "<META HTTP-EQUIV='Refresh' CONTENT='2;URL=hiv_vo_edit.php'>";
}
else
{
