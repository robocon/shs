...
$date_hn = $_POST["chkdate"].$_POST["hn"];
$date_vn = $_POST["chkdate"].$_POST["vn"];
...
$result3 = mysql_query($upopday) or die(mysql_error());
	...
	$sqlchk1="select row_id,doctor_date from register_chkup_soldier where yearcheck='$yearcheck' and idcard='".$_POST["idcard"]."'";
	//echo $sqlchk1;
	$querychk1=mysql_query($sqlchk1);
	$numchk1=mysql_num_rows($querychk1);
	if($numchk1 > 0){  //พบข้อมูล
	list($register_chkup_id,$doctor_date)=mysql_fetch_array($querychk1);
		if($doctor_date=="0000-00-00 00:00:00"){
			$datetime=date("Y-m-d H:i:s");
			$edit="UPDATE register_chkup_soldier SET doctor_date='$datetime', appoint='".$_POST['appoint']."' WHERE row_id='$register_chkup_id'";
...
if($result && isset($_POST["submit"])){
	...
	<tr>
      ...
      <? }?>
	   <? if(!empty($arrs["appoint"])){?>
	  <tr>
	    <td>นัดครั้งต่อไป: <?=$arrs["appoint"]?></td>
      </tr>	     
	   <? } ?>	  
	  <tr>
