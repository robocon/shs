...
<?
 $query="SELECT distinct(camp) as camp FROM dxofyear  WHERE yearchk='$nPrefix' group by hn order by camp ";
//echo $query;
  $result = mysql_query($query)or die("Query failed");
  $i=0;
  while($rows=mysql_fetch_array($result)){
  $i++;
  $sql1=mysql_query("select * from dxofyear where yearchk='$nPrefix' and camp='$rows[camp]' group by hn");
  $numcamp=mysql_num_rows($sql1);
  $totalcamp=$totalcamp+$numcamp;
  
  $sql2=mysql_query("select * from condxofyear_so where yearcheck='$showPrefix' and camp='$rows[camp]' group by hn");
