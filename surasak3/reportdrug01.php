...
$numcscd=0;
$cscd='จ่ายตรง';
    $query="CREATE TEMPORARY TABLE reportcscd01 SELECT date,hn,vn,billno,price,credit,depart,paidcscd,detail,row_id,txdate FROM opacc WHERE date LIKE '$yrmonth%' and credit = '$cscd' AND depart='PHAR' " ;
...
//or part='DPY'
	...
	if($ddl>0){
	$posdrhd = strpos($doctor,"HD");
	$posdr = strpos($doctor,"ว.");
	$posdrd = strpos($doctor,"(ท.");
	$posdrd1 = strpos($doctor,"00000");
		$posdrd2 = strpos($doctor,"พท.ป");
 ...
 if($posdr==true){
	...
	else	if($posdrd1==true){
			$dc="-";
			$dr="0001";
			$dr1="$dc$dr";
		}
	
		else	if($posdrd2==true){
			$dr = substr($doctor,($posdrd2+5),4);
			$dc="-";
			$dr1="$dc$dr";
		}
...
//$datem1=date_format(datem,'%d/ %m/ %Y');
	...
	$query6 = "SELECT b.drugcode, b.genname, a.slcode, a.amount, a.price, a.part , c.detail1, c.detail2, c.detail3 ,b.unit ,a.reason,b.tmt, a.drug_inject_slip FROM drugrx as a , druglst as b , drugslip as c WHERE a.hn = '$hn' AND a.drugcode = b.drugcode AND c.slcode = a.slcode AND a.idno = '$xrow' and (a.part='DDL' or a.part='DDY' or a.part='DDN' or a.part='DSY' or a.part='DSN' )";// or a.part='DPY'
	$row6 = mysql_query($query6);
	while(list($drugcode,$genname,$slcode,$amount,$price,$part,$detail1,$detail2,$detail3,$unit,$reason,$tmt,$drug_inject_slip) = mysql_fetch_array($row6)){
		...
		if($slcode=='' || $slcode=='ER' ){$slcode=$drug_inject_slip;};
				if($detail1==''){$detail1="ไม่ระบุ";};
        print (" <tr>\n".
        //   " <td BGCOLOR=66CDAA><font face='Angsana New'>$num1</td>\n".
