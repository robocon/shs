...
If (!empty($code)){
    include("connect.inc");
  $query="CREATE TEMPORARY TABLE patdata1 SELECT * FROM patdata WHERE date LIKE '$yrmonth%' and depart = 'denta' ";
    $result = mysql_query($query) or die("Query failed,opday");

    $query = "SELECT code,date,ptname,hn,detail,amount,price FROM patdata1 WHERE code = '$code'";
    $result = mysql_query($query)
        or die("query failed,opcard");
 $n=0;
    while (list ($code,$date,$ptname,$hn,$detail,$amount,$price) = mysql_fetch_row ($result)) {
  $n++;
        print (" <tr>\n".
