...
<title>พิมพ์ใบตรวจสุขภาพลูกจ้างชั่วคราว โรงพยาบาลค่ายสุรศักดิ์มนตรี</title>
<style type="text/css">
<!--
.tet {
...
<center>
<span class="tet1">พิมพ์ใบตรวจสุขภาพลูกจ้างชั่วคราว โรงพยาบาลค่ายสุรศักดิ์มนตรี</span><br />
  <br />
...
<!--    <script>
	...
	include("connect.inc");	
	$sql="SELECT  * FROM opcardchk  WHERE part='ลูกจ้าง61' and active='y' order by course desc, exam_no asc";
	//echo $sql."<br>";
...
$strSQL11 = "SELECT date_format(orderdate,'%d-%m-%Y') as orderdate2 FROM resulthead  WHERE hn='".$result2['HN']."' and (clinicalinfo ='ตรวจสุขภาพประจำปี61') order by orderdate desc";
//echo "===>".$strSQL1;
  ...
  <tr>
    <td colspan="2"><table width="100%">
      <tr>
        ...
        <td width="77%" align="center" valign="top" class="texthead"><strong>แบบรายงานการตรวจสุขภาพลูกจ้างชั่วคราว โรงพยาบาลค่ายสุรศักดิ์มนตรี</strong></td>
              ...
              <td><table width="100%"   class="text1" >
                <tr>
                  <td  valign="top" class="text2"><strong class="text" style="font-size:22px"><u>ข้อมูลผู้ตรวจสุขภาพ</u> </strong><strong>HN : <?=$result['hn']?> 
                    &nbsp;&nbsp;</strong><strong>ชื่อ :</strong> <span style="font-size:22px"><strong>
                    <?=$result['ptname']?>
                    </strong>&nbsp;&nbsp;<strong>อายุ :</strong> <span style="font-size:22px"><strong>
                    <?=$result2['agey']?> ปี
                    </strong></span>&nbsp;&nbsp;<strong>สังกัด :</strong> <span style="font-size:22px"><strong>
                    <?=$result2['course']?>
                    </strong></span></td>
                </tr>
              </table></td>
...
mmHg. </u></strong><span class="text3"><strong>P: </strong> <u>
 ...
 <? $sql="SELECT * FROM resulthead WHERE profilecode='CBC' and hn='".$result2['HN']."' and (clinicalinfo ='ตรวจสุขภาพประจำปี61')";
		...
		?>
          ...
          <tr>
            ...
            <table width="100%" border="0" cellpadding="1" cellspacing="0">
              <tr height="25">
                <td width="4%">&nbsp;</td>
                <td width="64%"><strong>จำนวนเม็ดเลือด (WBC)</strong></td>
                <td width="32%"><? 
    ...
    <td width="50%"  valign="top"><table width="100%" height="111" border="1" cellpadding="3" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse">
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="text3">
          ...
          <? $sql="SELECT * FROM resulthead WHERE profilecode='UA' and hn='".$result2['HN']."' and (clinicalinfo ='ตรวจสุขภาพประจำปี61')";
	$query = mysql_query($sql);
	$arrresult = mysql_fetch_array($query);
/////
		...
		?>
          ...
          <tr>
            <td colspan="3"><table width="100%" border="0" cellpadding="1" cellspacing="0">
              ...
              <tr height="25">
                <td width="4%">&nbsp;</td>
                <td width="50%"><strong>ความถ่วงจำเพาะ (SPGR)</strong></td>
                <td width="46%"><? 
              ...
              <tr height="25">
                <td>&nbsp;</td>
                <td><strong>ความเป็นกรด (PHU)</strong></td>
                <td><? 
      ...
      <tr>
        <td height="52" valign="top"><table width="98%" border="0" class="text3">
            <tr height="25">
              ...
              <td width="8%" valign="top" bgcolor="#CCCCCC"><strong>ผลการตรวจ</strong></td>
              <td width="9%" valign="top" bgcolor="#CCCCCC"><strong>ค่าปกติ</strong></td>
              <td width="51%" valign="top" bgcolor="#CCCCCC" style="font-size:16px;"><strong>สรุปผลการตรวจ</strong></td>
            </tr>
<?
if($result2["pid"]=="4" || $result2["pid"]=="5"){ 
	$sql1="SELECT * FROM resulthead WHERE (profilecode='HBSAG' or profilecode='ANTIHB') and hn='".$result2['HN']."' and (clinicalinfo ='ตรวจสุขภาพประจำปี61') GROUP BY `profilecode`
ORDER BY `autonumber` ASC"; 
}else{
	$sql1="SELECT * FROM resulthead WHERE (profilecode='GLU' or profilecode='CREA' or profilecode='BUN' or profilecode='URIC' or profilecode='CHOL' or profilecode='TRIG' or  profilecode='AST' or profilecode='ALT' or profilecode='LIPID' or profilecode='ALP' or profilecode='HBSAG' or profilecode='ANTIHB' or profilecode='HDL' or profilecode='LDL') and hn='".$result2['HN']."' and (clinicalinfo ='ตรวจสุขภาพประจำปี61') GROUP BY `profilecode`
ORDER BY `autonumber` ASC";
}
//echo $sql1;
...
if($objResult["labcode"]=='GLU'){
	if($objResult["result"]>=74 && $objResult["result"]<=106){
		...
		$app="ระดับน้ำตาลในเลือดต่ำกว่าค่าปกติ";	
	}
}
...
if($objResult["labcode"]=='10001'){  //ต่ำดี สูงไม่ดี
...
if($objResult["labcode"]=='LDL'){  //ต่ำดี สูงไม่ดี
	if($objResult["result"]>=0 && $objResult["result"]<=100){
		$app="ระดับไขมันในเลือดมีค่าอยู่ในเกณฑ์ปกติ";	
	}else	if($objResult["result"]>100){
		$app="ผิดปกติ ควรปรับพฤติกรรมการรับประทานอาหาร และออกกำลังกายอย่างสม่ำเสมอ";	
...
if($objResult["labcode"]=='LDLC'){  //ต่ำดี สูงไม่ดี
	if($objResult["result"]>=0 && $objResult["result"]<=100){
		$app="ระดับไขมันในเลือดมีค่าอยู่ในเกณฑ์ปกติ";	
	}else	if($objResult["result"]>100){
		$app="ผิดปกติ ควรปรับพฤติกรรมการรับประทานอาหาร และออกกำลังกายอย่างสม่ำเสมอ";	
