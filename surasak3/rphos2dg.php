...
<table>
 <tr>
  ...
  <th bgcolor=6495ED width="150"><font face='Angsana New'>เลขที่ใบส่งของ</th>
  ...
  <th bgcolor=6495ED><font face='Angsana New'>ราคาไม่รวม VAT</th>
...
If (!empty($drugcode)){
    ...
    $query = "SELECT stkno,docno,getdate,billdate,billno,comname,drugcode,tradname,lotno,expdate,packamt,packing,packpri,price,stkbak,packamt, packpri_vat FROM combill WHERE drugcode = '$drugcode' ";
	$result = mysql_query($query) or die("Query failed");
    $num=0;
    while (list ($stkno,$docno,$getdate,$billdate,$billno,$comname,$drugcode,$tradname,$lotno,$expdate,$packamt,$packing,$packpri,$price,$stkbak,$packamt,$packpri_vat) = mysql_fetch_row ($result)) {
	$num++;
/*
          if ($packamt > 0){
...
*/
        print (" <tr>\n".
           ...
           "  <td BGCOLOR=66CDAA width='150'><font face='Angsana New'>$billno</td>\n".
