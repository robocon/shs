...
//----------------------------จบเช็ค druginteraction
  ...
  $query = "SELECT title,prefix,runno FROM runno WHERE title = 'phardep1'";
    ...
    $query ="UPDATE runno SET runno = ".$_SESSION["sChktranx"]." WHERE title='phardep1'";
    $result = mysql_query($query)
        or die( mysql_error() );
//end  runno  for chktranx

    $query = "SELECT * FROM dphardep_pt WHERE row_id = '".$_GET["nRow_id"]."'  AND date = '".$_GET["sDate"]."'"; 
...
$inject = false;
	...
	$query = "SELECT a.tradname,a.drugcode, a.amount, a.price, a.slcode,a.row_id, a.part,a.office, b.detail1, b.detail2, b.detail3, b.detail4, a.drug_inject_amount,a.drug_inject_unit, a.drug_inject_amount2,a.drug_inject_unit2,a.drug_inject_time,a.drug_inject_slip,a.drug_inject_etc,a.injno FROM ddrugrx_pt as a, drugslip as b WHERE a.slcode = b.slcode AND a.idno = '".$_GET["nRow_id"]."'   AND a.date = '".$_GET["sDate"]."' ";
    $result = mysql_query($query) or die( mysql_error() );
$n='0';
    $d=substr($dDate,8,2);
    $m=substr($dDate,5,2);
	...
	$res= mysql_query($sqlopday) or die( mysql_error() );
...
echo "<table><tr>";
    ...
    <td>
    <div id='cut1' style="display:block">
    </td>
	
	<td><FONT COLOR="#FF0000"><B>เคยตัดสต๊อกแล้ว</B></FONT>&nbsp;&nbsp;&nbsp;</td>
	<?php }?>
    </td>
    <td><a target="_blank" href="drxprint_pt.php"><font face='Angsana New'>พิมพ์ใบสั่งยา</a></td>
