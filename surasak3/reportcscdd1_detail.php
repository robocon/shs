...
$query="CREATE TEMPORARY TABLE drugrx02 SELECT * FROM drugrx WHERE date like '$date%' ";
    $result = mysql_query($query) or die("Query failed,warphar2");
	
	$query="CREATE TEMPORARY TABLE depart01 SELECT * FROM depart WHERE date like '$date%' ";
    $result = mysql_query($query) or die("Query failed,warphar3");

	$query="CREATE TEMPORARY TABLE patdata01 SELECT * FROM patdata WHERE date like '$date%' ";
    $result = mysql_query($query) or die("Query failed,warphar4");
	
$sql23 = "Select a.date,a.hn, CONCAT(b.yot,' ',b.name,' ',b.surname) as full_name From opacc as a, opcard as b where a.hn=b.hn AND a.date like '".$date."%'  AND a.credit ='จ่ายตรง' AND a.paidcscd > 0 group by a.hn ORDER by a.date";
echo $sql23;
$result23 = mysql_query($sql23) or die(mysql_error());
while($listss=mysql_fetch_row($result23)){
//echo $listss[0]."<br>";
//echo $listss[1]."<br>";
//echo $listss[2]."<br>";
