...
for($r=0;$r<count($_SESSION['idnumber']);$r++){
	...
	$query ="UPDATE dphardep_pt SET paid = price,cashok = '$credit' WHERE row_id= '".$_SESSION['idnumber'][$r]."' and hn='".$hn_now."' and tvn='".$_SESSION["sVn"]."' ";
...
$sDetail='ค่ายา';
for($r=0;$r<count($_SESSION['idnumber']);$r++){
		$query = "SELECT * FROM dphardep_pt WHERE row_id = '".$_SESSION['idnumber'][$r]."' and hn='".$hn_now."' and tvn='".$_SESSION["sVn"]."'";
		$result = mysql_query($query) or die(mysql_error());
		$num1 = mysql_num_rows($result);
		for ($i = mysql_num_rows($result) - 1; $i >= 0; $i--) {
			if (!mysql_data_seek($result, $i)) {
...
//   $sql = "Select kewphar,hn From dphardep_pt WHERE hn ='$sHn'  AND  date LIKE '$today1%'  and pharin != '' and (kewphar is null or kewphar = '') ";
...
if($_POST["credit"] == "ค้างจ่าย"){
	for($r=0;$r<count($_SESSION['idnumber']);$r++){
		...
		if($numtest>0){
			...
			$sql = "INSERT INTO accrued_pt (date,txdate,hn,depart,detail,price,ptright,vn,status_pay) VALUES('$Thidate','".$_SESSION['dDate'][$r]."','$sHn','$sDepart','$sDetail', '$sNetprice','$sPtright','".$_SESSION["sVn"]."','n');";
			...
			if($numtest2>0){
				...
				$sql = "INSERT INTO accrued_pt (date,txdate,hn,depart,detail,price,ptright,vn,status_pay) VALUES('$Thidate','".$_SESSION['dDate'][$r]."','$asHn','$sDepart','$sDetail', '$asNetprice','$asPtright','".$_SESSION["sVn"]."','n');";
				$result = mysql_query($sql) or die("Query failed 662");
			}
		}
	}
