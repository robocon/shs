...
<form method='POST' action='report_sur03.php'>
...
echo "<FONT style=\"font-size: 14px; font-family: 'MS Sans Serif';\"><CENTER>สรุปยอดผู้ป่วยแบ่งตามประเภทบาดแผลผ่าตัด(ผ่าตัดใหญ่) ปีงบประมาณ ".$_POST["year"]."</CENTER></FONT>";
...
echo "<TR>";
	echo "<TD>".$month[$j]." ".$year."</TD>";
	 foreach( $cfg_surgery as  $key => $value){
		 ...
		 $sql = "Select type_scar,count(hn) From memo_sur where thaidate like '".$year."-".$j."%' AND surgery = '".$key."' AND type_case = 'TC1' Group by type_scar  ";
		 $result = mysql_query($sql);
		 while(list($type_scar,$count) = mysql_fetch_row($result)){
			$list[$type_scar] = $count;
			$sum1[$type_scar] += $count;
...
echo "<BR><BR>";
echo "<FONT style=\"font-size: 14px; font-family: 'MS Sans Serif';\"><CENTER>สรุปยอดผู้ป่วยแบ่งตามประเภทบาดแผลผ่าตัด(ผ่าตัดใหญ่) ปีพ.ศ. ".$_POST["year"]."</CENTER></FONT>";
echo "<TABLE border=\"1\" align=\"center\" border='1' bordercolor=\"#000000\" style=\"font-size: 14px; font-family: 'MS Sans Serif'; BORDER-COLLAPSE: collapse;\" cellpadding=\"2\">";
echo "<TR>";
	echo "<TD rowspan=\"2\" width=\"100\">เดือน ปี</TD>";
	 foreach( $cfg_surgery as  $key => $value){
