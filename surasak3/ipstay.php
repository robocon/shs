...
If (!empty($month)){
   $yrmn=$year."-".$month;
   print"วันนอนโรงพยาบาล รายโรค/หัตถการ (หรือกลุ่มโรค/กลุ่มหัตถการ) ซึ่งสามารถคัดเลือกผู้ป่วยที่มีวันนอนโรงพยาบาล นานกว่าที่ควรมาวิเคราะห์เพิ่มเติมได้
,  เดือน $month ปี$year";
    include("connect.inc");
     $query = "SELECT ptname,hn,an,diag,icd10,complica,dctype,date,dcdate,days,price,bedcode,doctor FROM ipcard  WHERE date LIKE '$yrmn%' ORDER BY days DESC";  
