...
<title>พิมพ์ใบตรวจสุขภาพบริษัท ผลิตภัณฑ์กระเบื้อง จำกัด</title>
<style type="text/css">
<!--
.tet {
...
<center>
<span class="tet1">พิมพ์ใบตรวจสุขภาพบริษัท ผลิตภัณฑ์กระเบื้อง จำกัด</span> <br />
  <br />
...
if(isset($_POST['hn'])){
	...
	$select = "select * from opd  where hn='".$result2['HN']."' and thidate like '2558-02-02%'";
  ...
  <tr>
    <td colspan="2"><table width="100%">
      <tr>
        ...
        <td width="77%" align="center" valign="top" class="texthead"><strong>แบบรายงานการตรวจสุขภาพบริษัท ผลิตภัณฑ์กระเบื้อง จำกัด</strong></td>
      ...
      <tr>
        <td align="center" valign="top" class="text3"><span class="text"><span class="text1"><span class="text2">ตรวจเมื่อวันที่ 13 มกราคม 2558</span></span></span></td>
...
mmHg. </u></strong><span class="text3"><strong>P: </strong> <u>
  ...
  <tr>
    <td width="50%"  valign="top"><table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000" class="text3" style="border-collapse:collapse;">
      <tr>
        <td height="27" align="center"><strong class="text" style="font-size:22px"><u>CBC : การตรวจเม็ดเลือด</u></strong></td>
      </tr>
      <tr>
...
/////
 ...
 ?>         
          <tr>
            <td height="19" colspan="3"><strong>ผลตรวจ :</strong> <? while($objResult = mysql_fetch_array($objQuery)){
																			 if($objResult["labcode"]=="WBC"){
																				...
																				if($chkwbc=="ปกติ" && $chkhct=="ปกติ" && $chkpltc=="ปกติ"){																																						
																						echo "ปกติ";
																				}else{
																						if($chkwbc=="ปกติ"){
																							echo "";
    ...
    <td width="50%"  valign="top"><table width="100%" border="1" style="border-collapse:collapse" bordercolor="#000000" cellpadding="0" cellspacing="0">
      <tr>
        <td height="27" align="center"><strong class="text" style="font-size:22px"><u>UA : การตรวจการทำงานของปัสสาวะ</u></strong></td>
      </tr>
      <tr>
...
/////
		...
		?>
          ...
          <?  } ?>        
          <tr>             
            <td height="27" colspan="3"><strong>ผลตรวจ :</strong> <? if($result['hn']=="57-10314"){ echo "ผิดปกติ ควรพบแพทย์เพื่อตรวจซ้ำหรือรับการรักษา";}else{ echo "ปกติ";}?></td>
            </tr>
        </table></td>
      </tr>
    </table></td>
...
?>       
      ...
      <table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#000000" class="text3" style="border-collapse:collapse;">
        <tr>
          <td colspan="2" align="center"><strong class="text" style="font-size:22px"><u>STOOL : การตรวจอุจจาระ</u></strong></td>
        </tr>
        <tr>
...
/////
		...
		$strSQL = "SELECT * FROM resultdetail  WHERE autonumber='".$arrresult['autonumber']."' and (labcode='COLORS' || labcode='CHARAC' || labcode='WBCS')";
		//echo "---->".$strSQL;
		$objQuery = mysql_query($strSQL);
		while($objResult = mysql_fetch_array($objQuery))
		{
			if($objResult["labcode"]=="COLORS"){
				$labmean="(สีของอุจจาระ)";
			}else if($objResult["labcode"]=="CHARAC"){
				$labmean="(ลักษณะ)";
			}else if($objResult["labcode"]=="WBCS"){
				$labmean="(เม็ดเลือดขาว)";
			}
		?>
              <tr>
 ...
 ?>
              <tr>
                <td height="18"><strong>ผลตรวจ :</strong><? if($result['hn']=="57-7693"){ echo "ผู้ตรวจสุขภาพไม่ได้ส่งอุจจาระ";}else{ echo "ปกติ";}?></td>
                <td height="18">&nbsp;</td>
              </tr>
          </table></td>
          <td width="50%"><table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td width="59%" align="center" bgcolor="#CCCCCC">labcode </td>
              <td width="41%" align="center" bgcolor="#CCCCCC">result</td>
            </tr>
            <? $sql="SELECT * FROM result1 WHERE profilecode='STOOL' ";
	$query = mysql_query($sql);
	$arrresult = mysql_fetch_array($query);
/////
		...
		$strSQL = "SELECT * FROM resultdetail  WHERE autonumber='".$arrresult['autonumber']."' and (labcode='RBCS' || labcode='MUCOUS' || labcode='PARASI')";
		//echo "---->".$strSQL;
		$objQuery = mysql_query($strSQL);
		while($objResult = mysql_fetch_array($objQuery))
		{
			if($objResult["labcode"]=="RBCS"){
				$labmean="(เม็ดเลือดแดง)";
			}else if($objResult["labcode"]=="MUCOUS"){
				$labmean="";
			}else if($objResult["labcode"]=="PARASI"){
				$labmean="(พยาธิ)";
			}
		?>
            <tr>
 ...
 ?>
        ...
        <tr>
          <td><strong class="text" style="font-size:18px"><u>ตรวจคลื่นหัวใจไฟฟ้า</u>&nbsp;&nbsp;&nbsp;&nbsp;</u>EKG : <strong><? if($result2['HN']=="57-10314"){ echo "ปกติ";}else{ echo "EKG sinus bradycardia ชีพจรเต้นช้า";} ?> </strong></u></strong></td>
        </tr>
        <tr>
          <td><strong class="text" style="font-size:18px"><u>ตรวจความหนาแน่นของกระดูก</u>&nbsp;&nbsp;&nbsp;&nbsp;</u>BMD : <strong><? echo "ปกติ"; ?> </strong></u></strong></td>
        </tr>
        <tr>
          <td><strong class="text" style="font-size:18px"><u>ตรวจความผิดปกติของตา</u>&nbsp;&nbsp;&nbsp;&nbsp;</u>จักษุ : <strong><? if($result2['HN']=="57-10314"){ echo "แผลเผ็นที่ตาดำ";}else{ echo "สงสัย เส้นประสาทตาเสื่อม";} ?> </strong></u></strong></td>
        </tr>        
        <tr>
        <?
...
<table width="100%" border="0" class="text4">
  <tr>
    ...
    ) </strong><strong>Doctor :พ.ต.เลอปรัชญ์ มังกรกนกพงศ์ (19-01-2015)</strong></td>
