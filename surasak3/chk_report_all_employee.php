...
<form action="chk_report_all_employee.php" method="POST">
	<table>
		<tr>
			<td></td>
			<td><h3>ค้นหาข้อมูล ผลตรวจสุขภาพลูกจ้างชั่วคราว</h3></td>
		</tr>
		<tr>
			<td>เลือกปี : </td>
			<td>
				<?php 
				$yearRange = range('2562', date('Y')+543);
				?>
		...
		<tr>
			...
			<td>
				...
				<input type="hidden" name="page" value="showForm">
			</td>
		</tr>
	</table>
</form>
...
if($page=='showForm'){
	$year = sprintf("%s", $_POST['yearSelect']);
	...
	LEFT JOIN ( 
		SELECT y.* 
		FROM ( 
			SELECT MAX(`id`) AS `lastId` FROM `chk_doctor` WHERE `yearchk` = '$shortYear' GROUP BY `hn` 
		) AS x 
...
<div align="center"><strong>ปี <?=$year;?> จำนวน <?=$num;?> ราย</strong></div>
