...
if (isset($_GET['nat'])) {
	...
	if (isset($_GET["act"]) && $_GET["act"] = "show1") {
		$query1 = "SELECT hn FROM appoint WHERE hn='" . $_GET['hn'] . "' AND appdate = '" . urldecode($_GET['nat']) . "' " . $tell . " AND apptime <> 'ยกเลิกการนัด' order by hn ";
	} else {
		$query1 = "SELECT hn FROM appoint WHERE appdate = '" . urldecode($_GET['nat']) . "' " . $tell . " AND apptime <> 'ยกเลิกการนัด' order by hn ";
	}
				...
				<td width="94%" height="600" valign="top">
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td width="51%" rowspan="5" align="center" valign="top">
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
						...
						<tr align="left" valign="top">
							...
							<td width="12%"><strong>เพศ : </strong>
								<? if ($rows["sex"] == "ช" || $rows["sex"] == "1") {
									echo "ชาย";
								} else if ($rows["sex"] == "ญ" || $rows["sex"] == "2") {
									echo "หญิง";
								} else {
									echo "ไม่ได้ระบุ";
								} ?></td>
					...
					<table width="100%" height="87%" border="0" cellpadding="0" cellspacing="0" style="border-top:1px solid #000000;">
						<tr>
							<td width="45%" height="416" align="left" valign="top">
