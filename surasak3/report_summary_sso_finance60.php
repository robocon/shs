    ...
    <tr>
      <td align="center">หน่วยงาน :
        ...
        <select name="camp" class="txtform" id="camp">
          <option value="ธกส.แม่ทะ60">ธกส.แม่ทะ60</option>
          <option value="เชียงใหม่สยามทีวี60">เชียงใหม่สยามทีวี60</option>
...
//echo "2-->";
$sql="SELECT  * ,c.`idcard`,c.`address`,c.`tambol`,c.`ampur`,c.`changwat`,c.`phone`,c.`congenital_disease` FROM opcardchk AS a LEFT JOIN opcard AS c ON c.hn = a.HN WHERE a.part='มูลนิธิคืนช้างสู่ธรรมชาติ60' and a.branch='ประกันสังคม' and a.active!='n' and a.HN !='60-3341' order by a.agey desc";
$showtext="(ตรวจตามสิทธิประกันสังคม เฉพาะ XRAY เท่านั้น)";
}else if($camp=="อีซูซุ60"){
//echo "2-->";
$sql="SELECT  * ,c.`idcard`,c.`address`,c.`tambol`,c.`ampur`,c.`changwat`,c.`phone`,c.`congenital_disease` FROM opcardchk AS a LEFT JOIN opcard AS c ON c.hn = a.HN WHERE a.part='อีซูซุ60' and a.course='ประกันสังคม' and a.active!='n' order by a.branch asc,a.agey desc";
$showtext="หจก.ลำปางศิริชัย (สาขาบ้านต้า, สำนักงานใหญ่, บริษัท อีซูซุลำปาง จำกัด)";
}else if($camp=="เชียงใหม่สยามทีวี60"){
//echo "2-->";
$sql="SELECT  * ,c.`idcard`,c.`address`,c.`tambol`,c.`ampur`,c.`changwat`,c.`phone`,c.`congenital_disease` FROM opcardchk AS a LEFT JOIN opcard AS c ON c.hn = a.HN WHERE a.part='เชียงใหม่สยามทีวี60' and a.course='ประกันสังคม' AND a.active!='n' order by a.agey desc";
}else if($camp=="ธกส.แม่ทะ60"){
//echo "2-->";
$sql="SELECT  * ,c.`idcard`,c.`address`,c.`tambol`,c.`ampur`,c.`changwat`,c.`phone`,c.`congenital_disease` FROM opcardchk AS a LEFT JOIN opcard AS c ON c.hn = a.HN  WHERE a.part='ธกส.แม่ทะ60' and a.course='ประกันสังคม' and a.active!='n' order by a.agey desc";
}else{
//echo "3-->";
...
?>  
  <tr>
    ...
    <td align="center"><? if(!empty($result2["bp1"])){ echo $result2["bp1"];}else{ echo $result2["bp3"];}?></td>
    <td align="center"><? if(!empty($result2["bp2"])){ echo $result2["bp2"];}else{ echo $result2["bp4"];}?></td>
    <td align="center"><?
...
?></td>
    <td align="left">
	<? 
		if($result2["cxr"]==""){ echo "ปกติ"; }else{ echo "<strong style='color:#FF0000'>ผิดปกติ</strong>"; }
	?></td>
  </tr>
<? } ?>  
</table>
