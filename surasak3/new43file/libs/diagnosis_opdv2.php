...
$db2 = $Conn = mysql_connect('localhost', 'root', '1234') or die( mysql_error() );
mysql_select_db('smdb', $Conn) or die( mysql_error() );

// $dirPath = realpath(dirname(__FILE__))."/export/2564/03";
$hospcode = '11512';
...
$where_thdate = " thidate LIKE '$thdate_opday%' ";
    ...
    if($numopd > 1){  //ถ้ามีหลาย record
        while(list($regisdate,$icd10,$type) = mysql_fetch_array($resultopd)){
            ...
            if($test_match > 0){
                ...
                dump("SELECT `code` FROM `clinic` WHERE detail LIKE '$clinic_name%'");
...
$filePath = '/var/www/html/sm3/surasak3/new43file/export/2564/03/diagnosis_opd.txt';
dump($filePath);
