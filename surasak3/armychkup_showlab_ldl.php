...
<table width="98%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;">
  <tr>
    ...
    <td width="12%" align="center" bgcolor="#FF9999"><strong>LDL</strong></td>
  </tr>
  <?
include("connect.inc");  
	$sql = "select * from armychkup where yearchkup='61' and camp !='' and age >=35 and ldl_lab ='' order by age desc";	
		//echo $sql;
		...
		$labquery = "select * from resulthead as a inner join resultdetail as b on a.autonumber=b.autonumber where a.profilecode='10001' AND a.hn ='".$result["hn"]."' and a.clinicalinfo='ตรวจสุขภาพประจำปี61'";
		//echo $labquery;
		...
		if($flaglab=="N" || $flaglab=="L"){		//ปกติ หรือ ต่ำ จะดี
			$ldllab="ปกติ";
		}else if($flaglab=="H"){  //สูง ไม่ดี
			$ldllab="ผิดปกติ";
		}else{
			$ldllab="";
		}
		$update="update armychkup set ldl_result='$resultlab',
														  ldl_flag='$flaglab',
														  ldl_lab='$ldllab' 
						where row_id='$rowid';";
		echo $update."<br>";		
