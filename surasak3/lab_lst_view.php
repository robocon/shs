...
$sql = "Select date_format(orderdate,'%Y-%m-%d') From resulthead where hn = '".$_GET["hn"]."' AND orderdate like '".$date_now."%' Order by  `autonumber` DESC limit 0,1";
...
<?php 
	...
	$select_date = "";
	$select_date .= "";
	$sql = "Select distinct date_format(orderdate,'%d-%m-') as orderdate1, date_format(orderdate,'%Y') as orderdate3, date_format(orderdate,'%Y-%m-%d') as orderdate2  From resulthead where hn='".$_GET["hn"]."' Order by orderdate DESC limit 0,10";
	//echo $sql;
	$result = mysql_query($sql);
	
	$xxx = explode("-", $date_now);

	$select_date .= $xxx[2]."/".$xxx[1]."/".($xxx[0]+543);
...
$lab_title = array();
$sql = "Select a.profilecode, count(b.labcode), a.testgroupname From (Select * From resulthead where hn='".$_GET["hn"]."' AND orderdate like '".$date_now."%' ) as a INNER JOIN resultdetail as b ON a.autonumber = b.autonumber group by a.profilecode  Order by  a.testgroupname ASC";
//echo $sql;
...
foreach($list_code_lab as $key => $value){
		...
		$r=4;
		$sql = "Select autonumber, profilecode,clinicalinfo, testgroupname From resulthead where hn='".$_GET["hn"]."' AND orderdate like '".$date_now."%' AND profilecode = '".$key."'  Order by autonumber DESC limit 0,1";
		$result = mysql_query($sql);
