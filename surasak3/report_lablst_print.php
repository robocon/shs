...
$sql = "Select date_format(orderdate,'%Y-%m-%d'),patientname,labnumber,sex From resulthead where hn = '".$_GET["hn"]."' AND labnumber= '".$_GET['labnumber']."' Order by  `autonumber` DESC limit 0,1";
...
<?php 
	...
	$select_date = "";
	$select_date .= "";
	$sql = "Select  date_format(orderdate,'%d-%m-') as orderdate1, date_format(orderdate,'%Y') as orderdate3, date_format(orderdate,'%Y-%m-%d') as orderdate2  From resulthead where hn='".$_GET["hn"]."'  Group by labnumber Order by orderdate DESC limit 0,10";
	//echo $sql;
	$result = mysql_query($sql);
...
$list_code_lab = array();
$lab_title = array();
$sql = "Select a.profilecode, count(b.labcode) From (Select * From resulthead where labnumber = '".$_GET['labnumber']."') as a INNER JOIN resultdetail as b ON a.autonumber = b.autonumber group by a.profilecode  Order by count(b.labcode) DESC , testgroupcode ASC";
//echo $sql;
...
echo "<FONT SIZE='5'><font face='Angsana New' style='line-height:20px;;' >HN:<b>".$_GET["hn"]."</b>&nbsp;&nbsp;ชื่อ:&nbsp;<b>",$patientname,"</FONT></b>";
...
foreach($list_code_lab as $key => $value){
		...
		$r=4;
		$sql = "Select autonumber, profilecode,clinicalinfo, testgroupname From resulthead where hn='".$_GET["hn"]."' AND labnumber = '".$_GET['labnumber']."' AND profilecode = '".$key."'  Order by autonumber DESC limit 0,1";
		$result = mysql_query($sql);
