...
<SCRIPT LANGUAGE="JavaScript">
	...
	function checkformf2(){
		...
		if(document.f2.credit[0].checked == false && document.f2.credit[1].checked == false && document.f2.credit[2].checked == false && document.f2.credit[3].checked == false && document.f2.credit[4].checked == false && document.f2.credit[5].checked == false && document.f2.credit[6].checked == false && document.f2.credit[7].checked == false){
			alert("กรุณาเลือกวิธี ชำระเงินด้วยครับ");
			return false;
...
<?php
    $query = "SELECT tradname,amount,price,part FROM drugrx WHERE idno = '$nRow_id' AND date = '".$_GET["sDate"]."' ";
    $result = mysql_query($query)
        or die("Query failed");
...
//        $x++;
    ...
    if (empty($sAn) && $sNetprice > 0){
        print (" <tr>\n".
           ...
           "  <td BGCOLOR=F5DEB3><font face='Angsana New'>0.00</td>\n".
...
//  OPD CASE
    if (empty($sAn) && $sNetprice > 0){
           $xNetpri=$sNetprice+0;
           $cPaid=$sNetprice+0; //opd case เก็บ 50 บาท
           $free=$sEssd+$sNessdy+$sDPY+'0'+$sDSY;
                            }

    if (empty($sAn) && $sNetprice < 0){
           $xNetpri=$sNetprice-0;
           $cPaid=$sNetprice-0; //opd case คืนยา,  คืนเงิน 50 บาท
           $free=$sEssd+$sNessdy+$sDPY+$sDSY-'0';
                            }
//  IPD CASE
    if (!empty($sAn) && $sNetprice > 0){
           $xNetpri=$sNetprice;
...
//ipd case คืนยา
    ...
    print "รวมงิน  $xNetpri บาท (<font face='Angsana New' size='5'><b>เบิกไม่ได้ $pay บาท</b>, เบิกได้ $free บาท)<br>";
          ...
          print "<form name='f2' method='POST' action='oprxbill50.php'>";
		 ...
		 <TR>
		 	...
		 	<TD>30บาท</TD>
		 	<TD align='right'>&nbsp;&nbsp;<INPUT TYPE='radio' NAME='credit' VALUE='HD' onclick=\"document.getElementById('detail2').innerHTML='ข้อมูลเพิ่มเติม'; detailhead2.style.display='';document.f2.detail_1.focus();checkptring(this.value);\"></TD>
		 	<TD>HD</TD>
