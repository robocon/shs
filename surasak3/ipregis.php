...
$cHn = sprintf("%s", $_POST['cHn']);
$cAdmitd = sprintf("%s", $_POST['cAdmitd']);
$cAn = sprintf("%s", $_POST['cAn']);
$cPtname = sprintf("%s", $_POST['cPtname']);
$cPtright = sprintf("%s", $_POST['cPtright']);
$cAge = sprintf("%s", $_POST['cAge']);
$cCamp = sprintf("%s", $_POST['cCamp']);
$cGoup = sprintf("%s", $_POST['cGoup']);
$cIdcard = sprintf("%s", $_POST['cIdcard']);
	...
	if($subbedcode=="47"){  //Ward Home Isolation
		if($subptright=="R02" || $subptright=="R03" || $subptright=="R04" || $subptright=="R16" || $subptright=="R33"){
			...
			$query ="UPDATE bed SET ptname='$cPtname',age='$cAge',idcard='$cIdcard',address='$cAddress',
							...
							an='$cAn',diagnos='$diag',diag1='$diag1',food='$food $addfood $foodContainerText',officer='',lastcalroom='$cAdmitd',chgdate='$cAdmitd',chgwdate='$cAdmitd',accno=1,bedpri='1000.00'  WHERE bedcode='$Bcode' ";
			...
			$query ="UPDATE bed SET ptname='$cPtname',age='$cAge',idcard='$cIdcard',address='$cAddress',
							muang='$cMuang',ptright='$cPtright',doctor='$doctor',date='$cAdmitd',hn='$cHn',
							an='$cAn',diagnos='$diag',diag1='$diag1',food='$food $addfood $foodContainerText',officer='',lastcalroom='$cAdmitd',chgdate='$cAdmitd',chgwdate='$cAdmitd',accno=1,bedpri='0.00'  WHERE bedcode='$Bcode' ";
			$result = mysql_query($query) or die("Query failed bed");  
			//// ward_log  admit ครั้งแรก  //// 			
			}
		}else{ //สิทธิอื่นๆ
		...
		$query ="UPDATE bed SET ptname='$cPtname',age='$cAge',idcard='$cIdcard',address='$cAddress',
						muang='$cMuang',ptright='$cPtright',doctor='$doctor',date='$cAdmitd',hn='$cHn',
						an='$cAn',diagnos='$diag',diag1='$diag1',food='$food $addfood $foodContainerText',officer='',lastcalroom='$cAdmitd',chgdate='$cAdmitd',chgwdate='$cAdmitd',accno=1,bedpri='1500.00'  WHERE bedcode='$Bcode' ";
		$result = mysql_query($query) or die("Query failed bed");  
		//// ward_log  admit ครั้งแรก  //// 	
		}else{ //สิทธิอื่นๆ
		//$admitD=date("Y-m-d H:i:s");
