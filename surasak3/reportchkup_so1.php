...
$sql = "Select a.date,a.txdate, a.hn, CONCAT(b.yot,' ',b.name,' ',b.surname) as full_name, a.depart, sum(a.price),b.idcard,b.note,sum(a.paidcscd),b.camp From opacc as a, opcard as b  where a.hn=b.hn   AND a.credit ='ตรวจสุขภาพ' group by a.hn, a.depart   ORDER by a.date,b.camp";
...
while(list($date,$txdate, $hn, $full_name, $depart, $price,$idcard,$note,$paidcscd,$camp) = Mysql_fetch_row($result)){
 ...
 $d=substr($date,8,2);
  ...
  $note=substr($note,0,25);
	...
	$list2[$hn] = $d."".$m."".$y."/".$full_name."/".$idcard."/".$camp;
switch($depart){
...
$num='0';
$i='0';
echo "<font face='Angsana New' size ='4'><center> <b>ลูกหนี้ตรวจสุขภาพประจำวันที่ $date2 <br></b> ";
...
<td><font face='Angsana New' size ='2'><center> <b>&nbsp;&nbsp;หน่วย&nbsp;&nbsp;</td>
...
$total=$list[$key]["PATHO"]+$list[$key]["XRAY"];
