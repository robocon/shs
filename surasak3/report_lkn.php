...
<center>
<span class="tet1">พิมพ์ใบตรวจสุขภาพโรงเรียนลำปางกัลยาณี</span> <br />
  <br />
...
$sql1="CREATE TEMPORARY TABLE  result1  Select * from  resulthead  WHERE hn='".$result2['HN']."' and clinicalinfo ='ตรวจสุขภาพกัลยาณี58' ";
  ...
  <tr>
    <td colspan="2"><table width="100%">
      <tr>
        ...
        <td width="77%" align="center" valign="top" class="texthead"><strong>แบบรายงานการตรวจสุขภาพโรงเรียนลำปางกัลยาณี</strong></td>
      ...
      <tr>
        <td align="center" valign="top" class="text3"><span class="text"><span class="text1"><span class="text2">ตรวจเมื่อวันที่ 16 มีนาคม 2558</span></span></span></td>
...
mmHg. </u></strong><span class="text3"><strong>P: </strong> <u>
  ...
  <tr>
    <td width="50%"  valign="top"><table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#000000" class="text3" style="border-collapse:collapse;">
      <tr>
        <td height="33" align="center"><strong class="text" style="font-size:22px"><u>CBC : การตรวจเม็ดเลือด</u></strong></td>
      </tr>
      <tr>
...
/////
 ...
 ?>         
          <tr>
            <td height="27" colspan="3"><strong>ผลตรวจ : </strong>              <? 
while($objResult = mysql_fetch_array($objQuery)){
if($objResult["labcode"]=="WBC"){
	if($objResult["result"] >= 5.0 && $objResult["result"] <= 10.0){
		...
		if($objResult["result"] < 5.0){
			$chkwbc = "ผิดปกติ มีระดับเม็ดเลือดขาวต่ำกว่าปกติ";
		}else if($objResult["result"] > 10.0){
			$chkwbc = "ผิดปกติ มีระดับเม็ดเลือดขาวสูงกว่าปกติ";
		}
	}
}
...
if($objResult["labcode"]=="NEU"){
	if($objResult["result"] >= 43 && $objResult["result"] <= 76){
		...
		if($objResult["result"] < 43){
			$chkneu = "ผิดปกติ ค่า NEU ต่ำ";
		}else if($objResult["result"] > 76){
			$chkneu = "ผิดปกติ ค่า NEU สูง";
		}
	}
}
...
if($objResult["labcode"]=="LYMP"){
	if($objResult["result"] >= 17 && $objResult["result"] <= 48){
		...
		if($objResult["result"] < 17){
			$chklymp = "ผิดปกติ ค่า LYMP ต่ำ";
		}else if($objResult["result"] > 48){
			$chklymp = "ผิดปกติ ค่า LYMP สูง";
		}
	}
}																					
...
if($objResult["labcode"]=="MONO"){
	if($objResult["result"] >= 4 && $objResult["result"] <= 10){
		...
		if($objResult["result"] < 4){
			$chkmono = "ผิดปกติ ค่า MONO ต่ำ";
		}else if($objResult["result"] > 10){
			$chkmono = "ผิดปกติ ค่า MONO สูง";
		}
	}
}	
...
if($objResult["labcode"]=="RBC"){
	if($objResult["result"] >= 4.00 && $objResult["result"] <= 6.2){
		...
		if($objResult["result"] < 4.00){
			$chkrbc = "ผิดปกติ ค่า RBC ต่ำ";
		}else if($objResult["result"] > 6.2){
			$chkrbc = "ผิดปกติ ค่า RBC สูง";
		}
	}
}
...
if($objResult["labcode"]=="HB"){
	if($objResult["result"] >= 12.5 && $objResult["result"] <= 16.4){
		...
		if($objResult["result"] < 12.5){
			$chkhb = "ผิดปกติ ค่า HB ต่ำ";
		}else if($objResult["result"] > 16.4){
			$chkhb = "ผิดปกติ ค่า HB สูง";
		}
	}
}
...
if($objResult["labcode"]=="HCT"){
	if($objResult["result"] >= 37 && $objResult["result"] <= 49){
		...
		if($objResult["result"] < 37){
			$chkhct = "ผิดปกติ มีระดับเม็ดเลือดแดงต่ำกว่าปกติ บ่งบอกถึงภาวะซีด";
		}else if($objResult["result"] > 49){
			$chkhct = "ผิดปกติ มีระดับเม็ดเลือดแดงสูงกว่าปกติ";
		}
	}
}
...
if($objResult["labcode"]=="MCV"){
	if($objResult["result"] >= 80 && $objResult["result"] <= 97){
		...
		if($objResult["result"] < 80){
			$chkmcv = "ผิดปกติ ค่า MCV ต่ำ";
		}else if($objResult["result"] > 97){
			$chkmcv = "ผิดปกติ ค่า MCV สูง";
		}
	}
}
...
if($objResult["labcode"]=="MCH"){
	if($objResult["result"] >= 27.0 && $objResult["result"] <= 31.2){
		...
		if($objResult["result"] < 27.0){
			$chkmch = "ผิดปกติ ค่า MCH ต่ำ";
		}else if($objResult["result"] > 31.2){
			$chkmch = "ผิดปกติ ค่า MCH สูง";
		}
	}
}
...
if($objResult["labcode"]=="MCHC"){
	if($objResult["result"] >= 31.8 && $objResult["result"] <= 35.4){
		...
		if($objResult["result"] < 31.8){
			$chkmchc = "ผิดปกติ ค่า MCHC ต่ำ";
		}else if($objResult["result"] > 35.4){
			$chkmchc = "ผิดปกติ ค่า MCHC สูง";
		}
	}
}
...
if($objResult["labcode"]=="PLTC"){
	if($objResult["result"] >= 140 && $objResult["result"] <= 400){
		...
		if($objResult["result"] < 140){
			$chkpltc = "ผิดปกติ ปริมาณเกร็ดเลือดมีค่าต่ำกว่าปกติ";
		}else if($objResult["result"] > 400){
			$chkpltc = "ผิดปกติ ปริมาณเกร็ดเลือดมีค่าสูงเกินปกติ";
		}
	}
}
...
if($chkwbc=="ปกติ" && $chkneu=="ปกติ" && $chklymp=="ปกติ" && $chkmono=="ปกติ" && $chkeos=="ปกติ" && $chkbaso=="ปกติ" && $chkrbc=="ปกติ" && $chkhb=="ปกติ" && $chkhct=="ปกติ" && $chkmcv=="ปกติ" && $chkmch=="ปกติ" && $chkmchc=="ปกติ" && $chkpltc=="ปกติ"){	
	if($objResult['flag']=='L' || $objResult['flag']=='H'){
		echo "ควรตรวจซ้ำหรือพบแพทย์";	
	}else{
		echo "ปกติ";
	}
}else{
	...
	echo " ควรตรวจซ้ำ หรือ พบแพทย์เพื่อหาสาเหตุ";	
	...
	?>
 	...
 	<? } ?>     
      <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse; border-top-style:none">
        <? if($result['hn']=="58.128"){ ?>
        <tr>
          <td><strong class="text" style="font-size:18px"><u>STOOL</u>&nbsp;&nbsp;&nbsp;&nbsp;</u>การตรวจอุจจาระ : <strong>
            <? echo "ปกติ"; ?>
          </strong></u></strong></td>
        </tr>
        <? } ?>
...
<table width="100%" border="0" class="text4">
  <tr>
    <td  width="50%" align="center"><strong>Authorise  LAB:</strong><?=$authorisename?> <strong> (<?=$authorisedate?>) CXR : </strong>พ.ต.ภูภูมิ วุฒิธาดา (ว.33906) รังสีแพทย์<strong> (23-03-2015) Doctor :พ.ต.เลอปรัชญ์ มังกรกนกพงศ์ (02-04-2015)</strong></td>
