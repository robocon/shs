...
// แจ้งเตือนตัดรอบบ่าย **************************************************
if(empty($sDiag)){ //ไม่มีชื่อโรค
	echo "<script> 
	Swal.fire(
	  'แจ้งเตือน',
	  'ยังไม่มีการวินิจฉัยโรคจากแพทย์',
	'warning'
	)
	</script>";
}	
...
<?php
	...
	//$ptright=iconv_substr($sPtright,4,'UTF-8');
	//$doctor=iconv_substr($sDoctor,5,'UTF-8');
    print "HN: $sHn, สิทธิ์:$ptright<br>";
    print "โรค: $sDiag, แพทย์ :$doctor<br>";
?>	
<font face='Angsana New' size="5"><A HREF="drxadddiag.php?sDate=<?php echo urlencode($sDate);?>&nRow_id=<?php echo urlencode($sRow_id);?>&aVn=<?=$sVn;?>&aHn=<?=$sHn;?>" target="_blank" >แก้ไขชื่อโรค</A></font>&nbsp;&nbsp;<font face='Angsana New' style="color:#FF0000; size:2;">(เฉพาะสิทธิที่ต้องนำไปเบิกต้นสังกัดเท่านั้น)</font><br>
<?
	$dpyaa=array();
    ...
    print "รวมงิน  $xNetpri บาท (<font face='Angsana New' size='5'><b>เบิกไม่ได้ $pay บาท</b>, เบิกได้ $free บาท)<br>";
	 ...
	 if (substr($sPtright,0,3)=='R03 ' OR 'R09' OR 'R07' OR 'R10' OR 'R11' OR 'R13'){
		$cPaid= $pay;
          ...
          print "<form name='f1' method='POST' action='oprxcscd_2022.php' Onsubmit='return checkformf1()'>";

		  print "เก็บเงินส่วนที่เบิกไม่ได้&nbsp;&nbsp;&nbsp;";
		  $submit = "เก็บเงินออกใบเสร็จ ส่วนที่เบิกไม่ได้";
	 }else{
		 print "<form  name='f1'  method='POST' action='oprxbill_2022.php' Onsubmit='return checkformf1()'>";
          print "เก็บเงิน&nbsp;&nbsp;&nbsp; ";
...
//ipd case คืนยา
    ...
    print "รวมงิน  $xNetpri บาท (<font face='Angsana New' size='5'><b>เบิกไม่ได้ $pay บาท</b>, เบิกได้ $free บาท)<br>";
          ...
          print "<form name='f2' method='POST' action='oprxbill_2022.php'>";
