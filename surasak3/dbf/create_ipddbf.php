...
$credits = array(
	'OFC' => 'จ่ายตรง', 'SSS' => 'ประกันสังคม', 'LGO' => 'อปท', 'UC' => 'UC', 'BKK' => 'กทม', 
);
?>
<span class="font1">
...
$dbname1 = "INS".$yy.$mm.".dbf";
	...
	if (!dbase_create($dbname1, $def1)) {
	  echo "Error, can't create the database1\n";
		...
		if($_POST['credit']=="BKK"){
			$sqlop1 ="
			SELECT a.`hn`, a.`dcdate`, a.`an`, a.`ptright` 
			FROM `ipcard` as a inner join ipmonrep as b ON a.an=b.an
			WHERE a.`dcdate` like '".$_POST['year']."-".$_POST['mon']."%' 
			AND (a.ptright like '%R02%' OR a.ptright LIKE '%R03%' OR a.ptright LIKE '%R53%') AND b.credit='กทม'";	
		}else{
			$sqlop1 ="
			SELECT `hn`, `dcdate`, `an`, `ptright` 
			FROM `ipcard`
			WHERE `dcdate` like '".$_POST['year']."-".$_POST['mon']."%' AND $newcredit";
		}
...
//--------------------Start DataSet2-------------------------//
	...
	if (!dbase_create($dbname2, $def2)) {
	  echo "Error, can't create the database2\n";
		...
		if($_POST['credit']=="BKK"){
			$sqlop2 ="select a.hn,a.dcdate,a.an,a.ptright from  ipcard AS a INNER JOIN ipmonrep AS b ON a.an=b.an  where a.dcdate like '".$_POST['year']."-".$_POST['mon']."%' AND (a.ptright like '%R02%' OR a.ptright LIKE '%R03%' OR a.ptright LIKE '%R53%') AND b.credit='กทม'";
		}else{
			$sqlop2 ="select hn,dcdate,an,ptright from  ipcard  where dcdate like '".$_POST['year']."-".$_POST['mon']."%' and $newcredit ";
		}
		//echo "test-->".$sqlop3."<br>";
...
if($idcard2 !=""){
	...
	$namepat2 = iconv("UTF-8", "WINDOWS-874",$namepat2);
	$yot2 = iconv("UTF-8", "WINDOWS-874",$yot2);
	$name2 = iconv("UTF-8", "WINDOWS-874",$name2);
	$surname2 = iconv("UTF-8", "WINDOWS-874",$surname2);
...
$dbname7 = "IPD".$yy.$mm.".dbf";
	...
	if($_POST['credit']=="BKK"){
		$sql7 ="select a.* from ipcard AS a INNER JOIN ipmonrep AS b ON a.an=b.an where a.dcdate like '".$_POST['year']."-".$_POST['mon']."%' AND (a.ptright like '%R02%' OR a.ptright LIKE '%R03%' OR a.ptright LIKE '%R53%') AND b.credit='กทม'";   //  Query เอาข้อมูลจากตาราง ipcard
	}else{	
...
$dbname9 = "IDX".$yy.$mm.".dbf";
	...
	if($_POST['credit']=="BKK"){
		$sqlipc ="select a.* from ipcard AS a INNER JOIN ipmonrep AS b ON a.an=b.an where a.dcdate like '".$_POST['year']."-".$_POST['mon']."%' AND (a.ptright like '%R02%' OR a.ptright LIKE '%R03%' OR a.ptright LIKE '%R53%') AND b.credit='กทม'";   //  Query เอาข้อมูลจากตาราง ipcard
	}else{
...
$dbname10 = "IOP".$yy.$mm.".dbf";
	...
	if($_POST['credit']=="BKK"){
		$sqliop ="select a.* from ipcard AS a INNER JOIN ipmonrep AS b ON a.an=b.an where a.dcdate like '".$_POST['year']."-".$_POST['mon']."%' AND (a.ptright like '%R02%' OR a.ptright LIKE '%R03%' OR a.ptright LIKE '%R53%') AND b.credit='กทม'";   //  Query เอาข้อมูลจากตาราง ipcard	
	}else{
...
$dbname11 ="CHT".$yy.$mm.".dbf";
	$def11 = array(
	  ...
	  array("PTTYPE","C",2),	  
 	  array("PERSON_ID","C",13),	 
	  array("SEQ","C",15),
	  array("OPD_MEMO","C",255),
	  array("INVOICE_NO","C",50),
	  array("INVOICE_LT","C",50));
...
$dbname12 = "CHA".$yy.$mm.".dbf";
	...
	if (!dbase_create($dbname12, $def12)) {
	  echo "Error, can't create the database11\n";
		...
		//--------------------------------- ค่าใช้จ่ายผู้ป่วยใน	---------------------------------//	
	if($_POST['credit']=="BKK"){
		$dbsql ="select a.an from ipcard AS a INNER JOIN ipmonrep AS b ON a.an=b.an where a.dcdate like '".$_POST['year']."-".$_POST['mon']."%' AND (a.ptright like '%R02%' OR a.ptright LIKE '%R03%' OR a.ptright LIKE '%R53%') AND b.credit='กทม' group by a.an";   //  Query เอาข้อมูลจากตาราง ipcard			
	}else{
...
//--------------------------------- ค่าใช้จ่ายผู้ป่วยใน	---------------------------------//	
if($_POST['credit']=="BKK"){
	$dbsql ="select a.* from ipcard AS a INNER JOIN ipmonrep AS b ON a.an=b.an where a.dcdate like '".$_POST['year']."-".$_POST['mon']."%' AND (a.ptright like '%R02%' OR a.ptright LIKE '%R03%' OR a.ptright LIKE '%R53%') AND b.credit='กทม'";   //  Query เอาข้อมูลจากตาราง ipcard	
}else{	
...
while($rowsdb = mysql_fetch_array($dbresult)){
	...
	$newvn=sprintf('%03d',$vn);
		...
		// 10 = ค่าห้อง/ค่าอาหาร				
		$sqlip ="select * from  ipacc  where an='".$an14."' and part='BFY' group by code HAVING sum(price) > 0";  // เอาข้อมูลมาตามเงื่อนไข โดยไม่สนวันที่
		//echo $sqlip."</br>"; 
		...
		while($rowsip2 = mysql_fetch_array($resultip2)){			
			...
			$detail= 	$rowsdp["detail"];	
						...
						if($code14 =="SURG"){
							...
							//echo $code;
						}else if($rowsdp["depart"]=="WARD" && $rowsdp["part"]=="DPY"){
							$sqldrx="SELECT codex FROM  labcare WHERE code ='".$code14."'";
							//echo $sqldrx."</br>";
							$resultdrx = mysql_query($sqldrx) or die("Query labcare failed");
							$rowsdrx=mysql_fetch_array($resultdrx);	
							$code = substr($rowsdrx["codex"],0,4);							
						}else{
			...
			while($rowsdb = mysql_fetch_array($dbresult)){
				...
				$resultip = mysql_query($sqlip) or die("Query ipcard failed16");
					...
					$i++;
						...
						$drugname16=str_replace(array("\r","\n","\r\n",'"',"'","<",">","&"),'',$drugname16);
						$drugname16 = iconv("UTF-8","WINDOWS-874",$drugname16);
						...
						//CLINIC
						$sqlclinic ="select code from clinic where  detail =  '".$rowsop["clinic"]."'";   //  Query เอาข้อมูลจากตาราง inputm
						$resultclinic = mysql_query($sqlclinic) or die("Query clinic failed");
						$rowsclinic = mysql_fetch_array($resultclinic);	
						$clinic = $rowsclinic["code"];
						//echo"==>".$clinic."<br>";
					...
					$rowsdrx=mysql_fetch_array($resultdrx);
						...
						$unit = iconv("UTF-8","WINDOWS-874",$unit);
						$packing = iconv("UTF-8","WINDOWS-874",$packing);						
					...
					$sigtext = iconv("UTF-8","WINDOWS-874",$sigtext);
