...
<strong>ส่งออกข้อมูล DBF คนไข้นอกประจำวัน dataset_V25640115 (อัพเดทล่าสุด วันที่ 01-06-65)</strong></font></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a target=_top  href="../../nindex.htm"><< ไปเมนู</a>
<br />
<hr />
<span class="font1">
...
//echo 	$sqlop1 ;
		...
		$sql1 ="select row_id,thidate,hn,an,vn,ptright,icd10 from opday where hn ='".$hnop."' and thidate like '$dateopacc%'";   //  Query เอาข้อมูลจากตาราง opday
		//echo "test-->".$sql1."<br>";
...
//---------------------ใช้ข้อมูลจากตาราง opcard---------------------//
		$sqlop ="select * from opcard where hn ='".$hnop."'";   //  Query เอาข้อมูลจากตาราง opday
		$resultop = mysql_query($sqlop) or die("Query opcard failed");
   		$rowsop = mysql_fetch_array($resultop);
		$cid1=$rowsop["idcard"];  //  CID ใช้ตัวแปรนี้นำเข้าข้อมูล		
...
if($_POST['credit']	=="000"){
	...
	if($day=="00"){
		...
		//clinic
		$sqlclinic ="select code from clinic where  detail =  '".$rows3["clinic"]."' ";   //  Query เอาข้อมูลจากตาราง inputm
...
$dbname5 = "ODX".$yy.$mm.".dbf";
	...
	if (!dbase_create($dbname5, $def5)) {
	  echo "Error, can't create the database\n";
		...
		$sql5 ="select txdate,hn from opacc where txdate LIKE '$txdate%' and credit='$newcredit' group by hn";
		//echo $sql5."<br>";
		...
		$sql51="select icd10,type from diag where hn='$hn5' and svdate LIKE '$date5%'";		
		//echo $sql51."<br>";
		...
		$resultop = mysql_query($sqlop) or die("Query opday failed");
   		$rowsop = mysql_fetch_array($resultop);
			...
			$toborow=$rowsop["toborow"];
			$codeptright = substr($rowsop["ptright"],0,3);
			...
			if($diag5==""){
				...
				$rowsdoc = mysql_fetch_array($resultdoc);
					if($numdoc > 0){
							if($codeptright=="R07" && $newcredit=="30บาท"){
								$newdrdx = "ว".$rowsdoc["doctorcode"];
							}else{
	...
	if (!dbase_create($dbname12, $def12)) {
	  echo "Error, can't create the database11\n";
		...
		list($amountopacc) = Mysql_fetch_row($result123);
			...
			$newdatecha =$newdatech.$datecha[1].$datecha[2];  //  DATE ใช้ตัวแปรนี้นำเข้าข้อมูล
				$chrgitem12 ="B1";
			}elseif($chrgitem=="DENTA"){  //ค่าบริการทางทันตกรรม
		...
		//	$amountopacc=$rows12["paidcscd"];
			...
			$newdatecha =$newdatech.$datecha[1].$datecha[2];  //  DATE ใช้ตัวแปรนี้นำเข้าข้อมูล
				$chrgitem12 ="E1";
			}elseif($chrgitem=="NID"){  //บริการฝังเข็ม
...
//---------------Start Dataset13---------------//
//แฟ้มข้อมูลที่ 13 มาตรฐานแฟ้มข้อมูลอุบัติเหตุ ฉุกเฉิน และรับส่งเพื่อรักษา
...
$dbname14 = "ADP".$yy.$mm.".dbf";
	...
	if (!dbase_create($dbname14, $def14)) {
	  echo "Error, can't create the database14\n";
		...
		//--------------------------------- ค่าใช้จ่ายผู้ป่วยนอก	---------------------------------//	
		$sql14 ="select * from  opacc  where credit like '$newcredit%' and date like '".$_POST['year']."-".$_POST['mon']."-".$_POST['day']."%' and (depart ='DENTA' OR depart='NID' OR depart='PATHO' OR depart='XRAY' OR  depart='SURG' OR  depart='EMER' OR  depart='OTHER') group by substring(txdate,1,10),hn,depart";
		...
		//echo $sql14."<br>";
   		...
   		while($rows14 = mysql_fetch_array($result14)){	
			...
			$yy14=$yy14-543;
			$orderdate="$yy14-$mm14-$dd14";
			$dateopd ="$yy14$mm14$dd14";  //  DATEOPD ใช้ตัวแปรนี้นำเข้าข้อมูล	
			//echo "--->".$dateopd."<br>";
			
//---------------------ใช้ข้อมูลจากตาราง opday---------------------//	
		$sqlop ="select * from opday where hn='".$hn14."' and thidate like '$subdate%' order by row_id desc limit 1";   //  Query เอาข้อมูลจากตาราง opday
		$resultop = mysql_query($sqlop) or die("Query opday failed Line 1362");
   		$rowsop = mysql_fetch_array($resultop);
		...
		if($_POST['credit']=="UCS"){
			...
			$sqladp="select code, sum(amount) as qty, sum(price) as rate, sum(yprice) as total, sum(nprice) as totcopay,depart,part,detail from  patdata  where date like '".$subdate."%' and hn='".$hn14."' and depart='$depart' and (an ='' || an is null) group by code";
			//echo $sqladp."<br>";
			...
			if($code14=="COVR22" || $code14=="COVR23"){
				$type="10";
			}else{
				...
				if($code14=="INJ"){
					...
					$codex="55020";		
				}else if($code14=="55080" && $newcredit=="30บาท"){  //ค่าติดตาม OP SI 300 บาท เพราะพยาบาลบันทึกรหัสผิด
					$codex="COVR23";
					...
					From ( 
						...
						AND `profilecode` = 'COVID'
						AND `clinicalinfo` = 'AgCG3 ,' 
					) as a , 
					resultdetail as b  
					...
					list($labcode,$result,$unit,$normalrange,$flag)=mysql_fetch_array($result);
					
					if($result=="negative" || $result=="Negative" || $result=="NEGATIVE"){
						$status1="2";
					}else{
						$status1="1";
					}
				...
				$db14 = dbase_open($dbname14, 2);
					if ($db14) {
						if($code14=="AgCG3" && $newcredit=="30บาท"){ 
...
//---------------Start Dataset15---------------//
//แฟ้มข้อมูลที่ 15 มาตรฐานแฟ้มข้อมูลกรณีที่ผู้ป่วยมีการลากลับบ้าน
...
//---------------Start Dataset16---------------//
//แฟ้มข้อมูลที่ 16 มาตรฐานแฟ้มข้อมูลการใช้ยา
$dbname16 = "DRU".$yy.$mm.".dbf";
	...
	while($rows161 = mysql_fetch_array($result161)){	
		...
		while($rows16 = mysql_fetch_array($result16)){	
			...
			$sqldrglist=mysql_query("select drugcode from druglst where drugcode='".$rows16["drugcode"]."'");
			list($drugcode16)=mysql_fetch_array($sqldrglist);
	...
	echo "ดาวน์โหลดข้อมูล DBF ผู้ป่วยนอกเพื่อนำเข้าโปรแกรม E-Claim...สิทธิ $newcredit...ประจำวันที่ $dbfname <a href=$ZipName>คลิกที่นี่ดาวน์โหลด</a>";	
