...
<?
		...
		$showyear="25".$nPrefix;
?>		
<form method="POST" action="show_xrayarmychkup.php">
<input name="act" type="hidden" value="show">
<b>ข้อมูลกำลังพลที่มา X-RAY ตรวจสุขภาพประจำปี <?=$$showyear;?></b>       <a target=_self  href='../nindex.htm'><< ไปเมนูหลัก </a>
...
echo "<hr>";
	...
	$tbsql="select  * from chkup_solider where xray between '$startdate 00:00:00' and '$enddate 23:59:59' and yearchkup='$nPrefix' order by camp asc";
	//echo $tbsql;
	$tbquery=mysql_query($tbsql);
	$tbnum=mysql_num_rows($tbquery);
?> 
<div align="center"><b>ข้อมูลกำลังพลที่มา X-RAY ตรวจสุขภาพ</b><b>ประจำปี
    <?=$showyear;?>
</b> </div>
...
<table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;">
  ...
  <?
	if($tbnum < 1){
		...
		$i++;
		$chkdate=substr($tbrows["xray"],0,10);
