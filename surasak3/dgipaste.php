...
print "<form method='post' action='dgipseek.php'>";
print "<font face='Angsana New'><a target=_BLANK href='dgicode.php'>รหัสยา</a>&nbsp;&nbsp;";
print "<input type='text' name='drugcode' size='8' value=$sDcode>&nbsp;&nbsp;&nbsp;&nbsp;";
print "จำนวน  <input type='text' name='amount' size='4'>&nbsp;</font>";
print "<p><font face='Angsana New'><a target=_BLANK href='slicode.php'>วิธีใช้</a>&nbsp;&nbsp;&nbsp;&nbsp;"; 
...
If (!empty($drugcode)){
    ...
    while (list ($drugcode, $tradname, $salepri) = mysql_fetch_row ($result)) {
        print (" <tr>\n".
           "  <td BGCOLOR=66CDAA><a target='right'  href=\"infoip.php? Dgcode=$drugcode& Amount=$amount& Trade=$tradname & Price=$salepri & Slcode=$slipcode\">$drugcode</a></td>\n".
