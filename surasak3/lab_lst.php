...
$sql = "Select patientname,sex,dob From resulthead where hn = '".$_POST["search_hn"]."'  limit 1";
...
$i=0;
	...
	$sql = "Select date_format(orderdate,'%Y-%m-%d') as dateresult, date_format(orderdate,'%d') as dateresult2, date_format(orderdate,'%m') as dateresult4, date_format(orderdate,'%Y') as dateresult3,labnumber From resulthead where hn = '".$_POST["search_hn"]."' Group by labnumber order by orderdate DESC";
	...
	while($arr = mysql_fetch_assoc($result)){
		...
		$i++;
$sql = "Select sourcename,clinicianname,clinicalinfo From resulthead where hn = '".$_POST["search_hn"]."'  limit 1";
$result2 = Mysql_Query($sql);
list($sourcename,$clinicianname,$clinicalinfo) = Mysql_fetch_row($result2);
...
<tr >
	...
	<td align="center"><A HREF="lab_lst_view.php?hn=<?php echo urlencode($_POST["search_hn"]);?>&lab_date=<?php echo urlencode($arr["dateresult"]);?>&labnumber=<?=$arr['labnumber'];?>" target="_blank" >ดูข้อมูล</A></td>
