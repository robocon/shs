...
if($_POST["act"]=="add"){
	...
	while($rows=mysql_fetch_array($query)){
		...
		`autonumber` , `orderdate` , `labnumber` , `hn` , `patienttype` , `patientname` , `sex` , `dob` , `sourcecode` , `sourcename` , `room` , `cliniciancode` , `clinicianname` , `priority` , `clinicalinfo`  ) VALUES (
		'', '".$Thidate2."', '".date("ymd").sprintf("%03d", $nLab)."', '".$rows["idcard"]."', '".$patienttype."', '".$ptname."', '".$gender."', '".$dbirth."', '', '', '','".$cliniciancode."', 'MD022 (ไม่ทราบแพทย์)', '".$priority."', '".$clinicalinfo."');";
		...
		foreach ($arrlab as $value) {
			...
			$sql_detail = "Select code,oldcode,detail From labcare where code = '".$value."' limit 0,1 ";
			$query_detail = mysql_query($sql_detail);
			list($code,$oldcode,$detail) = mysql_fetch_row($query_detail);   
