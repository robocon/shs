...
if($action == 'print'){
    ...
    $qLab = $dbi->query("SELECT `depart` FROM `labcare` WHERE `code` = '$code'");
    $lab = $qLab->fetch_assoc();
    ...
    $qMed = $dbi->query($sql);
    if($qMed->num_rows === 0){
        $dbi->query("INSERT INTO `medicalcertificate` (`thidate`,`number`,`hn`,`part`,`doctor`)VALUES('$thDateTime','$nRunno','$cHn','$cPart','$cDoctor');");
    }
