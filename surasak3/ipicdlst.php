...
If (!empty($month)){
   $yrmn=$year."-".$month;
   print"โรคหลักที่พบบ่อย 10 อันดับแรก,  เดือน $month ปี$year";
    include("connect.inc");
     $query = "SELECT ptname,hn,an,diag,icd10,complica,dctype,date,dcdate,days,price,bedcode,doctor FROM ipcard  WHERE date LIKE '$yrmn%' ORDER BY icd10 ASC";  
