...
$sql = "select * from km where doc_id='".$_GET['doc_id']."'" ;
$result = mysql_query($sql) ;
...
<form action="" method="post"  enctype="multipart/form-data" name="f1" id="f1" onSubmit="JavaScript:return fncSubmit()">
  <table width="765"  border="0" align="center" class="fontthai" bgcolor="#FFCC99">
    <tr>
      <td colspan="3" align="center" class="style13">ระบบจัดเก็บองค์ความรู้</td>
    </tr>
    <tr>
      <td class="style51">ประเภท KM:</td>
      <td colspan="2"><select name="type" size="1" class="style5" id="type">
...
<? 
	 $strsql="SELECT * FROM km_file WHERE doc_id='".$_GET['doc_id']."' order by file_name ASC "; 
...
<input type="hidden" name="row<?=$x;?>" value="<?=$strrow['row_id'];?>" />      </td>
    </tr>
    <? 
	$x++;
	} ?>
...
<tr>
  <td width="100%" align="center" class="style51"><a href="javascript:MM_openBrWindow('km_insert.php?doc_id=<?=$dbarr['doc_id'];?>','','width=500,height=500')">เพิ่มไฟล์</a></td>
</tr>
</table>      </td>
    </tr>
    <tr class="style51">
