...
$d1 = substr($today,8,2);
$d2 = substr($today,5,2);
$d3 = substr($today,0,4)-543;

$mk = mktime(0,0,0,$d2,$d1+1,$d3);
$dmk = date("Y-m-d",$mk);
$cutmonn = explode("-",$dmk);
$today1 = ($cutmonn[0]+543)."-".$cutmonn[1]."-".$cutmonn[2];

    print "<input type=button onclick='history.back()' value='<< กลับไป'>";
    include("connect.inc");
    $query="CREATE TEMPORARY TABLE opday1 SELECT * FROM opday WHERE thidate between '$today 16:00:00' and '$today1 07:59:59'";
...
<?php
  ...
  $ex= 'EX01';
    ...
    $query = "SELECT vn,thdatehn,thidate,hn,ptname,an,diag,ptright,doctor,okopd,toborow,borow,goup,officer FROM opday WHERE thidate between '$today 16:00:00' and '$today1 07:59:59' ";
    $result = mysql_query($query)
        or die("Query failed");
