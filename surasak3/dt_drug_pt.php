...
$limit30checkday = 30;
$sql = "CREATE TEMPORARY TABLE drugrx_pt_notinj SELECT row_id FROM drugrx_pt WHERE hn = '".$_SESSION["hn_now"]."' AND drugcode <> 'INJ' AND 
	(
...
$sql = " Select date_format(date,'%d-%m-%Y'), tradname, amount, slcode From drugrx_pt where amount > 0 AND hn = '".$_SESSION["hn_now"]."' AND drugcode = '".$_GET["search"]."' AND status = 'Y' AND  date between '".$date1."' AND '".$date2."' Order by date DESC ";
...
$sql2 = " Select tradname, amount, slcode,idno From drugrx_pt where amount > 0 AND hn = '".$_SESSION["hn_now"]."' AND drugcode = '".$_GET["search"]."' AND  date like '".$date2."%' Order by date DESC ";
$result2 = mysql_query($sql2);
$rows2 = mysql_num_rows($result2);
list($tradname2, $amount2, $slcode2,$idno2)=mysql_fetch_array($result2);

$sql3 = " Select doctor From phardep_pt where row_id='$idno2'";
...
if(isset($_GET["action"]) && $_GET["action"] == "viewtolist"){
	...
	echo "<FORM name=\"form_list\" METHOD=POST ACTION=\"dt_drug_reason_pt.php\" onsubmit=\"return viatch($count,'$code');\">
...
for($i=0;$i<$count;$i++){
	...
	$date1 = (date("Y",$times)+543).date("-m-d H:i:s",$times);
	$date2 = (date("Y")+543).date("-m-d H:i:s");
	$sql = " Select date_format(date,'%d/%m/%Y'), amount, slcode From drugrx_pt where amount > 0 AND hn = '".$_SESSION["hn_now"]."' AND drugcode = '".$_SESSION["list_drugcode"][$i]."' AND status = 'Y' AND  date between '".$date1."' AND '".$date2."' Order by date DESC ";
	$result = mysql_query($sql);
	...
	echo "<div  align=\"center\"><INPUT TYPE=\"submit\" value=\"      ตกลง      \" ></div></TD> <TD></TD></TR>";
...
<?php
	...
	FROM drugrx_pt as a INNER JOIN (Select `drugcode`,`lock_dr` From druglst_pt ".$where1.") as b ON a.drugcode = b.drugcode
	WHERE a.hn = '".$_SESSION["hn_now"]."' AND a.an is not null AND a.date like '".$_GET["date_remed"]."%' AND a.drugcode <> 'INJ' AND a.row_id not in (Select row_id From drugrx_pt_notinj)
...
<?php
	...
	SELECT a.date, a.drugcode, a.tradname, a.slcode, sum( a.amount ) AS amount, a.reason, a.part, a.drug_inject_amount,a.drug_inject_unit,a.drug_inject_amount2,a.drug_inject_unit2 ,a.drug_inject_time, a.drug_inject_slip , a.drug_inject_type,  a.drug_inject_etc, a.part,b.lock_dr 
	FROM drugrx_pt as a INNER JOIN (Select `drugcode`,`lock_dr` From druglst_pt ".$where1.") as b ON a.drugcode = b.drugcode
	WHERE a.hn = '".$_SESSION["hn_now"]."' AND a.date like '".$_GET["date_remed"]."%' AND a.drugcode <> 'INJ' AND a.row_id not in (Select row_id From drugrx_pt_notinj)
	GROUP BY a.drugcode, a.slcode
	HAVING sum( a.amount ) >0
	";
...
<?php
	...
	$sql = "Select a.drugcode, b.tradname, a.slcode, a.amount, b.part, b.lock_dr, a.drug_inject_amount,a.drug_inject_slip,  a.drug_inject_type,  a.drug_inject_etc  From dr_drugsuit_detail as a, druglst_pt as b where a.drugcode = b.drugcode ".$where1." AND a.for_id = '".$_GET["date_sult"]."' ";
...
if(isset($_GET["action"]) && $_GET["action"] == "addtolist"){
	...
	$sql = "Select part From druglst_pt Where drugcode = '".$_GET["drugcode"]."' limit 1";
	$result = Mysql_Query($sql);
...
if(isset($_GET["action"]) && $_GET["action"] == "listdrugprov"){
	...
	$sql = " Select row_id, item, stkcutdate From phardep_pt where hn = '".$_SESSION["hn_now"]."' AND whokey = 'DR' AND idname='".$_SESSION["dt_doctor"]."' AND date like '".((date("Y")+543).date("-m-d"))."%' Order by row_id DESC limit 1";
	$result = Mysql_Query($sql);
	...
	$sql = "Select drugcode, amount, slcode, drug_inject_amount, drug_inject_unit, drug_inject_time,  drug_inject_slip,  drug_inject_type,  drug_inject_etc, reason   From drugrx_pt where idno = '".$id."' AND hn='".$_SESSION["hn_now"]."' AND  date like '".((date("Y")+543).date("-m-d"))."%' ";
	$result = Mysql_Query($sql);
	while($arr = Mysql_fetch_assoc($result)){
	...
	if($_SESSION["nRunno"] == ""){
		...
		$query = "SELECT runno FROM runno WHERE title = 'phardep1' limit 0,1";
		$result2 = mysql_query($query) or die("Query failed");
		list($_SESSION["nRunno"]) = mysql_fetch_row($result2);
		 $_SESSION["nRunno"]++;
		
		$query ="UPDATE runno SET runno = ".$_SESSION["nRunno"]." WHERE title='phardep1'";
		$result2 = mysql_query($query) or die("Query failed");
...
if(isset($_GET["action"]) && $_GET["action"] == "drug"){
	...
	$sql = "Select drugcode, tradname, genname,unit, stock, salepri, part, `lock`, lock_dr From druglst_pt where ".$where." (drugcode like '%".$_GET["search"]."%' OR genname LIKE '%".$_GET["search"]."%' OR  tradname LIKE '%".$_GET["search"]."%') Order by drugcode ASC";
	$result = Mysql_Query($sql)or die(Mysql_error());
	...
	if(Mysql_num_rows($result) > 0){
		...
		while($arr = Mysql_fetch_assoc($result)){
			...
			echo "<tr bgcolor=\"$bgcolor\" ".$style.">
					...
					".$obj."</td>
					<td align=\"right\" bgcolor=\"$bgcolor\">ชื่อยา : </td>
					<td bgcolor=\"$bgcolor\">",$arr["genname"]," / ",$arr["tradname"],"</td>
...
if($_GET["search"] == "1BONA"){
	...
	$sql = "Select count(row_id) From drugrx_pt where drugcode = '".$_GET["search"]."' AND date BETWEEN '".(date("Y",$limit_date)+543).date("-m-d H:i:s",$limit_date)."' AND '".(date("Y")+543).date("-m-d H:i:s")."' ";
	...
	$sql = "CREATE TEMPORARY TABLE drugrx2 Select slcode, drugcode, amount From  `drugrx_pt` where drugcode = '".$_GET["search"]."' Order by row_id DESC  limit ".$limit_row;
	$result = Mysql_Query($sql);
	...
	$sql = "Select part From druglst_pt Where drugcode = '".$_GET["search"]."' limit 1 ";
	$result = Mysql_Query($sql);
	list($part) = Mysql_fetch_row($result);
	echo ",".$part;
	
	$sql = "Select tradname From druglst_pt Where drugcode = '".$_GET["search"]."' limit 1 ";
	$result = Mysql_Query($sql);
...
if(isset($_GET["action"]) && $_GET["action"] == "addslip"){
	...
	$sql = "CREATE TEMPORARY TABLE drugrx2 Select slcode, drugcode, amount From  `drugrx_pt` where drugcode = '".$_GET["search"]."' Order by row_id DESC  limit 20";
	//$result = Mysql_Query($sql);
...
if(isset($_GET["action"]) && $_GET["action"] == "checkdrugcode"){
	...
	$sql = "SELECT count(drugcode) as amountcode FROM `druglst_pt` where drugcode = '".$_GET["search"]."' ";
	$result = Mysql_Query($sql);
	$arr = Mysql_fetch_assoc($result);
...
if(isset($_GET["action"]) && $_GET["action"] == "checkptright"){
	...
	$sql = "SELECT lockptright,tradname FROM `druglst_pt` where drugcode = '".$_GET["search"]."' ";
	$result = Mysql_Query($sql);
	$arr = Mysql_fetch_assoc($result);
...
if(isset($_GET["action"]) && $_GET["action"] == "checkdrugamount"){
	if((substr($_SESSION["ptright_now"],0,3) == "R07"  || substr($_SESSION["ptright_now"],0,3) == "R09"  )){
	$sql = "SELECT limit_pay, limit_ptright FROM `druglst_pt` where drugcode='".$_GET["chkdrugcode"]."'";
	$result = Mysql_Query($sql);
	$arr = Mysql_fetch_assoc($result);
	...
	$sql = "SELECT limit_pay FROM `druglst_pt` where drugcode='".$_GET["chkdrugcode"]."'";
	$result = Mysql_Query($sql);
	$arr = Mysql_fetch_assoc($result);
...
///////////////////////////////////////////////////////
		...
		$sql = " Select row_id, doctor From phardep_pt where hn = '".$_SESSION["hn_now"]."' AND whokey = 'DR' AND idname <> '".$_SESSION["dt_doctor"]."' AND date like '".((date("Y")+543).date("-m-d"))."%' AND dr_cancle is null Order by row_id DESC limit 1 ";
		...
		if($rows > 0){
			...
			while(list($row_id, $doctor) = mysql_fetch_row($result)){
				$sql = " Select b.tradname, a.drugcode, a.amount, b.unit ,a.slcode From drugrx_pt as a LEFT JOIN druglst_pt as b ON a.drugcode = b.drugcode where a.idno = '".$row_id."'  ";
		...
		$sql  = "SELECT  drugcode FROM drugrx_pt  WHERE hn = '".$_SESSION["hn_now"]."' AND drugcode <> 'INJ' ";
		$result = mysql_query($sql);
		$rows = mysql_num_rows($result);
		if($rows > 0){
	...
	$list_session2 = " '".implode("','",$listinteraction)."' ";//listยาที่มาจาหแพทย์อื่น
	$list_session3 = " '".implode("','",$listremedint)."' ";//listยาที่มาจากการremed

	$sql = "SELECT first_drugcode, between_drugcode, effect, action, follow, onset, violence, referable, status  FROM drug_interaction  where (first_drugcode = '".$_GET["drugcode"]."' AND between_drugcode in (".$list_session.") ) OR (between_drugcode = '".$_GET["drugcode"]."' AND first_drugcode in (".$list_session.") ) OR (first_drugcode = '".$_GET["drugcode"]."' AND between_drugcode in (".$list_session2.") ) OR (between_drugcode = '".$_GET["drugcode"]."' AND first_drugcode in (".$list_session2.") ) OR (first_drugcode = '".$_GET["drugcode"]."' AND between_drugcode in (".$list_session3.") ) OR (between_drugcode = '".$_GET["drugcode"]."' AND first_drugcode in (".$list_session3.") ) ";
	...
	$rows = Mysql_num_rows($result);
		if($rows == 0){
			...
			$arr = Mysql_fetch_assoc($result);
			$i=0;
			$sql = " Select genname From  druglst_pt where drugcode in ('".$arr["first_drugcode"]."','".$arr["between_drugcode"]."') ";
			$result = Mysql_Query($sql);
...
function add_drug(drugcode){
	...
	url = 'dt_drug_pt.php?action=addamount&search=' + drugcode;
	xmlhttp.open("GET", url, false);
	xmlhttp.send(null);
	...
	//url = 'dt_drug_pt.php?action=addslip&search=' + drugcode;
	//xmlhttp.open("GET", url, false);
	//xmlhttp.send(null);
...
function addslip2(action,str,len,no) {
		...
		if(str.length >= len){
			url = 'dt_drug_pt.php?action='+action+'&search=' + str+'&num=' + no;
...
function ajaxcheck(action,str,drugcode){
	xmlhttp = newXmlHttp();
	url = 'dt_drug_pt.php?action='+action+'&search=' + str+'&chkdrugcode=' + drugcode;
	xmlhttp.open("GET", url, false);
	xmlhttp.send(null);
...
function viewlist(){
	...
	xmlhttp = newXmlHttp();
	url = 'dt_drug_pt.php?action=viewtolist';
	xmlhttp.open("GET", url, false);
	xmlhttp.send(null);
...
function alert500(){
	...
	var stat = '';
	xmlhttp = newXmlHttp();
	url = 'dt_drug_pt.php?action=alert500';
	xmlhttp.open("GET", url, false);
	xmlhttp.send(null);
...
function select_dateremed(date_remed){
	...
	xmlhttp = newXmlHttp();
	url = 'dt_drug_pt.php?action=date_remed&date_remed=' + date_remed;
	xmlhttp.open("GET", url, false);
	xmlhttp.send(null);
...
function select_dateremed2(date_remed){
	...
	xmlhttp = newXmlHttp();
	url = 'dt_drug_pt.php?action=date_remed2&date_remed=' + date_remed;
	xmlhttp.open("GET", url, false);
	xmlhttp.send(null);
...
function select_datesult(date_sult){
	...
	xmlhttp = newXmlHttp();
	url = 'dt_drug_pt.php?action=date_sult&date_sult=' + date_sult;
	xmlhttp.open("GET", url, false);
	xmlhttp.send(null);
...
function del_list(){
	...
	for(i=0;i<eval(document.form_list.elements.length);i++){
		...
		if(document.form_list.elements[i].name == "check_list[]" && document.form_list.elements[i].checked == true){
			url = 'dt_drug_pt.php?action=deltolist&number=' + document.form_list.elements[i].value;
			xmlhttp.open("GET", url, false);
			xmlhttp.send(null);
...
function drug_interaction(drugcode){
	...
	xmlhttp = newXmlHttp();
	url = 'dt_drug_pt.php?action=drug_interaction&drugcode=' + drugcode;
	xmlhttp.open("GET", url, false);
	xmlhttp.send(null);
...
function listdrugprov(){
	...
	if(confirm('ท่านต้องการแก้ไขข้อมูลใช้การจ่ายยาใช้หรือไม่')){
		xmlhttp = newXmlHttp();
		url = 'dt_drug_pt.php?action=listdrugprov';
		xmlhttp.open("GET", url, false);
		xmlhttp.send(null);
		viewlist();
	}
...
function viatch(ing,code){
	...
	if(code=="ER"){
		if(eval(document.getElementById("total_phar_price").value) > 1000){
			xmlhttp = newXmlHttp();
			url = 'dt_drug_pt.php?action=drug_500';
			xmlhttp.open("GET", url, false);
			xmlhttp.send(null);
...
//date_format( a.date, '%d/%m/%Y' )date_format( a.date, '%Y-%m-%d' )
	...
	FROM drugrx_pt as a INNER JOIN (Select `drugcode`,`lock_dr` From druglst_pt ".$where1." ) as b ON a.drugcode = b.drugcode
	WHERE a.hn = '".$_SESSION["hn_now"]."' AND a.an is null and a.drugcode <> 'INJ' AND a.row_id not in (Select row_id From drugrx_pt_notinj)
...
//date_format( a.date, '%d/%m/%Y' )date_format( a.date, '%Y-%m-%d' )
	...
	SELECT DISTINCT  a.date AS date1,  a.date as date2 
	FROM drugrx_pt as a INNER JOIN (Select `drugcode`,`lock_dr` From druglst_pt ".$where1." ) as b ON a.drugcode = b.drugcode
	WHERE a.hn = '".$_SESSION["hn_now"]."' AND a.an is not null AND a.drugcode <> 'INJ' AND a.row_id not in (Select row_id From drugrx_pt_notinj)
