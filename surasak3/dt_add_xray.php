...
$_SESSION["list_bill"] = "";
	...
	$sql = "INSERT INTO `xray_doctor` (`date` ,`hn` ,`vn` ,`yot` ,`name` ,`sname` ,`detail` ,`doctor` ,`status` ,`xrayno` ,`film` ,`type_diag`,`detail_all`,`dbirth`) 
	VALUES 
	('".$Thidate."', '".$_SESSION["hn_now"]."', '".$_SESSION["vn_now"]."', '".$_SESSION["yot_now"]."', '".$_SESSION["name_now"]."', '".$_SESSION["surname_now"]."', '".$detail2."', '".$_SESSION["dt_doctor"]."', 'N', '".$_SESSION["nRunno"]."', '".$_POST["type"]."', '".$_POST["type_diag"]."', '".$detail2."', '".$dbirth."');";
	$result = mysql_query($sql);
	$xray_no = $_SESSION["nRunno"];
	...
	for($i=0;$i<$item;$i++){
		...
		$sql1 = "INSERT INTO `xray_doctor_detail` ( `date` ,`hn` ,`xrayno` ,`doctor_detail`,`detail_all`)
		VALUES 
		('".(date("Y")+543).date("-m-d H:i:s")."','".$_SESSION["hn_now"]."','$xray_no','$detail','$detail2');";
		$q=mysql_query($sql1);
