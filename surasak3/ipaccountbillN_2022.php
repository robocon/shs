...
print "<CENTER>โรงพยาบาลค่ายสุรศักดิ์มนตรี ลำปาง<br></CENTER>";
 $Thaidate=date("d-m-").(date("Y")+543)."  ".date("H:i:s");
   print "<CENTER>สรุปค่ารักษาพยาบาลผู้ป่วยใน&nbsp;&nbsp;<B>เลขที่ใบเสร็จ&nbsp;..................</B>&nbsp;&nbsp;เมื่อวันที่&nbsp;&nbsp;$Thaidate</CENTER>";
...
$sql = "Select sum(price),status FROM ipacc WHERE an = '$an' and status='จำหน่าย' group by status ";
...
print "<b><CENTER>รวมเงินทั้งสิ้น&nbsp;&nbsp; $nNprice1 บาท</b>&nbsp;&nbsp;&nbsp;";
...
$part='dpy';
 //   $query = "SELECT * FROM ipacc WHERE an = '$cAn' and accno='$accno' and part ='$part' ";
  $query = "SELECT * FROM ipacc WHERE an = '$cAn' and part like '%n'   ";
    $result = mysql_query($query)
        or die("Query failed ipacc");
    for ($i = mysql_num_rows($result) - 1; $i >= 0; $i--) {
        if (!mysql_data_seek($result, $i)) {
...
print "  </table> <div style=\"page-break-before: always;\"></div>";
   print "<br><B>สรุปค่ารักษาพยาบาลผู้ป่วยส่วนเกิน/เบิกไม่ได้</B><br>";
...
////
  //  $query = "SELECT date,depart,detail,amount,price,paid,part,idname FROM ipacc WHERE an = '$cAn' and accno='$accno' ORDER BY depart,part,date ASC ";
  $query = "SELECT date,depart,detail,sum(amount),sum(price),paid,part,idname,code,price FROM ipacc WHERE an = '$cAn' and part like '%n'   group by code  ORDER by part,code ";
...
if($amount < '0' ){$color='#FF0000';}else {$color='F5DEB3';};
		print("<tr>\n".
               ...
               // "<td bgcolor=F5DEB3>$day</td>\n".
				...
				//	   "<td bgcolor=F5DEB3>$dpycode</td>\n".  
                "<td bgcolor='$color' align='right'>$amount</td>\n".  
                "<td bgcolor='$color' align='right'>&nbsp;$price</td>\n".  
             //   "<td bgcolor=F5DEB3>$paid</td>\n".  
                "<td bgcolor='$color' align='right'>&nbsp;$part</td>\n".  
             //   "<td bgcolor=F5DEB3>$idname</td>\n".  
                " </tr>\n");
		      }
 include("unconnect.inc");
