...
if(isset($_GET["action"]) && $_GET["action"] == "view"){
	...
	$hn = $_GET['hn'];
	$currDate = date('Y-m-d');
	$sql = "SELECT `row_id` FROM `appoint` WHERE `en_date` = '$currDate' AND `hn` = '$hn' AND `depcode` LIKE 'U16%' AND `detail2` LIKE '%ฉีดยา%'  ";
	$qApp = mysql_query($sql);
	if(mysql_num_rows() > 0){
		echo "<p>ผู้ป่วยมีบันทึกนัดฉีดยาจากห้องฉุกเฉินแล้วในวันนี้ กรุณาตรวจสอบข้อมูลก่อนบันทึก</p>";
	}
