...
<div><A HREF="..\nindex.htm">&lt;&lt; ไปเมนู</A> &nbsp;&nbsp;<a href="report_lablst.php" target="_blank">โปรแกรมใหม่</a></div>
<?php
...
$sql = "Select dob From resulthead where hn = '".$_POST["search_hn"]."'  limit 1";
$result2 = Mysql_Query($sql);
list($dob) = Mysql_fetch_row($result2);

$sql3 = "Select yot,name,surname,sex From opcard where hn = '".$_POST["search_hn"]."'  limit 1";
$result3 = Mysql_Query($sql3);
list($yot,$fname,$sname,$sex) = Mysql_fetch_row($result3);
$patientname="$yot $fname  $sname";
...
$i=0;
	...
	$sql = "Select autonumber,date_format(orderdate,'%Y-%m-%d') as dateresult, date_format(orderdate,'%d') as dateresult2, date_format(orderdate,'%m') as dateresult4, date_format(orderdate,'%Y') as dateresult3, labnumber From resulthead where hn = '".$_POST["search_hn"]."' order by orderdate DESC";
...
<tr >
	...
	<td align="center"><A HREF="lab_lst_view.php?hn=<?php echo urlencode($_POST["search_hn"]);?>&lab_date=<?php echo urlencode($arr["dateresult"]);?>" target="_blank" >ดูข้อมูล</A></td>
