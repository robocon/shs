...
#btn_show_icf:hover, .icf_code:hover{
  cursor: pointer;
}
.icf_code{
  color: blue;
        ...
        <div style="position: relative;">
          <input name="yot" type="text" id="yot" value="<?=$cYot;?>" size="5" class="notify43">
            ...
            <div id="res_yot" style="position: absolute; top: 0; left: 0; background-color: #ffffff; z-index: 1; padding: 4px; display: none;">
              <div id="close_res_yot" onclick="close_res_yot()">[ปิดหน้าต่าง]</div>
              <div style="padding: 8px;">
                ค้นหาคำนำหน้า : <input type="text" id="search_res_yot">
              </div>
              <table style="width:600px;" class="chk_table">
                <tr style="background: #bbbbbb; color: #000000;">
                  <th>ตัวย่อ</th>
                  <th>รายละเอียด</th>
                </tr>
                <?php 
                $sql_prefix = "SELECT * FROM `f43_person_1` ORDER BY `sort` ASC, `id` ASC";
                ...
                if($q!==false)
                {
                  ...
                  while ($pref = mysql_fetch_assoc($q)) {
                    ?>
                    <tr class="find_my_prefix" data-prefix="<?=$pref['detail'];?>">
                      <td>
                        <a href="javascript:void(0)" style="color: #a67a42;" data-prefix-selected="<?=$pref['abbreviations'];?>" class="prefix-selected"><?=$pref['abbreviations'];?></a>
                      </td>
                      <td><?=$pref['detail'];?></td>
                    </tr>
                    <?php
...
<table border="0" align="center">
  ...
  <tr>
    ...
    <td><input type="text" name="phone" id="phone" size="10" value="<?=$cPhone;?>"></td>
    <td align="right" class="fonthead">&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
					...
					if( search_txt.length > 3 ){
							...
							for (var index = 0; index < icf_list.length; index++) {
									...
									if( regex1.test(element) == true ){
											...
											htm += '<td style="color: #000000;">'+element+'</td>';
											htm += '</tr>';
									}
							}
