<?php
  print "รายการอุปกรณ์เวชภัณฑ์PT <br> ";
if(isset($_GET["action"]) && $_GET["action"] == "drugcode"){
	include("connect.inc");
	
	$sql = "Select drugcode,tradname from druglst_pt  where  drugcode like '%".$_GET["search1"]."%' limit 10 ";
...
function searchSuggest(str,len,getto) {
		...
		if(str.length >= len){
			...
			url = 'dglst_pt.php?action=drugcode&search1=' + str+'&getto=' + getto;
...
If (!empty($drugcode)){
    ...
    $query = "SELECT drugcode,tradname,genname,salepri,part,stock,mainstk,totalstk, pack, packpri_vat, comcode, comname, unitpri,code24, edpri,spec FROM druglst_pt WHERE drugcode LIKE '$drugcode%' ";
    ...
    while (list ($drugcode, $tradname,$genname, $salepri,$part,$stock,$mainstk,$totalstk,$pack,$packpri_vat, $comcode, $comname, $unitpri,$code24, $edpri,$spec) = mysql_fetch_row ($result)) {
        print (" <tr>\n".
           "  <td BGCOLOR=66CDAA><a   href=\"dgedit_pt.php?Dgcode=$drugcode\">$drugcode</a></td>\n".
