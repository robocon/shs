...
?>
<form action="money_sso_61.php" method="post" class="hide">
    <div>
        เลือกแผนกที่ต้องการ
        <select name="branch" id="">
            ...
            foreach ($branchs as $key => $branch) {
                ...
                $selected = ( $op == $branch['branch'] ) ? 'selected="selected"' : '' ;
...
if ( $action == 'show' ) {
    ...
    $post_branch = input_post('branch');

    $where_branch = ''; 
    if( $post_branch != '1' ){
        $where_branch = "AND `branch` = '$post_branch'";
    }
...
<div style="text-align: center; font-weight: bold;">รายชื่อตรวจสุขภาพประจำปี 2561</div>
<?php
if( $post_branch != '1' ){
    ?><div style="text-align: center; font-weight: bold;"><?=$post_branch;?></div><?php
}
?>
