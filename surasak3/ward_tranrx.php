<?php
	...
	$cPtname=$_SESSION["ptname_ipd"];
	$cHn=$_SESSION["hn_ipd"];
	$cAn=$_SESSION["an_ipd"];
	$cPtright=$_SESSION["ptright_ipd"];
	$ward=substr($_SESSION["bed"],0,2);
	if($ward=="42"){
		$cWard="หอผู้ป่วยรวม";
	}elseif($ward=="43"){
		$cWard="หอผู้ป่วยสูติ";
		...
		$cWard="หอผู้ป่วยพิเศษ";
	}
	$cBed=substr($_SESSION["bed"],2);
	...
	$x=$_POST['totalitem'];
for ($n=0; $n<$x; $n++){
	$query = "SELECT drugcode,tradname,unit,salepri,part,freepri FROM druglst  WHERE drugcode='".$_SESSION["list_drugcode"][$n]."' ";  
    $result = mysql_query($query) or die("Query failed");

    while (list ($drugcode,$tradname,$unit,$salepri,$part,$freepri) = mysql_fetch_row ($result)){
		$aDgcode[$n] = $drugcode;
		$aTrade[$n]  = jschars($tradname);
		$aSalepri[$n]  = $salepri;
		$aFreepri[$n]  = $freepri;
		$aPart[$n] = $part;
		$aUnit[$n] = $unit;
		$aAmount[$n] = $_SESSION["list_drugamount"][$n];
		$aSlipcode[$n] = $_SESSION["list_drugslip"][$n];
		$aMoney[$n]= $_SESSION["list_drugamount"][$n]*$salepri;
   ...
   If (!empty($aDgcode[$n])){
	   ...
	   If (!empty($status)){
			print"$status<br>";
				   ...
				   //จำนวน $nDay วัน
    print  "<font face='Angsana New'>เบิกเวชภัณฑ์ผู้ป่วยใน,  วันที่ $Thaidate<br> ";
