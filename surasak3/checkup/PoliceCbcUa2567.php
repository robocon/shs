...
$camp = "ศูนย์ฝึกอบรมตำรวจภูธร ภาค 5 67";
?>
...
FROM ( SELECT `row`,`HN`,`exam_no` FROM `opcardchk` WHERE `part` = '$camp' ORDER BY `row` ASC ) AS b 
LEFT JOIN ( SELECT `hn`,`ptname`,`cxr` FROM `out_result_chkup` WHERE `part` = '$camp' ) AS a ON a.`hn` = b.`HN` 
...
{ 
    ?>
    <h3 class="headerExtra">แบบรายงานการตรวจสุขภาพผู้สมัครสอบเพื่อบรรจุเป็นเข้านักเรียนนายสิบตำรวจ ประจำปีงบประมาณ 2567</h3>
    <h3 class="headerExtra">โรงพยาบาลค่ายสุรศักดิ์มนตรี อ.เมือง จ.ลำปาง โทร 054-839-305-6 ต่อ 1135</h3>
    <h3 class="headerExtra">หน่วยงาน : ศูนย์ฝึกอบรมตำรวจภูธร ภาค 5 วันที่ตรวจ 16 กรกฎาคม 2567</h3>
