...
$query = "SELECT row_id,drugcode,tradname,unitpri,salepri,stock,mainstk,rxaccum,rxrate,stkpmon FROM druglst 
WHERE `drugcode` NOT LIKE '12%' 
AND `drugcode` NOT LIKE '13%' 
AND `drugcode` NOT LIKE '14%' 
AND `drugcode` NOT LIKE '15%' 
AND `drugcode` NOT LIKE '17%' 
AND `drugcode` NOT LIKE '18%' 
AND `drugcode` NOT LIKE '19%' 
AND `drugcode` NOT LIKE '20%' 
AND `drugcode` NOT LIKE '21%' 
AND `drugcode` NOT LIKE '22%' 
AND `drugcode` NOT LIKE '23%' 
AND `drugcode` NOT LIKE '24%' 
AND `drugcode` NOT LIKE '25%'
AND `drugcode` NOT LIKE '30%'
AND `drugcode` NOT LIKE '16%'
OR `drugcode` IN ('16OR014','16OR015','16OR016','16OR017')
";
$result = mysql_query($query) or die("Query druglst failed");

while(list($row_id,$drugcode,$tradname,$unitpri,$salepri,$stock,$mainstk,$rxaccum,$rxrate,$stkpmon) = mysql_fetch_row ($result)) {
	$n++; 
	$nRow_id=$row_id;
	$cDrugcode=$drugcode;
	$cTradname=$tradname;
