...
// ให้สิทธิเฉพาะ ศูนย์คอม+ทะเบียนที่เห็น เพราะมันแชร์ไปที่หน้าของ ER ด้วย 
// มันมีปัญหาว่า จนท. ER เผลอไปกด
...
if( in_array($_SESSION['smenucode'], $white_list) === true ){
    ...
    while (list ($vn,$thdatehn,$thidate,$hn,$ptname,$an,$diag,$ptright,$doctor,$okopd,$toborow,$borow,$goup,$officer,$kew,$phaok,$idcard) = mysql_fetch_row ($result)) {
        ...
        "  <td BGCOLOR=$color><font face='Angsana New'><a target=_BLANK  href=\"opdcard_reg.php?cHn=$hn&cVn=$vn&cTdatehn=$thdatehn\">$hn</a></td>\n".  
...
<table>
    <tr>
        ...
        <th bgcolor=6495ED><font face='Angsana New'>พิมพ์ใบตรวจโรค</th>
        <th bgcolor=6495ED><font face='Angsana New'>พิมพ์ใบต่อ</th>
        <th bgcolor=#008080 align="center"><font face='Angsana New'>ใบยินยอมฉีดวัคซีน COVID19</th>
    </tr>
<?php
...
while (list ($vn,$thdatehn,$thidate,$hn,$ptname,$an,$diag,$ptright,$doctor,$okopd,$toborow,$borow,$goup,$officer,$kew,$phaok,$idcard,$row_id) = mysql_fetch_row ($result)) {
    ...
    "  <td BGCOLOR=#00CC99 align='center'><font face='Angsana New'><a target=_BLANK  href=\"rxform2.5_1.php? cTdatehn=$thdatehn&cPtname=$ptname&cHn=$hn&cDoctor=$doctor&cDiag=$diag&cOkopd=$okopd&ctoborow=$toborow&cVn=$vn\">พิมพ์</a></td>\n".	
	// "  <td BGCOLOR=$color><font face='Angsana New'>$borow</td>\n".
    //  "  <td BGCOLOR=$color><font face='Angsana New'>$officer</td>\n".
    //   "  <td BGCOLOR=#FF0000><font face='Angsana New'>$detel$detel1</td>\n".
    " </tr>\n");
...
$yr=substr($today,6,4) +543;
$today="$d-$m-$yr";
print "วันที่ $today  รายชื่อผู้ป่วยตรวจสุขภาพกองทัพบกประจำปีเรียงตามลำดับล่าสุด";
...
<table>
    <tr>
        ...
        <th bgcolor=6495ED><font face='Angsana New'>พิมพ์ใบตรวจโรค</th>
        <th bgcolor=6495ED><font face='Angsana New'>พิมพ์ใบต่อ</th>
		<th bgcolor=#008080 align="center"><font face='Angsana New'>สติ๊กเกอร์ตรวจสุขภาพกองทัพบก</th>
    </tr>
<?php
$query = "SELECT a.vn,a.thdatehn,a.thidate,a.hn,a.ptname,a.an,a.diag,a.ptright,a.doctor,a.okopd,a.toborow,a.borow,a.goup,a.officer,a.kew,a.phaok,b.idcard,a.row_id FROM opday as a,opcard as b WHERE a.hn=b.hn and a.thidate LIKE '$today%' and a.toborow like '$ex%' order by a.thidate desc ";
$result = mysql_query($query) or die("Query failed");
...
while (list ($vn,$thdatehn,$thidate,$hn,$ptname,$an,$diag,$ptright,$doctor,$okopd,$toborow,$borow,$goup,$officer,$kew,$phaok,$idcard,$row_id) = mysql_fetch_row ($result)) {
    ...
    "  <td BGCOLOR=$color><font face='Angsana New'><a target=_BLANK  href=\"rxform2.1.php? cTdatehn=$thdatehn&cPtname=$ptname&cHn=$hn&cDoctor=$doctor&cDiag=$diag&cOkopd=$okopd&ctoborow=$toborow&cVn=$vn\">$toborow</a></td>\n".
    "  <td BGCOLOR=$color><font face='Angsana New'><a target=_BLANK  href=\"opdcard_reg.php?cHn=$hn&cVn=$vn\">$hn</a></td>\n".  
    "  <td BGCOLOR=#00CC99 align='center'><font face='Angsana New'><a target=_BLANK  href=\"opdprint_stkchkup1.php?cTdatehn=$thdatehn&cHn=$hn&cVn=$vn\">พิมพ์</a></td>\n".	
	// "  <td BGCOLOR=$color><font face='Angsana New'>$borow</td>\n".
    //  "  <td BGCOLOR=$color><font face='Angsana New'>$officer</td>\n".
    //   "  <td BGCOLOR=#FF0000><font face='Angsana New'>$detel$detel1</td>\n".
    " </tr>\n");
