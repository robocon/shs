...
<div class="container mt-4">
    ...
    <?php 
					...
					if(!empty($drug_code)&& empty($drug_date)){
						$sql = "SELECT drugcode,tradname,date,amount FROM drugrx WHERE hn='$patient_id' AND drugcode='$drug_code' AND an IS NULL ORDER BY date DESC";			
					}else if(!empty($drug_date) && empty($drug_code)){
						$sql = "SELECT drugcode,tradname,date,amount FROM drugrx WHERE hn='$patient_id' AND date LIKE '$drug_date%' AND an IS NULL ORDER BY date DESC";			
					}else if(!empty($drug_code) && !empty($drug_date)){
						$sql = "SELECT drugcode,tradname,date,amount FROM drugrx WHERE hn='$patient_id' AND (drugcode='$drug_code' AND date LIKE '$drug_date%') AND an IS NULL ORDER BY date DESC";			
					}else{
						$sql = "SELECT drugcode,tradname,date,amount FROM drugrx WHERE hn='$patient_id' AND an IS NULL ORDER BY date DESC";
					}			
					//echo $sql;
					$query = mysql_query($sql);	
					...
					while($item = mysql_fetch_array($query)){ ?>
						<div class="card custom-drug-card mb-3">
							<div class="card-body">
								...
								<p class="card-text">
									<strong>🔢 จำนวน:</strong> <?= $item['amount'] ?> เม็ด
								</p>
							</div>
						</div>
					<?php } ?>
