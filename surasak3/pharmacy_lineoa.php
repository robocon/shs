        ...
        <div class="card-body">
            <form method="post" class="row g-3 mb-4">
				...
				<div class="col-md-5">              
					...
					$sql = "SELECT drugcode,tradname FROM drugrx WHERE date >='2566-01-01 00:00:00' AND hn='$patient_id' AND an IS NULL GROUP BY drugcode ORDER BY drugcode ASC";			
					//echo $sql;
                ...
                <div class="col-md-5">
                    <label class="form-label">เลือกวันที่ได้รับ</label>
					 <?php
					// คำสั่ง SQL ดึงชื่อยา
					$sql = "select substring(date,1,10) as showdate from drugrx where date >='2566-01-01 00:00:00' AND hn='$patient_id' and an is null group by substring(date,1,10) ORDER BY `date` DESC";			
					//echo $sql;
					$query = mysql_query($sql);			
					?>					
...
<div class="container mt-4">
    ...
    <?php 
					...
					if(!empty($drug_code)&& empty($drug_date)){
						$sql = "SELECT drugcode,tradname,date,amount FROM drugrx WHERE date >='2566-01-01 00:00:00' AND hn='$patient_id' AND drugcode='$drug_code' AND amount > 0 AND an IS NULL ORDER BY date DESC";			
					}else if(!empty($drug_date) && empty($drug_code)){
						$sql = "SELECT drugcode,tradname,date,amount FROM drugrx WHERE date >='2566-01-01 00:00:00' AND hn='$patient_id' AND date LIKE '$drug_date%'  AND amount > 0 AND an IS NULL ORDER BY date DESC";			
					}else if(!empty($drug_code) && !empty($drug_date)){
						$sql = "SELECT drugcode,tradname,date,amount FROM drugrx WHERE date >='2566-01-01 00:00:00' AND hn='$patient_id' AND (drugcode='$drug_code'  AND amount > 0 AND date LIKE '$drug_date%') AND an IS NULL ORDER BY date DESC";			
					}else{
						$sql = "SELECT drugcode,tradname,date,amount FROM drugrx WHERE date >='2566-01-01 00:00:00' AND hn='$patient_id'  AND amount > 0 AND an IS NULL ORDER BY date DESC";
					}			
					//echo $sql;
					$query = mysql_query($sql);	
