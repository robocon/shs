...
$oneDayList = array('STAT','STAT1');
$continueList = array('OLD','CONT');

for($i=0;$i<$item;$i++){
	// เงื่อนไขเดิมคือ amount > 0

	// เงื่อนไขใหม่คือ CONT + OLD จะแสดงทั้งหมดถึงแม้ว่ารายการยาจะเป็น 0
	// STAT + STAT1 จะต้องมี amount > 0
	if( 
		( $_POST["Statcon"][$i] == "STAT" || $_POST["Statcon"][$i] == "STAT1" ) && $_POST["Amount"][$i] > 0 
		|| 
		( $_POST["Statcon"][$i] == "OLD" || $_POST["Statcon"][$i] == "CONT" ) 
	){
