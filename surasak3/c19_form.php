...
if($action=="test_hn")
{
    ...
    if($opcard_q->num_rows > 0)
    {
        ...
        $opday_q = $dbi->query("SELECT `toborow` FROM `opday` WHERE `thdatehn` = '$th_date_hn' limit 1");
        ...
        {
            echo $dbi->error." ผู้ป่วยยังไม่ได้ลงทะเบียน";
        }
    }
    else
    {
...
<!--                <div class="w3-third">
            ...
            <div class="w3-row-padding">
                ...
                <?php 
                $plan_count_list = array(1,2,3,4,5,6);
                foreach ($plan_count_list as $key => $plan) {
                    $plan_checked = (4==$plan) ? 'checked="checked"' : '';
                    ?>
                    <div class="w3-third">
