...
$sql1 = "Select * from  `dxofyear_emp` where `row_id` ='".$_POST['row_id']."'";
...
if(isset($_POST["row_id"]) && $_POST["row_id"] != ""){
	...
	if(empty($dxdr_ofyear['ldl'])){
		$_POST['stat_ldl'] = '';
		$_POST['reason_ldl'] = '';
		$dxdr_ofyear['ldlrange'] = '';
		$dxdr_ofyear['ldlflag'] = '';
	}else{
		if($_POST['stat_ldl']=='ปกติ'){
			$_POST['reason_ldl'] = '';
			$dxdr_ofyear['ldlrange'] = '';
			$dxdr_ofyear['ldlflag'] = '';
		}
...
// Refresh to doctor page
	header('Location: report_dxofyear_emp_manual.php?hn='.$_POST["hn"]);
	exit;
...
if($result && isset($_POST["submit"])){
	...
	<tr>
	  ...
	  <tr>
		<td>ผลการตรว?: <?php echo $arrs["summary"]." ".$arrs["diag"];?></td>
	  </tr>
      <?
      	if($_POST['normal41']=="ผิดปกติ"|$_POST['normal42']=="ผิดปกติ"|$_POST['normal43']=="ผิดปกติ") $text41="ตัว";
      ...
      <tr>
	    <td>ความผิดปกต? <?=$text41?> <?=$text44?> <?=$text46?> <?=$text47?> <?=$text49?> <?=$text81?> <?=$text?></td>
      </tr>
      <? }?>
	  <tr>
		<td>แพทย?: <?php echo $arrs["doctor"];?></td>
	  </tr>
	</table>
