...
if($_SESSION["smenucode"]=="ADM" || $_SESSION["smenucode"]=="ADMCSCD"){ 
	$sql="select * from opacc where (date >= '$chkdate1 00:00:00' and date <='$chkdate2 23:59:59') and credit='จ่ายตรง'  order by hn asc";
}else{
	$sql="select * from opacc where (date >= '$chkdate1 00:00:00' and date <='$chkdate2 23:59:59') and credit='จ่ายตรง'  order by date asc";	
}
//echo $sql;
...
$sqlopc="select count(row_id) from patdata where hn='".$rows["hn"]."'and date like '$txdate%' and code ='9902' group by hn limit 1";
//echo "$sqlopc<br>";
$queryopc=mysql_query($sqlopc);
$numcovid=mysql_num_rows($queryopc);
...
if($_POST["act"]=="update"){
	...
	if($_SESSION["smenucode"]=="ADM" || $_SESSION["smenucode"]=="ADMCSCD"){
		$edit="update opacc set credit_detail='$approvecode', status_stm='$status_stm',paidcscd='$paidcscd' where row_id='$postid'";
	}else{
