...
include("connect.inc");
	...
	$nPrefix="25".$nPrefix;
?>	
<title>รายงานผลการตรวจสุขภาพกำลังพล ทบ. ประจำปี <?=$nPrefix;?></title>
<style type="text/css">
<!--
body,td,th {
...
<form name="form1" method="post" action="reportchkup_armytp3_new.php" >
    ...
    <tr>
      <td align="center">หน่วย :
        ...
        <select name="camp" id="camp">
		 ...
		 <?
		 $sql="select distinct(camp) as camp from condxofyear_so where yearcheck = '$nPrefix' order by camp";
...
if($_POST["act"]=="show"){
	...
	$result="select * from condxofyear_so where yearcheck='$nPrefix' and camp='$_POST[camp]' group by hn";
	}
...
while($chkrows=mysql_fetch_array($object)){
	...
	$result1="select * from condxofyear_so where yearcheck='$nPrefix' and camp='$_POST[camp]' group by hn";
	}
...
<?
	if($_POST["camp"]=="all"){
		$showcamp="ทุกหน่วย";
		$sql="SELECT * FROM condxofyear_so where yearcheck = '$nPrefix' group by hn order by camp asc, age desc";
	}else{
		$showcamp=substr($_POST["camp"],4);
		$sql="SELECT * FROM condxofyear_so where yearcheck = '$nPrefix' AND camp = '$_POST[camp]' group by hn order by camp asc, age desc";	
	}
	//echo $sql;
	$query=mysql_query($sql);
