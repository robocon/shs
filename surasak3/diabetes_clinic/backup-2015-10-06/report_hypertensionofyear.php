...
$m=date('m');
	...
	$tbsql="SELECT * FROM `hypertension_clinic` WHERE thidate between '2013-10-01' and '2014-09-30' GROUP BY hn ORDER BY joint_disease DESC";
	//echo $tbsql;
	$tbquery=mysql_query($tbsql);
	$tbnum=mysql_num_rows($tbquery);
?> 
<p align="center"><strong>รายงานผู้ป่วย HT ประจำปีงบประมาณ 2557</strong></p>
...
?>
  <tr>
	...
	<?
	if($tbrows["joint_disease_dm"]=="Y" || $tbrows["joint_disease_nephritic"]=="Y" || $tbrows["joint_disease_myocardial"]=="Y" || $tbrows["joint_disease_paralysis"]=="Y"){
		$sql="SELECT thidate, bp1, bp2 FROM opd WHERE hn = '".$tbrows["hn"]."' AND (bp1 !='' OR bp2 !='') AND thidate between '2556-10-01 00:00:00' and '2557-09-30 23:59:59' ORDER  BY thidate DESC LIMIT 3";
		//echo $sql;
    ...
    <td align="center" bgcolor="#CCFFCC">
    <?
		$sql="SELECT thidate, bp1, bp2, organ FROM opd WHERE hn = '".$tbrows["hn"]."' AND (bp1 !='' OR bp2 !='') AND organ like '%ตรวจตามนัด%'   AND thidate between '2556-10-01 00:00:00' and '2557-09-30 23:59:59' ORDER  BY thidate DESC LIMIT 3";
		$query=mysql_query($sql);
