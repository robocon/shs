...
if ( $page == 'search' ) {
    ...
    $sql = "SELECT * FROM `appoint` WHERE `hn` = '$hn' AND ( `appdate_en` IS NOT NULL AND `appdate_en` >= '$curr_date' ) ORDER BY `row_id` DESC ";
    $db->select($sql);
    $rows = $db->get_rows();
        ...
        <table class="chk_table">
            ...
            foreach ($items as $key => $item) {
                ...
                <tr>
                    ...
                    <td><?=$item['detail'];?></td>
                    <td><?=$item['doctor'];?></td>
                    <td><a href="appoint_edit.php?page=form&id=<?=$item['row_id'];?>">แก้ไข LAB/XRAY</a></td>
					<td><a href="appoint_update.php?row_id=<?=$item['row_id'];?>" target="_blank">แก้ไขรายละเอียด</a></td>
                </tr>
                <?php
            }
            ?>
