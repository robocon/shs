...
if($_GET["action"] == "add"){
	...
	if($count_drug==0){
		...
		if($part == "DPY"){
						...
						$update = ",dpy=dpy+".($freepri * $_POST["amount"]);
						$update .= ",dpn=dpn+".(($salepri - $freepri)*$_POST["amount"]);
		...
		if($result1==true && $result2==true && $result3 == true){
			echo "บันทึกข้อมูลเรียบร้อยแล้ว<BR><A HREF=\"drxadddrug.php?sDate=".urlencode($_GET["sDate"])."&nRow_id=".urlencode($_GET["nRow_id"])."\">เพิ่มยา</A><BR>หากไม่เพิ่มยาแล้วให้ปิดหน้านี้เลยครับ
			<SCRIPT LANGUAGE=\"JavaScript\">
				
				opener.location.reload();
				setTimeout(\"window.location.href='drxadddrug.php?sDate=".urlencode($_GET["sDate"])."&nRow_id=".urlencode($_GET["nRow_id"])."';\",5000);
...
<TR>
	<TD>วิธีใช้</TD>
	<TD><INPUT TYPE="text" NAME="slcode"></TD>
</TR>
<TR>
...
$i=1;
    while (list ($drugcode, $tradname,$genname,$salepri,$stock,$mainstk) = mysql_fetch_row ($result)) {
        print (" <FORM name= \"form".$i."\" METHOD=POST ACTION=\"drxadddrug.php?action=add&sDate=".urlencode($_GET["sDate"])."&nRow_id=".urlencode($_GET["nRow_id"])."\"><tr>\n".
