...
$hn_now = $_GET['hn_now'];
$sql = "Select date_format(orderdate,'%Y-%m-%d') From resulthead where hn = '$hn_now' AND labnumber = '$labnumber' AND orderdate like '$date_now%' Order by  `autonumber` DESC limit 0,1";
...
<body>
	...
	$select_date = "";
	$select_date .= "";
	$sql = "Select distinct date_format(orderdate,'%d-%m-') as orderdate1, date_format(orderdate,'%Y') as orderdate3, date_format(orderdate,'%Y-%m-%d') as orderdate2  
	From resulthead 
	where hn='$hn_now' 
	...
	$sql = "Select a.profilecode, count(b.labcode), a.testgroupname From (Select * From resulthead where hn='$hn_now' AND labnumber = '$labnumber' AND orderdate like '" . $date_now . "%' ) as a INNER JOIN resultdetail as b ON a.autonumber = b.autonumber group by a.profilecode  Order by  a.testgroupname ASC";
	$result = mysql_query($sql);
					...
					<TR>
						<TD valign="top">
							...
							foreach ($list_code_lab as $key => $value) {
								$r = 4;
								$sql = "Select autonumber, profilecode,clinicalinfo, testgroupname From resulthead where hn='$hn_now' AND labnumber = '$labnumber' AND orderdate like '" . $date_now . "%' AND profilecode = '" . $key . "'  Order by autonumber DESC limit 0,1";
								$result = mysql_query($sql);
								while (list($autonumber, $profilecode, $clinicalinfo, $testgroupname) = mysql_fetch_row($result)) {
									...
									$sql = "Select *, date_format(authorisedate,'%d-%m-%Y') as authorisedate2, date_format(authorisedate,'%H:%i:%s') as authorise_time From resultdetail where autonumber = '" . $autonumber . "' ";
									$result2 = mysql_query($sql);

									while ($arr2 = mysql_fetch_assoc($result2)) {

										if ($arr2["flag"] != 'N') $fontbgcolor = "red";
										else $fontbgcolor = "#000000";
										if ($i % 2 == 0)
											$bgcolor = "#FFFFBB";
										else
											$bgcolor = "#FFFFFF";
										...
										?>
											<TR bgcolor="<?php echo $bgcolor; ?>">
												<TD>
													<FONT COLOR="#0035D5"><B><?php echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", $arr2["labname"]; ?></B></FONT>
												</TD>
												<TD align="center">
													<FONT COLOR="<?php echo $fontbgcolor; ?>"><B><?php echo $arr2["result"]; ?></B></FONT>
												</TD>
												<TD align="left">
													<FONT COLOR="red"><B><?php if ($arr2["flag"] != 'N') { echo "[", $arr2["flag"], "]"; }; ?></B></FONT>
												</TD>
										...
										$authoriseTime = $arr2['authorise_time'];
...
<?php echo "<CENTER><B>Authorise name : </B>" . $authorisename . "&nbsp;&nbsp;&nbsp;<B>Authorise date :</B> " . $authorisedate . " ($authoriseTime)</CENTER>"; ?>
</body>
</html>