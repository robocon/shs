...
<table>
 <tr>
  ...
  <th bgcolor=6495ED><font size='2'>แพทย์</th>
...
$op = "";
if($_POST['rip']=="ค่ายา"){ $op = " and detail='ค่ายา' ";}
elseif($_POST['rip']=="ค่าหัตถการ"){ $op = " and detail!='ค่ายา' ";}

 $query = "SELECT * FROM opacc WHERE   date LIKE '$today%' and credit = '$doctor'  and price>$money $op ORDER  BY row_id ";
...
for ($n=$x; $n>=1; $n--){
	...
	if($aDetail[$n]=="ค่ายา"){
		$query = "SELECT * FROM phardep WHERE  date LIKE '$txDate[$n]%'";
		$result = mysql_query($query) or die("Query failed1");
		
		$row = mysql_fetch_array($result);
		$doctor = $row['doctor'];
		$cPtname[$n]=$row['ptname']; 
		
	}else{
		$query = "SELECT * FROM depart WHERE  date LIKE '$txDate[$n]%'";
...
$aDate[$n]=substr($aDate[$n],8,2)."-".substr($aDate[$n],5,2)."-".substr($aDate[$n],0,4)." ".substr($aDate[$n],11);
	print("<tr>\n".
	"<td bgcolor=F5DEB3><font face='Angsana New'>$num</td>\n".
	"<td bgcolor=F5DEB3><font face='Angsana New'>$aDate[$n]</td>\n".
	"<td bgcolor=F5DEB3><font face='Angsana New'><a href='chkmonylimit2.php?d=$txDate[$n]&li=$aDetail[$n]&n=$n' target='_blank'>$aHn[$n]</a></td>\n".
	...
	"<td bgcolor=F5DEB3 align='right'><font face='Angsana New' size='3px;'><strong>$aPaid[$n]</strong></td>\n".  
...
//	"<td bgcolor=F5DEB3 align='right'><font face='Angsana New'>$aCredit_d[$n]</td>\n".  
	...
	"<td bgcolor=F5DEB3 align='right'><font face='Angsana New'>$doctor</td>\n". 
