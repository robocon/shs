...
$numcscd=0;
$cscd="จ่ายตรง";
$where="AND ((depart='PHAR' and price='1601.00') || (depart='PHAR' and price='517.00') || (depart='PHAR' and price='1381.00') || (depart='PHAR' and price='5011.00') || (depart='PHAR' and price='6199.00')  || (depart='PHAR' and price='6111.00'))";

    $query="CREATE TEMPORARY TABLE reportcscd01 SELECT date,hn,vn,billno,price,credit,depart,paidcscd,detail,row_id,txdate FROM opacc WHERE date LIKE '$thimonth%' and credit = '$cscd'  AND paidcscd > $numcscd $where" ;
	//echo $query;
     $result = mysql_query($query) or die("Query failed billtran 26");


     $query="SELECT * FROM reportcscd01";
     $result = mysql_query($query) or die("Query reportcscd01, Line 32 failed");
   ...
   $result = mysql_query($query);
    while (list ($date,$hn,$vn,$billno,$price,$cerdit,$depart,$paidcscd,$detail,$row_id,$txdate) = mysql_fetch_row ($result)) {	
		...
		if($depart=='PHAR'){
			...
			if($num2 > 0){  //เช็คว่าในตาราง drugrx มี idno ที่เป็น row_id ของตาราง phardep หรือไม่
			while($result2 = mysql_fetch_array($row2)){
				if($result2['part']=="DDL"||$result2['part']=="DDY"){
					$ddl+=$result2['suma'];
			...
			$sql1 = "select * from dphardep_pt where date = '".$txdate."' and hn='$hn' ";
			$row1 = mysql_query($sql1);
			$result1 = mysql_fetch_array($row1);
			
			$sql2 = "select sum(price) as suma,part,drugcode,DPY,DPN,amount  from ddrugrx_pt where idno = '".$result1['row_id']."' group by part";
			$row2 = mysql_query($sql2);
			$num2= mysql_num_rows($rows2);
			while($result2 = mysql_fetch_array($row2)){
				if($result2['part']=="DDL"||$result2['part']=="DDY"){
...
$where="AND ((depart='PHAR' and price='1601.00') || (depart='PHAR' and price='517.00') || (depart='PHAR' and price='1381.00') || (depart='PHAR' and price='5011.00') || (depart='PHAR' and price='6199.00')  || (depart='PHAR' and price='6111.00'))";
    ...
    $query="CREATE TEMPORARY TABLE reportcscd02 SELECT date,hn,vn,billno,price,credit,depart,paidcscd,detail,row_id,txdate FROM opacc WHERE date LIKE '$thimonth%' and credit = '$cscd' $where" ;
	//echo $query;
	
    $result = mysql_query($query) or die("Query failed billdisp 461");


     $query="SELECT * FROM reportcscd02";
     $result = mysql_query($query) or die("Query reportcscd02, Line 467 failed");
...
<Dispensing>\r\n";
   ...
   $result = mysql_query($query);
    while (list ($date,$hn,$vn,$billno,$price,$cerdit,$depart,$paidcscd,$detail,$row_id,$txdate) = mysql_fetch_row ($result)) {	
	//echo "1) ".$date."-->".$hn."-->".$vn."-->".$billno."-->".$price."-->".$cerdit."-->".$depart."-->".$paidcscd."-->".$detail."-->".$row_id."-->".$txdate."<br>";
	$numcscd++;
	$pitem=0; 
	$num1=11512;
	$num2=543;
...
//-------------------- Add to zip --------------------//
	...
	echo "<p style='color:#FF0000; font-weight:bold;'>โปรแกรมส่งเบิกข้อมูลค่าอุปกรณ์ CSCD ของหมอธนบดินทร์</p>";
	echo "ดาวน์โหลดข้อมูลส่งเบิกอุปกรณ์ CSCD... <a href=$ZipName>คลิกที่นี่</a> <br>";
	echo "<a href='../exportcscdpt_data.php'><< กลับหน้าเดิม</a>";	
//-------------------- Close add to zip --------------------//
