	...
	if($idno==""){
		...
		for($r=0;$r<count($_SESSION['idnumber']);$r++){
			...
			if($nrow>0){
				...
				$query = "SELECT * FROM dphardep_pt WHERE row_id = '".$_SESSION['idnumber'][$r]."' and hn='$nhn' and tvn = '$sVn' ";
				//echo $query;
...
for($r=0;$r<count($_SESSION['idnumber']);$r++){
		...
		$query = "SELECT * FROM dphardep_pt WHERE row_id = '".$_SESSION['idnumber'][$r]."' and hn='$sHn' and tvn = '".$_SESSION["sVn"]."'  ";
		//echo $query;
		$result = mysql_query($query) or die(mysql_error());
	...
	for($r=0;$r<count($_SESSION['idnumber']);$r++){
			...
			$query = "SELECT a.drugcode,a.tradname,a.amount,a.price,a.part FROM ddrugrx_pt as a,dphardep_pt as b WHERE a.idno = '".$_SESSION['idnumber'][$r]."' and a.hn = '$sHn' and b.tvn='".$_SESSION["sVn"]."' AND a.idno = b.row_id ";
...
//echo $query;
	...
	//        $x++;
				$items++;
				$drugs = 4;
				$sql="select dpy_code from druglst_pt where drugcode like '%$drugcode%'";
...
print "<form name='f2' method='POST' action='opbill3_pt.php' Onsubmit='return checkformf2()'>";
