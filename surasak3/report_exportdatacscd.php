<?
session_start();
include("connect.inc");
?>
<style type="text/css">
<!--
body,td,th {
	font-family: TH SarabunPSK;
	font-size: 20px;
}
.txt{
	font-family: TH SarabunPSK;
	font-size: 20px;
}
-->
</style>
  <?
  $seldate=date("d");
  $selmon=date("m");
  ?>
<form method="POST" action="report_exportdatacscd.php">
<input type="hidden" name="act" value="show">
<p><strong>แสดงข้อมูลที่เตรียมส่งเบิกค่าชดเชยทางการแพทย์ผู้ป่วยนอก สิทธิเบิกจ่ายตรง (CSOP)<br />
ผู้พัฒนาระบบ&nbsp;&nbsp;&nbsp;ส.ท. เทวิน ศรีแก้ว เจ้าหน้าที่ศูนย์บริการคอมพิวเตอร์&nbsp;&nbsp;&nbsp;โทร. 8500<br />
<div style="color:#FF0000">เริ่มใช้ตั้งแต่วันที่ 28 เดือนมีนาคม พ.ศ.2562 เป็นต้นไป</div></strong>
</p>
<p><a href="update_exportdatacscd.php">โปรแกรมอัพเดทสถานะ ICD10 เพื่อส่งเบิก สกส.</a></p>
  <strong>ข้อมูลประจำวันที่ : </strong>
  <select name="rptdate" class="txt" id="rptdate">
  <?
  for($i=1;$i<=31;$i++){
	  if($i < 10){
	  	$dd="0".$i;
	  }else{
	  	$dd=$i;
	  }
  ?>
    <option value="<?=$dd;?>" <? if($seldate==$dd){ echo "selected='selected'";}?>><?=$dd;?></option>
  <?
  }
  ?>
  </select>
  &nbsp; 

  <select size="1" name="rptmo" class="txt">
    <option selected>-------เลือก-------</option>
    <option value="01" <? if($selmon=="01"){ echo "selected='selected'";}?>>มกราคม</option>
    <option value="02" <? if($selmon=="02"){ echo "selected='selected'";}?>>กุมภาพันธ์</option>
    <option value="03" <? if($selmon=="03"){ echo "selected='selected'";}?>>มีนาคม</option>
    <option value="04" <? if($selmon=="04"){ echo "selected='selected'";}?>>เมษายน</option>
    <option value="05" <? if($selmon=="05"){ echo "selected='selected'";}?>>พฤษภาคม</option>
    <option value="06" <? if($selmon=="06"){ echo "selected='selected'";}?>>มิถุนายน</option>
    <option value="07" <? if($selmon=="07"){ echo "selected='selected'";}?>>กรกฎาคม</option>
    <option value="08" <? if($selmon=="08"){ echo "selected='selected'";}?>>สิงหาคม</option>
    <option value="09" <? if($selmon=="09"){ echo "selected='selected'";}?>>กันยายน</option>
    <option value="10" <? if($selmon=="10"){ echo "selected='selected'";}?>>ตุลาคม</option>
    <option value="11" <? if($selmon=="11"){ echo "selected='selected'";}?>>พฤศจิกายน</option>
    <option value="12" <? if($selmon=="12"){ echo "selected='selected'";}?>>ธันวาคม</option>

  </select>
  <? 
			   $Y=date("Y")+543;
			   $date=date("Y")+543+5;
			  
				$dates=range(2547,$date);
				echo "<select name='thiyr'  class='txt'>";
				foreach($dates as $i){

				?>
      
      <option value='<?=$i?>' <? if($Y==$i){ echo "selected"; }?>><?=$i;?></option>
      <?
				}
				echo "<select>";
				?>
      <p style="margin-left: 65px;"><input type="submit" value="แสดงข้อมูล" name="B1"  class="txt" />&nbsp;&nbsp;&nbsp;<input type="button" value="กลับหน้าหลัก" onclick="window.location.href='../nindex.htm' " class="txt" /></p>
</form>
<hr>
<?
if($_POST["act"]=="show"){
$thiyr=$_POST["thiyr"];
$yrmonth=$_POST["rptmo"];
$yrdate=$_POST["rptdate"];
$yrmonthdate="$thiyr-$yrmonth-$yrdate";
$showdate="$yrdate/$yrmonth/$thiyr";
?>
<div align="center"><strong>ข้อมูลที่ยังไม่ได้ให้รหัสโรค ICD10 ประจำวันที่ <?=$showdate;?></strong></div>
<table width="98%" border="1" align="center" cellpadding="2" cellspacing="0" bordercolor="#000000">
  <tr>
    <td width="5%" align="center"><strong>ลำดับ</strong></td>
    <td width="10%" align="center"><strong>HN</strong></td>
    <td width="30%" align="center"><strong>ชื่อ - นามสกุล</strong></td>
    <td width="15%" align="center"><strong>Approve Code</strong></td>
	<td width="15%" align="center"><strong>Billno</strong></td>
	<td width="10%" align="center"><strong>จำนวนเงิน</strong></td>
    <td width="10%" align="center"><strong>ขอเบิก</strong></td>
  </tr>
<?
$sql="select hn,sum(price) as price, sum(paidcscd) as paidcscd,credit_detail,billno from opacc where txdate LIKE '$yrmonthdate%' AND credit ='จ่ายตรง' and icd10_cscd='' group by credit_detail,hn";
$query=mysql_query($sql);
$i=0;
$num=mysql_num_rows($query);
if($num < 1){
echo "<tr><td colspan='5' align='center'>------------------------------------- ไม่มีข้อมูล -------------------------------------</td></tr>";
}
while ($rows=mysql_fetch_array($query)) {	  //วนลูป
$i++;
$sql2="select * from opcard where hn='$rows[hn]'";
//echo $sql2."<br>";
$query2=mysql_query($sql2);
$result2=mysql_fetch_array($query2);
$ptname=$result2["yot"]." ".$result2["name"]."&nbsp;&nbsp;".$result2["surname"];
?>
  <tr>
    <td align="center"><?=$i;?></td>
    <td align="center"><?=$rows["hn"];?></td>
    <td><?=$ptname;?></td>
	<td align="center"><?=$rows["credit_detail"];?></td>  
    <td align="center"><?=$rows["billno"];?></td>  
	<td align="right"><?=$rows["price"];?></td>  
    <td align="right"><?=$rows["paidcscd"];?></td>  
  </tr>
<?
}
?>  
</table>
<?
}
?>

