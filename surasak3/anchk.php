  ...
  if (!empty($an)) {
    ...
    while (list($an, $hn, $ptname, $ptright, $date, $dcdate, $diag, $doctor, $bedcode, $fname) = mysql_fetch_row($result)) {
      ...
      print (" <tr>\n" .
        ...
        "  <td BGCOLOR=F5DEB3 align=\"center\"><FORM METHOD=POST ACTION=\"uploadfilean.php\" enctype=\"multipart/form-data\" >
<INPUT TYPE=\"file\" NAME=\"upload_file\"><BR><INPUT TYPE=\"submit\" value=\"ตกลง\"><INPUT TYPE=\"hidden\" name=\"an\" value=\"" . $an . "\"><BR><A HREF=\"" . $fname
        . "\" target=\"_blank\">$fname ดูข้อมูล</A>
</FORM></td>\n" .
        " </tr>\n");
    }
    ?>
  ...
  <table>
    ...
    <?php
    $query = "SELECT date,status,office,an FROM dcstatus WHERE an = '$findAn'  order by date desc";
    ...
    if($rows>0){
      ...
      ?>
      <tr><td colspan="4"><strong>ไม่พบประวัติผู้ป่วยใน</strong></td></tr>
      <?php
    }
  ?>
  </table>
