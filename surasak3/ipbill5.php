...
*/
    if ($billno == '' ){print "ไม่มีเลขที่ใบเสร็จ";}
		...
		include("connect.inc");
       $query = "INSERT INTO ipmonrep(date,admit,dcdate,days,an,hn,ptname,ptright,price,paid,debt,cash,idname,bfy,bfn,dpy,dpn,ddl,ddy,ddn,dsy,dsn,blood,lab,xray,sinv,surg,ncare,denta,pt,stx,mc,billno,credit,credit_detail,tool)VALUES('$Thidate','$cAdmit','$cDcdate','$cDays','$cAn','$cHn','$cPtname','$cPtright','$Netpri','$Netpaid','$debt','$paid','$sOfficer','$BFY','$BFN','$DPY','$DPN','$Essd','$Nessdy','$Nessdn','$DSY','$DSN','".($Blood+$Bloody+$Bloodn)."','".($Labo+$Laboy+$Labon)."','".($Xray+$Xrayy+$Xrayn)."','".($Sinv+$Sinvy+$Sinvn)."','".($Surg+$Surgy+$Surgn)."','".($Ncare+$Ncarey+$Ncaren)."','".($Dent+$Denty+$Dentn)."','".($Physi+$Physiy+$Physin)."','".($Stx+$Stxy+$Stxn)."','".($Mc+$Mcy+$Mcn)."','$billno','$credit','$detail_1','".($Tool+$Tooly+$Tooln)."');";
 
       $result = mysql_query($query) or die("Query failed,insert into ipmonrep");
//       echo mysql_errno() . ": " . mysql_error(). "\n";
//       echo "<br>";

       $sql = "UPDATE ipcard SET price='$Netpri', paid= $Netpaid+$paid, calc='$Thidate' WHERE an='$cAn' ";
...
//พิมพ์ใบเสร็จ
  ...
  //$sql = "UPDATE ipacc SET paid=price,billno='$billno' WHERE an='$cAn' and (paid is null or paid = 0.00) and billno is null  ";
  $name_select = implode(' or row_id=',$_SESSION['idacc']);
  $sql = "UPDATE ipacc SET paid=price,billno='$billno' WHERE row_id = $name_select ";
...
print "      <td width='30%'><font face='Angsana New'>$cAdmitALL</td>";
...
$dstart = $dd."-".$mm."-".$yy;
$dlast = $dd2."-".$mm2."-".$yy2;
print "      <td width='100%' colspan=3 ><font face='Angsana New'>$cDiag (ค่ารักษาพยาบาลระหว่างวันที่ $dstart ถึงวันที่ $dlast)</td>";
/*print "      <td width='0%'><font face='Angsana New'>";
print "      <td width='0%'><font face='Angsana New'>";
print "      <td width='0%'><font face='Angsana New'>";*/
print "       </font></td>";
print "    </tr>";
...
//print "      <td width='55%'><font face='Angsana New'>สรุปค่ารักษาพยาบาล:<br>";
//print "รายการ<br>";
print "<td width='50%'><font face='Angsana New' size='3'>1. ค่าห้อง/ค่าอาหาร<br>";	
print "   .......ค่าห้อง/ค่าอาหาร(ส่วนเกิน)<br>";	
print "2. อวัยวะเทียม/อุปกรณ์ในการบำบัดรักษา<br>";	
print "3. ยาและสารอาหารทางเส้นเลือดที่ใช้ในโรงพยาบาล<br>";
print "4. ยาที่นำไปใช้ต่อที่บ้าน<br>	";
...
//print "      <td width='24%'><font face='Angsana New'>รายการ<br>";
//print "        เบิกไม่ได้<br>";
print "       <td width='10%' align='right'> <font face='Angsana New' size='3'>...<br>";
print "        $BFN<br>";
print "        $DPN<br>";
print "        $Nessdn<br>";
print "        $DNessdn<br>"; //4. ยาที่นำไปใช้ต่อที่บ้าน เบิกไม่ได้
...
//print "        ...<br>";
print "        ...</font></td>";
print "       <td width='10%' valign='middle' align='right'><font face='Angsana New' size='3'><br>";
print "        <br>";
print "        <br>";
print "        <br>";
...
print "      <td width='9%'></td>";
print "      <td width='69%'>";
print "        <p><font face='Angsana New' size='3'>1. ค่าห้องค่าอาหาร&nbsp;</font></td>";
print "      <td width='11%' valign='bottom'><font face='Angsana New' size='3'>$BFY</font></td>";
print "      <td width='11%' valign='bottom'><font face='Angsana New' size='3'>$BFN</font></td>";
print "    </tr>";
print "    <tr>";
print "      <td width='9%'></td>";
print "      <td width='69%'><font face='Angsana New' size='3'>2. &#3588;&#3656;&#3634;&#3605;&#3619;&#3623;&#3592; &#3588;&#3656;&#3634;&#3623;&#3636;&#3648;&#3588;&#3619;&#3634;&#3632;&#3627;&#3660;&#3650;&#3619;&#3588;</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$invlab</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'></td>";
print "    </tr>";
print "    <tr>";
print "      <td width='9%'></td>";
print "      <td width='69%'><font face='Angsana New' size='3'>3. &#3588;&#3656;&#3634;&#3618;&#3634;(&#3651;&#3609;&#3610;&#3633;&#3597;&#3594;&#3637;&#3618;&#3634;&#3627;&#3621;&#3633;&#3585;&#3649;&#3627;&#3656;&#3591;&#3594;&#3634;&#3605;&#3636;)</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$BEssd</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'></td>";
print "    </tr>";
print "    <tr>";
print "      <td width='9%'></td>";
print "      <td width='69%'><font face='Angsana New' size='3'>4. &#3588;&#3656;&#3634;&#3618;&#3634;(&#3609;&#3629;&#3585;&#3610;&#3633;&#3597;&#3594;&#3637;&#3618;&#3634;&#3627;&#3621;&#3633;&#3585;&#3649;&#3627;&#3656;&#3591;&#3594;&#3634;&#3605;&#3636;)</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$BNessdy</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$BNessdn</font></td>";
print "    </tr>";
print "    <tr>";
print "      <td width='9%'></td>";
print "      <td width='56%'><font face='Angsana New' size='3'>5. &#3588;&#3656;&#3634;&#3648;&#3623;&#3594;&#3616;&#3633;&#3603;&#3601;&#3660;&#3607;&#3637;&#3656;&#3652;&#3617;&#3656;&#3651;&#3594;&#3656;&#3618;&#3634;</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$DSY</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$DSN</font></td>";
print "    </tr>";
print "    <tr>";
print "      <td width='9%'></td>";
print "      <td width='69%'><font face='Angsana New' size='3'>6. &#3588;&#3656;&#3634;&#3629;&#3640;&#3611;&#3585;&#3619;&#3603;&#3660;/&#3648;&#3588;&#3619;&#3639;&#3656;&#3629;&#3591;&#3617;&#3639;&#3629;&#3607;&#3634;&#3591;&#3585;&#3634;&#3619;&#3649;&#3614;&#3607;&#3618;&#3660;</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$equip</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$DPN</font></td>";
print "    </tr>";
print "    <tr>";
print "      <td width='9%'></td>";
print "      <td width='69%'><font face='Angsana New' size='3'>7. &#3588;&#3656;&#3634;&#3610;&#3619;&#3636;&#3585;&#3634;&#3619;&#3607;&#3634;&#3591;&#3585;&#3634;&#3619;&#3649;&#3614;&#3607;&#3618;&#3660;</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$medcare</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'></td>";
print "    </tr>";
print "    <tr>";
print "      <td width='9%'></td>";
print "      <td width='69%'><font face='Angsana New' size='3'>8. &#3588;&#3656;&#3634;&#3610;&#3619;&#3636;&#3585;&#3634;&#3619;&#3629;&#3639;&#3656;&#3609;&#3607;&#3637;&#3656;&#3652;&#3617;&#3656;&#3648;&#3585;&#3637;&#3656;&#3618;&#3623;&#3586;&#3657;&#3629;&#3591;&#3585;&#3633;&#3610;&#3585;&#3634;&#3619;&#3619;&#3633;&#3585;&#3625;&#3634;</font></td>";
print "      <td width='11%'><font face='Angsana New' size='3'></td>";
print "      <td width='11%'><font face='Angsana New' size='3'>$Mc</font></td>";
print "    </tr>";
...
print "      <td width='10%'></td>";
print "      <td width='20%'></td>";
print "      <td width='10%' align='right'><font face='Angsana New' size='3'>$Nsubtotal</font></td>";
print "      <td width='18%' align='right'><font face='Angsana New' size='3'>$Ysubtotal</font></td>";
print "      <td width='10%'></td>";
print "    </tr>";
print "    <tr>";
print "      <td width='5%'></td>";
print "      <td width='50%'><font face='Angsana New' size='3'><B>$cbaht</B></font></td>";
print "      <td width='5%'></td>";
print "      <td width='10%'><font face='Angsana New' size='4'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<B>$paid</B></font></td>";
print "      <td width='5%'></td>";
print "    </tr>";
print "  </table>";
print "</div>";
