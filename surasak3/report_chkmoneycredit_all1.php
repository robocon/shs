...
if($credit=="all"){
	$query = "SELECT *,sum(paid) as sumpaid FROM opacc  WHERE (date >= '$chkdate1' and date <= '$chkdate2') and ((credit = 'ยกเลิก' and billno like '%/%') OR credit='เงินสด'  OR credit='เงินโอน' OR credit='กรุงไทย'  OR credit='เช็ค') group by date, hn, billno ORDER  BY substring(date,1,10) asc, billno asc";
}else if($credit=="ยกเลิก"){
	$query = "SELECT *,sum(paid) as sumpaid FROM opacc WHERE (date >= '$chkdate1' and date <= '$chkdate2') and (credit = 'ยกเลิก' AND LENGTH(billno) > 6) group by date, hn, billno ORDER  BY substring(date,1,10) asc, billno asc";
}else{
	$query = "SELECT *,sum(paid) as sumpaid FROM opacc  WHERE (date >= '$chkdate1' and date <= '$chkdate2') and credit='$credit' group by date, hn, billno ORDER  BY substring(date,1,10) asc, billno asc";
}	
//echo $query;
