...
<HTML>
 <HEAD>
  <TITLE>ระบบลงทะเบียนอัตโนมัติ โรงพยาบาลค่ายสุรศักดิ์มนตรี</TITLE>
 </HEAD>
...
include("connect.inc");
  ...
  if(!isset($_SESSION["case_service"]) || $_SESSION["case_service"] == ""){
		...
		$_SESSION["case_service"] = "EX01 รักษาโรคทั่วไปในเวลาราชการ";
  ...
  if(isset($_GET["type_value"]) && $_GET["type_value"] != ""){
		...
		switch($_GET["type_value"]){
			...
			case "EX11":
				$_SESSION["case_service"] = "EX11 รักษาโรคนอกเวลาราชการ";
			break;
		}
...
case 3:
		...
		$thdatehn=$d.'-'.$m.'-'.$yr.$_POST["cHn"];
		$today = date("Y-m-d");
		$code21 = '21'; 
		$thdatevn=$d.'-'.$m.'-'.$yr.$vn;
		$case = $_SESSION["case_service"];

		$sql = "Select * From opcard where hn = '".$_POST["cHn"]."' limit 0,1 ";
		...
		if($rows > 0){
			...
			$arr = Mysql_fetch_assoc($result);
				...
				$sql = "INSERT INTO opday(thidate,thdatehn,hn,vn,thdatevn,ptname,
                	     ptright,goup,camp,note,idcard,toborow,borow,dxgroup,officer,phaok,time1,kew)VALUES('$thidate','$thdatehn','".$_POST["cHn"]."','$vn','$thdatevn','$cPtname','$cPtright','".$arr_member["goup"]."','".$arr_member["camp"]."',Null,'".$arr_member["idcard"]."','$case','$borow','$code21','".$_SESSION["sOfficer"]."','X','".date("H:i:s")."','".$kew."');";
				$result = Mysql_Query($sql);
////////////คิดเงิน 50 บาท
		...
		print "<center><font style=\"font-size: 45px\"><b>โรงพยาบาลค่ายสุรศักดิ์มนตรี </b><br> ";
		print "<center><font style=\"font-size: 30px\"><b>ผู้ป่วยตรวจโรคทั่วไป </b><br> ";
		...
		print "<center><font style=\"font-size: 30px\">$cPtname<br>"; 
		print "<center><font style=\"font-size: 40px\"color=\"red\"><b>รอรับบริการที่จุดคัดแยก</b></font><br>";
...
default : 
		...
		<div id=\"check_type\" style=\"display:none\">
			<A HREF=\"".$_SERVER['PHP_SELF']."?type_value=EX01\">EX01&nbsp;รักษาโรคทั่วไปในเวลาราชการ</A><BR>
			<A HREF=\"".$_SERVER['PHP_SELF']."?type_value=EX11\">EX11&nbsp;รักษาโรคนอกเวลาราชการ</A>
		</div>
		...
		<TABLE>
		<TR>
			<TD align=\"center\"><FONT style=\"font-size:40px; \" ><B>โรงพยาบาลค่ายสุรศักดิ์มนตรี</B></FONT></TD>
		</TR>
		<TR>
			<TD align=\"center\"><FONT style=\"font-size:20px; \" ><BR>ระบบลงทะเบียนอัตโนมัติ<BR>
			</FONT><FONT style=\"font-size:25px; \" color=\"#000066\">
			<B>";
			
			if($_SESSION["case_service"] == "EX01 รักษาโรคทั่วไปในเวลาราชการ")
				echo "สำหรับตรวจโรคทั่วไปเท่านั้น";
			else if($_SESSION["case_service"] == "EX11 รักษาโรคนอกเวลาราชการ")
				echo "ตรวจโรคทั่วไปนอกเวลาราชการ";
			
			echo "</B></FONT></TD>
		</TR>
		<TR>
			<TD align=\"center\"><BR><INPUT TYPE=\"text\" NAME=\"id_barcode\" maxlength=\"13\" size=\"70\"></TD>
		</TR>
		<TR>
			<TD align=\"center\"><FONT style=\"font-size:20px; \" ><BR>กรุณารูดบัตรที่มีบาร์โค้ด เครื่องจะลงทะเบียนให้อัตโนมัติ</FONT></TD>
		</TR>
		<TR>
			<TD align=\"center\"><FONT style=\"font-size:20px; \" color=\"red\"><BR>
				<B>* ผู้ที่ต้องการตรวจ ทันตกรรม, ฝังเข็ม,สูติ, มวลกระดูก<BR>
				กรุณาติดต่อที่ฝ่ายทะเบียน</B>
			</FONT></TD>
		</TR>
		</TABLE>
		</TD>
