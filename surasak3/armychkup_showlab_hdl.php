...
<table width="98%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;">
  ...
  <?
include("connect.inc");  
	$sql = "select * from armychkup where yearchkup='61' and camp !='' and age >=35 and hdl_lab ='' order by age desc";	
		//echo $sql;
		...
		$labquery = "select * from resulthead as a inner join resultdetail as b on a.autonumber=b.autonumber where b.labcode='HDL' AND a.hn ='".$result["hn"]."' and a.clinicalinfo='ตรวจสุขภาพประจำปี61'";
		//echo $labquery;
		...
		if($flaglab=="N" || $flaglab=="H"){		//ปกติ หรือ สูง จะดี
			$hdllab="ปกติ";
		}else if($flaglab=="L"){  //ต่ำ ไม่ดี
			$hdllab="ผิดปกติ";
		}else{
			$hdllab="";
		}
		$update="update armychkup set hdl_result='$resultlab',
														  hdl_flag='$flaglab',
														  hdl_lab='$hdllab' 
						where row_id='$rowid';";
		echo $update."<br>";		
