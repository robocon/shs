...
If (!empty($hn)){
    ...
    while (list ($hn,$yot,$name,$surname,$dbirth,$ptright,$note,$idguard) = mysql_fetch_row ($result)) {
        print (" <tr>\n".
           "  <td BGCOLOR=F5DEB3><a   href=\"preappoi1.php?cHn=$hn&cYot=".rawurlencode($yot)."&cName=".rawurlencode($name)."&cSurname=".rawurlencode($surname)."&Age=$dbirth&ptright=".rawurlencode($ptright)."&note=".rawurlencode($note)."&idguard=".rawurlencode($idguard)."\">$hn</a></td>\n".
