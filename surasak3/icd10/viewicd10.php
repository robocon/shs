...
$thidate=$_REQUEST['date'];
	...
	if($_REQUEST['list']=="all"){
		...
		$status="รายการที่ ไม่มี ICD10";
	
		  $select="SELECT  *  FROM opday  WHERE thidate  LIKE  '$thidate%' and (icd10 ='' OR icd10 is NULL)  ";


}elseif($_REQUEST['list']=="notnull"){
	
			$status="รายการที่มี ICD10";
...
<h1 class='forntsarabun'> แสดงรายการ Icd10 ตาม <?=$status;?> <input type=button value='กลับ' onClick="JavaScript:history.back();">
</h1> 
