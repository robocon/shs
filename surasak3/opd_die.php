...
<TR>
	<TD colspan="2" class="tb_head">รายชื่อคนไข้เสียชีวิต</TD>
</TR>
<TR class="tb_detail">
	<TD align="right" >ข้อมูลปี : </TD>
	<TD>
...
<TR class="tb_detail">
	...
	<TD>
		<SELECT NAME="opcard_st">
			<OPTION value="%%">ทั้งหมด</OPTION>
			<OPTION value="%เสียชีวิต">ผู้ป่วยนอก</OPTION>
			<OPTION value="%(ใน)">ผู้ป่วยใน</OPTION>
		</SELECT>
	</TD>
</TR>
<TR >
...
<?php
	if(isset($_POST["opcard_year"]) && $_POST["opcard_year"] != ""){
		...
		$sql = "Select hn,  name, surname, date_format(dbirth,'%d/%m/%Y') as dbirth2, idguard, date_format(lastupdate,'%d/%m/%Y') as lastupdate2  From opcard where left(hn,2) = '".$_POST["opcard_year"]."' AND (idguard like 'MX04%' AND idguard like '".$_POST["opcard_st"]."') Order by regisdate ASC ";
