...
<form method='POST' action='report_sur05.php'>
...
<?php
if(isset($_POST["year"])){
echo "<FONT style=\"font-size: 14px; font-family: 'MS Sans Serif';\"><CENTER>สรปุยอดผู้ป่วยแบ่งตามประเภทผู้ป่วยห้องผ่าตัดใหญ่ ปีงบประมาณ ".$_POST["year"]."</CENTER></FONT>";
...
echo "<TR>";
	echo "<TD>".$month[$j]." ".$year."</TD>";
	 foreach( $cfg_type_wounded as  $key => $value){
		 ...
		 $sql = "Select count(hn) From memo_sur where thaidate like '".$year."-".$j."%' AND type_wounded = '".$key."' AND type_case = 'TC1' ";
...
echo "<BR><BR>";
echo "<FONT style=\"font-size: 14px; font-family: 'MS Sans Serif';\"><CENTER>สรปุยอดผู้ป่วยแบ่งตามประเภทผู้ป่วยห้องผ่าตัดใหญ่ ปีพ.ศ. ".$_POST["year"]."</CENTER></FONT>";
echo "<TABLE border=\"1\" align=\"center\" border='1' bordercolor=\"#000000\" style=\"font-size: 14px; font-family: 'MS Sans Serif'; BORDER-COLLAPSE: collapse;\" cellpadding=\"2\">";
echo "<TR>";
	echo "<TD>เดือน ปี</TD>";
	 foreach( $cfg_type_wounded as  $key => $value){
