<?php
   ...
   print "<form method='POST' action='payfilm.php'>";
    print "<p><font face='Angsana New'>คิดค่าปริ้นฟิลม์ 70 บาท</font></p>";
...
<TR BGCOLOR=66CDAA>
	<TD align="center" BGCOLOR="<?php echo $color ?>"><a href="payfilm.php?chn=<?=$arr["hn"]?>"><?php echo $arr["vn"];?></a></TD>
...
elseif(isset($_GET['chn'])){
	...
	$thdatehn=$d.'-'.$m.'-'.$yr.$cHn; 
	$check = "select * from depart where hn = '".$chn."' and  detail = 'ค่าบริการพิมพ์ Film Xray' and date like '".(date("Y")+543).date("-m-d")."%' ";
	$resultcheck = mysql_query($check);
	$cal = mysql_num_rows($resultcheck);
		if($cal==0){
			$query2 = "select * from labcare where code='film' and part= 'XRAY' ";
		...
		//runno  for chktranx
			...
			list($nVn) = mysql_fetch_array($result);
				/////////////////////////////////////////////////////////////
			$query = "INSERT INTO depart(chktranx,date,ptname,hn,an,depart,item,detail,price,sumyprice,sumnprice,paid, idname,accno,tvn,ptright)VALUES('$nRunno','$thidate','$cPtname','$chn','','".$arr2['depart']."','1','".$arr2['detail']."', '".$arr2['price']."','".$arr2['yprice']."','".$arr2['nprice']."','','$sOfficer','0','$nVn','$cPtright');";
			$result = mysql_query($query);
			$idno=mysql_insert_id();
		 
			$query = "INSERT INTO patdata(date,hn,an,ptname,item,code,detail,amount,price,yprice,nprice,depart,part,idno,ptright)
VALUES('$thidate','$chn','','$cPtname','1','".$arr2['code']."','".$arr2['detail']."','1','".$arr2['price']."','".$arr2['yprice']."','".$arr2['nprice']."','".$arr2['depart']."','".$arr2['part']."','$idno','$cPtright');";
			$result = mysql_query($query) or die("Query failed,cannot insert into patdata");

			$query ="UPDATE opday SET other=(other+".$arr2['price'].") WHERE thdatehn= '$thdatehn' AND vn = '".$nVn."' ";

      		$result = mysql_query($query) or die("Query failed,update opday");
			if($result){
				echo "<font style='font-size:20px;' ><br><br>เพิ่มค่าบริการปริ้นฟิลม์เรียบร้อยแล้ว";
				echo "<br><a href='payfilm.php' >กลับหน้าแรก</a></font>";
			}
		}else{
