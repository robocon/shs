...
?>
<span class="font1">
	<?= $wardname; ?> (รายการอาหาร) &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<a target="_blank" href="ffwprn_new.php?id=<?= $lbedcode; ?>">พิมพ์รายการอาหาร</a>&nbsp;&nbsp;&nbsp;&nbsp;<a
		target="_blank"
		href="ffwprn_new2.php?id=<?= $lbedcode; ?>&typefood=<?= rawurlencode($_POST['menu']); ?>">พิมพ์บัตรอาหาร</a>
...
<table class="font1">
	...
	while (list($an, $bed, $ptname, $diagnos, $diag1, $food, $bedcode, $age, $hn, $ptright, $bedname, $bedpri, $hi_type) = mysql_fetch_row($result)) {
		...
		$viewFood = $food;
		$viewFood = str_replace('ไม่ต้องการแยกภาชนะ','',$viewFood);
		$viewFood = str_replace('ต้องการแยกภาชนะ','แยกภาชนะ',$viewFood);
		...
		print(" <tr>\n" .
			...
			"  <td BGCOLOR=#FFCC66><a target=_BLANK  href=\"food_ex.php?cAn=$an\">$viewFood</a></td>\n" .
