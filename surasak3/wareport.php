...
//      $query = "SELECT date,ptname,hn,an,code,detail,price,row_id,ptright,depart FROM patdata WHERE date LIKE '$today%' and ptright LIKE 'R07%' and depart='DENTA'  ORDER BY code ASC ";
    ...
    $query="CREATE TEMPORARY TABLE warphar SELECT date_format(date,'%d- %m- %Y'),ptname,hn,diag,price,ptright FROM phardep WHERE date LIKE '$yrmonth%' and ptright LIKE 'R07%' AND (an is Null OR an ='')";
    $result = mysql_query($query) or die("Query failed,warphar");
 //   echo mysql_errno() . ": " . mysql_error(). "\n";
 //   echo "<br>";
  print "1. รายงานค่ายาผู้ป่วยนอก ประกันสังคม เดือน $yrmonth <a target=_self  href='../nindex.htm'><<ไปเมนู</a><br> ";
    print "<table>";
    print " <tr>";
...
print " </tr>";
   ...
   $result = mysql_query($query);
    ...
    while (list ($date,$ptname,$hn,$diag,$price,$ptright) = mysql_fetch_row ($result)) {
	if($price < 0){
		$num--;
	}else{
		$num++;
    ...
    print "<table>";
$sum1[0] = "1. รายงานค่ายาผู้ป่วยนอกประกันสังคมจำนวน : ".$num." รายการ";
$sum = $sum + $num;

//wadeplab table
      print("<tr>\n".
              "<td bgcolor=F5DEB3>2. ค่าทำLABผู้ป่วยนอกประกันสังคม เดือน $yrmonth</td>\n".
              " </tr>\n");
    $query="CREATE TEMPORARY TABLE wadeplab SELECT date_format(date,'%d- %m- %Y'),ptname,hn,price,depart,ptright FROM depart WHERE date LIKE '$yrmonth%' and ptright LIKE 'R07%'  and depart='PATHO'  AND (an is Null OR an ='')";
   ...
   $result = mysql_query($query);
      $num=0;
    while (list ($date,$ptname,$hn,$price,$depart,$ptright) = mysql_fetch_row ($result)) {
	if($price < 0){
		$num--;
	}else{
		$num++;
		...
		$sum2[1] = $sum2[1]+$price;
		      ...
		      print "<table>";
$sum1[1] = "2. ค่าทำLABผู้ป่วยนอกประกันสังคมจำนวน : ".$num." รายการ";
$sum = $sum + $num;
//wadepxr table
      print("<tr>\n".
              "<td bgcolor=F5DEB3>3. ค่าทำXRAY ผู้ป่วยนอกประกันสังคม เดือน $yrmonth</td>\n".
              " </tr>\n");
    $query="CREATE TEMPORARY TABLE wadepxr SELECT date_format(date,'%d- %m- %Y'),ptname,hn,price,depart,ptright FROM depart WHERE date LIKE '$yrmonth%' and ptright LIKE 'R07%'  and depart='XRAY'  AND (an is Null OR an ='')";
    $result = mysql_query($query) or die("Query failed,wardep");
    print "<table>";
    print " <tr>";
    print "  <th bgcolor=6495ED>#</th>";
   ...
   $result = mysql_query($query);
	...
	if($price < 0){
		...
		$sum2[2] = $sum2[2]+$price;
		      ...
		      print "<table>";
$sum1[2] = "3. ค่าทำXRAYผู้ป่วยนอกประกันสังคมจำนวน : ".$num." รายการ";
$sum = $sum + $num;
//wadepetc table
      print("<tr>\n".
              "<td bgcolor=F5DEB3>4. ค่า อื่นๆ ผู้ป่วยนอกประกันสังคม เดือน $yrmonth</td>\n".
              " </tr>\n");
    $query="CREATE TEMPORARY TABLE wadepxr2 SELECT date_format(date,'%d- %m- %Y'),ptname,hn,price,depart,ptright FROM depart WHERE date LIKE '$yrmonth%' and ptright LIKE 'R07%'  and depart != 'PATHO' and depart != 'XRAY'  AND (an is Null OR an ='')";
   ...
   $result = mysql_query($query);
	...
	if($price < 0){
		...
		$sum2[3] = $sum2[3]+$price;
		      ...
		      print "<table>";
$sum1[3] = "4. ค่า อื่นๆ ผู้ป่วยนอกประกันสังคมจำนวน : ".$num." รายการ";
$sum = $sum + $num;
