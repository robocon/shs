...
<form method="POST" action="icd10top5.php">
...
include("connect.inc");
    ...
    $querydb1="CREATE TEMPORARY TABLE opday1 SELECT  tb1.row_id, tb1.icd10, tb2. dbirth FROM opday as tb1 inner join opcard as tb2 on tb1.hn=tb2.hn WHERE tb1.thidate LIKE '$yrmonth%'  and tb1.icd10 !='' and (tb1.an='' or tb1.an is null)";
    $resultdb1 = mysql_query($querydb1) or die(mysql_error());
		
	$querydb2="CREATE TEMPORARY TABLE ipday1 SELECT * FROM ipcard  WHERE dcdate LIKE '$yrmonth%'   and icd10 !='' and substring(age,1,2) < '15' ";
	echo $query2;
...
<br> ";
   ...
   $query1="SELECT  COUNT(*) as c1 FROM opday1 where substring(dbirth,1,4) > '2543' and substring(dbirth,1,4) < '2557'";
   $result1 = mysql_query($query1);
   list ($c1) = mysql_fetch_row ($result1);


   $query="SELECT  icd10,COUNT(*) AS duplicate FROM opday1 where substring(dbirth,1,4) > '2543' and substring(dbirth,1,4) < '2557' GROUP BY icd10 HAVING duplicate > 0 ORDER BY duplicate DESC  limit 30";
   ...
   $query1="SELECT  icd10,COUNT(*) AS duplicate FROM ipday1 GROUP BY icd10 HAVING duplicate > 0 ORDER BY duplicate DESC  limit 30";
   $result1 = mysql_query($query1);
   $n1=0;
