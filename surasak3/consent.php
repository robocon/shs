...
<TR  style='line-height:15px;'>
	<TD colspan="2">
<A HREF="javascript:void(0);" style="text-decoration: none;color:#000000;" Onclick="window.print();">ข้าพเจ้า</A><?php if($_POST["pttt"] == "ผู้แทน"){?>(&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;)<?php }?><U> <?php echo $fullname;?>             </U>  HN<U>              <?php echo $_GET["hn"];?>          </U>  อายุ <U><?php echo substr(calcage($dbirth),0,-8);?></U><BR>
<B><?php if( $_POST["confirm"] == "ไม่ยินยอม") { echo "ไม่ยินยอมรับการรักษา"; }else{echo $_POST["confirm"];?></B> ทำ 
<?php 
	$comma = "";
	foreach( $_POST["list_lab"] as $value){
		echo $comma.$value;
		$comma = ", ";
	}
}
...
<font style="font-family:'MS Sans Serif'; font-size:10px;">	<?php echo $_POST["pttt"];?><U>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</U><BR><BR>
	<?php if($_POST["pttt"] == "ผู้ป่วย"){?>
		( <?php echo $fullname;?> )
		<?php }else{ ?>
		(&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;)
		<?php } ?>
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function checkForm(){
		...
		if(ff1.confirm[0].checked == false && ff1.confirm[1].checked == false){
				
				alert("กรุณาเลือกข้อมูลให้ครบครับ");
				stat = false;
		}else	if(ff1.pttt[0].checked == false && ff1.pttt[1].checked == false){
				
				alert("กรุณาเลือกข้อมูลให้ครบครับ");
				stat = false;
		}

		for(i=0;i<eval(ff1.elements.length);i++){

			if(ff1.elements[i].name == "list_lab[]" ){
				if(ff1.elements[i].checked == true){
					stat2 = true;
					break;
				}
			}
		...
		if(ff1.confirm[0].checked == true)
		if(stat2 == false){
			stat = false;
			alert("กรุณาเลือกรายการหัตถการอย่างน้อย 1 รายการครับ");
		}
...
<TR>
	<TD>
<TABLE style="font-family:  MS Sans Serif; font-size: 18 px;">
<TR>
	<TD align="right"><INPUT TYPE="radio" NAME="confirm" value="ยินยอม"></TD>
	<TD>ยินยอม</TD>
	<TD align="right"><INPUT TYPE="radio" NAME="confirm" value="ไม่ยินยอม"></TD>
	<TD>ไม่ยินยอม</TD>
</TR>
<TR>
	<TD align="right"><INPUT TYPE="radio" NAME="pttt" value="ผู้ป่วย"></TD>
	<TD>ผู้ป่วย</TD>
	<TD align="right"><INPUT TYPE="radio" NAME="pttt" value="ผู้แทน"></TD>
	<TD>ผู้แทน</TD>
</TR>
<?php 
$sql = "Select lst_labcare From trauma_lst_labcare where for_id = '".$_GET["id"]."' ";
	...
	while($arr = Mysql_fetch_assoc($result)){
		...
		echo "<TR>
					<TD><INPUT TYPE=\"checkbox\" NAME=\"list_lab[]\" value=\"".$list_labcare[$arr["lst_labcare"]]."\"></TD>
					<TD colspan=\"3\">".$list_labcare[$arr["lst_labcare"]]."</TD>
					</TR>";
	}
	...
	while($arr = Mysql_fetch_assoc($result)){
		...
		echo "<TR>
					<TD><INPUT TYPE=\"checkbox\" NAME=\"list_lab[]\" value=\"".$arr["labcare"]."\"></TD>
					<TD colspan=\"3\">".$arr["labcare"]."</TD>
					</TR>";
	}

?>
<TR><TD colspan="4" align="center"><INPUT TYPE="submit" value="ตกลง"></TD></TR>
</TABLE>
	</TD>
</TR>
</TABLE>
