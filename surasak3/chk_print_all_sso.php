...
// ปริ้นผลรายบุคคลแบบรวม
// เอา chk_doctor_print.php มาใช้งาน 98%

include 'bootstrap.php';

function toThai($str){
    return iconv('UTF8', 'TIS620', $str);
}

$part = rawurldecode($_GET['part']); 
$db = Mysql::load();
...
foreach ($items as $key => $item) {
    ...
    if( !empty($lab_opd) ){
        ...
        $pdf->Rect(62, 51, 3, 3);
        if( toThai($user['prefix']) == 'น.ส.' OR toThai($user['prefix']) == 'นางสาว' ){
        ...
        $pdf->SetXY(78, 49);
        $pdf->Cell(5, 6, toThai($user['name']), 0, 1);
        ...
        $pdf->SetXY(115, 49);
        $pdf->Cell(5, 6, toThai($user['surname']), 0, 1);
        ...
        $pdf->SetXY(31, 55);
        $pdf->Cell(40, 6, toThai($user['address']), 0, 1);
        ...
        $test_diag = false;
        if( !empty($user['diag']) ){
            $pre_diag_txt = 'Diag แพทย์: '.htmlspecialchars_decode(toThai($user['diag']));
            $diag_txt .= preg_replace('/\s+/',' ',$pre_diag_txt );
            $test_diag = true;
        }

        if( !empty($user['cxr_detail']) ){
            $pre_diag_txt = 'เอ็กซเรย์ทรวงอก: '.htmlspecialchars_decode(toThai($user['cxr_detail']));
            $newline = '';
            if($test_diag === true){
                $newline = "\n";
            }
        ...
        $pdf->SetXY(40, 266);
        $pdf->Cell(46, 6, '( '.toThai($user['name'].' '.$user['surname']).' )', 0, 1, 'C');
        ...
        $pdf->SetXY(133, 266);
        $pdf->Cell(46, 6, '( '.toThai($def_doctor).' )', 0, 1, 'C');
