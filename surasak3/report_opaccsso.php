...
<title>ข้อมูลค่าใช้จ่ายผู้ป่วยนอก (สิทธิประกันสังคม)</title>
<style type="text/css">
<!--
body,td,th {
...
<p align="center"><strong>ข้อมูลค่าใช้จ่ายผู้ป่วยนอก (สิทธิประกันสังคม)</strong></p>
<form action="report_opaccsso.php" method="post" name="form1">
...
echo "<hr>";
    ...
    */
	$tbsql="SELECT * FROM `drugrx` INNER JOIN opcard ON drugrx.hn = opcard.hn
WHERE drugrx.part
LIKE 'DPY' AND opcard.ptright = 'R07 ประกันสังคม' AND drugrx.amount !='0' AND (
drugrx.date
...
?> 
<div align="center"><strong>ข้อมูลค่าใช้จ่ายผู้ป่วยนอก (สิทธิประกันสังคม)</strong></div>
...
<table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;">
  ...
  <?
	if($tbnum < 1){
		echo "<tr><td colspan='9' align='center' style='color:red;'>------------------------ ไม่มีข้อมูล ------------------------</td></tr>";
	}else{
		$i=0;
...
?>
  <tr>
    ...
    <td align="left" bgcolor="#CCFFCC"><?=$tbrows["yot"]." ".$tbrows["name"]." ".$tbrows["surname"];?></td>
