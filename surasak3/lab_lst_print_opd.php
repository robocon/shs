...
$sql = "Select date_format(orderdate,'%Y-%m-%d'),patientname,labnumber,sex From resulthead where hn = '$hn' AND orderdate like '".$date_now."%' Order by  `autonumber` DESC limit 0,1";
...
$list_code_lab = array();
$lab_title = array();
$sql = "Select a.profilecode, count(b.labcode) From (Select * From resulthead where hn='$hn' AND orderdate like '".$date_now."%' ) as a INNER JOIN resultdetail as b ON a.autonumber = b.autonumber group by a.profilecode  Order by count(b.labcode) DESC , testgroupcode ASC";
//echo $sql;
...
foreach($list_code_lab as $key => $value){
		...
		$r=4;
		$sql = "Select autonumber, profilecode,clinicalinfo, testgroupname From resulthead where hn='$hn' AND orderdate like '".$date_now."%' AND profilecode = '".$key."'  Order by autonumber DESC limit 0,1";
		$result = mysql_query($sql);
