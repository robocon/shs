...
// Tell bootstrp to using utf-8 file encode
...
SELECT `row_id`,`genname`,`tradname` FROM `druglst` WHERE `genname` LIKE '%$word%';
	...
	while($item = mysql_fetch_assoc($query)){
		...
		// );
		// $pre_res[] = "{'row_id':'".$item['row_id']."', 'genname':'".$item['genname']."', 'tradname': '".$item['tradname']."'}";
		
		var_dump($item['tradname']);
		// var_dump(iconv("UTF-8", "TIS-620", $item['tradname']));
...
<br>
<form method="post" action="drugreact_testv2.php" class="container">
	<div>
