...
function checkptring(opt){
	...
	function checkformf2(){
		...
		&& document.f2.credit[12].checked == false && document.f2.credit[13].checked == false && document.f2.credit[14].checked == false && document.f2.credit[15].checked == false 
		&& document.f2.credit[16].checked == false){
			alert("กรุณาเลือกวิธี ชำระเงินด้วยครับ");
			return false;
...
//    session_register("aBDDSN");
	...
	$cAn = $vAn;
	$cHn = $vHn;
	$cPtname = $_GET["vPtname"];
	$cPtright =$_GET["vPtright"];
	$cDiag=$_GET["vDiag"];
/*
ipmonrep table:
  date
...
if ($row->part=="DDN" and $row->status!="จำหน่าย"){  //จะมีปัญหายอดไม่ตรง ถ้าหากมีการจ่ายยาในวันที่ D/C
...
//   print "แพทย์: $cDoctor<br>";
   print "สรุปค่ารักษาพยาบาลรวมทั้งหมดในการเจ็บป่วยครั้งนี้ ณ วันที่จำหน่าย $sDiscdate<br>";
   print "<font face='Angsana New'>จำนวนทั้งสิ้น $item รายการ ดังนี้<br>";
//
print "<table width='97%' border='1' align='left' cellpadding='5' cellspacing='2' bordercolor='#FFFFFF'>";
print " <tr>";
...
print "  <th bgcolor=73C6B6>วันเวลาเข้าห้องผ่าตัด</th>";
print "  <th bgcolor=73C6B6>วันเวลาออกห้องผ่าตัด</th>";
print " </tr>";

////
$query = "SELECT date,depart,code,detail,amount,price,paid,part,idname FROM ipacc WHERE an = '$vAn'  ORDER BY depart,part,date ASC ";
    $result = mysql_query($query)
        or die("Query failed ipacc");
    $num=0;
    while (list ($date,$depart,$code,$detail,$amount,$price,$paid,$part,$idname,$startdatetime, $enddatetime) = mysql_fetch_row ($result)) {
	    $num++;
...
if($enddatetime  == Null){
		...
		print("<tr>\n".
                ...
                "<td bgcolor=#FCF3CF ><font face='Angsana New'>$amount</td>\n".  
                "<td bgcolor=#FCF3CF ><font face='Angsana New'>$price</td>\n".  
				...
				"<td bgcolor=#FCF3CF ><font face='Angsana New'>$in_surg</td>\n".  
				"<td bgcolor=#FCF3CF ><font face='Angsana New'>$out_surg</td>\n".  
                " </tr>\n");
		      }
$sql = "Select sum(price),status FROM ipacc WHERE an = '$vAn' and status='จำหน่าย' group by status ";
...
//ยาเวชภัณฑ์ที่นำไปใช้ต่อที่บ้าน
 ...
 $nNprice=$BFN+$DPN+$Nessdn+$DNessdn+$DSN+$Mc+$Bloodn+$Labon+$Xrayn+$Sinvn+$Tooln+$Surgn+$Ncaren+$Dentn+$Physin+$Stxn+$Mcn+$Mcy;
   print "<b><center>สรุปค่ารักษาพยาบาล(ค้างจ่าย) ณ วันที่จำหน่าย $sDiscdate เบิกไม่ได้/ส่วนเกิน</b></center><br>";
...
$debt=$nNprice;
$debt=number_format($debt,2,'.',''); //เพิ่มจุดทศนิยม
print "<b>ค้างจ่ายเบิกไม่ได้/ส่วนเกิน $debt บาท</b><br>";
//print "จนท. $sOfficer วันที่ $Thaidate<br>";

    print "<form name='f1'  method='POST' action='ipbill2_2022.php'  Onsubmit='return checkformf1()' >";
    print "เก็บเงินเบิกไม่ได้/ส่วนเกิน&nbsp;&nbsp;&nbsp; <input type='text' name='paid' size='10' value=$debt>&nbsp;&nbsp;บาท&nbsp;&nbsp;ใบเสร็จเลขที่&nbsp;<input type='text' name='billno' size='10' ><br>";
