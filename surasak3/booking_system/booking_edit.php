...
return $pAge;
	...
	$date_in=$str1[2].'/'.$str1[1].'/'.($str1[0]-543);
	
	if(empty($dbarr['date_in'])){
		$date_in = date('d/m/Y');
	}
...
<form name="f2" method="post" action="booking_edit.php?do=save" onsubmit="JavaScript:return fncSubmit();">
 <table  border="0" class="forntsarabun">
  ...
  <tr>
    ...
    <td>รับป่วยเมื่อ</td>
    <td colspan="3">
    <input name="date_in" type="text" class="forntsarabun" id="datepicker-th-2"  value="<?=$date_in;?>" autocomplete="off"/>
  ...
  <tr>
    ...
    <td>
	  ...
	  <option value="พิเศษ1800" <? if($dbarr['bed']=="พิเศษ1800"){ echo "selected"; }?>>พิเศษ1800</option>
...
include("../Connections/connect.inc.php"); 
	...
	$date_in=($strdate[2]+543).'-'.$strdate[1].'-'.$strdate[0];
