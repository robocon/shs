...
foreach ($build as $key => $value){
	...
	$sql = "SELECT bed,date_format(date,'%d') as date1,date_format(date,'%m') as date2,date_format(date,'%Y') as date3,ptname,an,diagnos,doctor,ptright,age,accno, bedcode, last_drug FROM bed WHERE bedcode LIKE '".$value."%' AND an != '' AND date like '".(date("Y")+543)."-".date("m-d")."%' ORDER BY bed ASC";
	$result = Mysql_Query($sql);
