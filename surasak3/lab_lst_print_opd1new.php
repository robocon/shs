...
$sql_opcard = "SELECT CONCAT((SUBSTRING(`dbirth`,1,4)-543),SUBSTRING(`dbirth`,5,6)) AS `dbirth` FROM `opcard` WHERE `hn` = '$gethn' LIMIT 1";
...
while($rowsop=mysql_fetch_array($queryop)){
	...
	// ข้อมูลหัวกระดาษ
	$sql = "Select date_format(orderdate,'%Y-%m-%d') as neworderdate,patientname,labnumber,sex,dob,comment,clinicianname From resulthead where hn = '$hn' AND labnumber = '$getlabnumber' limit 0,1";
			...
			<td align="center">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td align="left" colspan="5"><?=getAge($dateB)." ปี";?>&nbsp;&nbsp;&nbsp;<strong>Doctor : </strong><?=$rows["clinicianname"];?> <strong>Comment : </strong><?=$rows["comment"];?></td>
					</tr>
				</table>
			</td>
		</tr>
	...
	<div class="iBannerFix">  
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td width="6%" align="right"><span class="">หมายเหตุ&nbsp;</span></td>
				<td width="27%"><span class=""><b>L, H</b> หมายถึง ค่าที่ต่ำหรือสูงกว่าค่าอ้างอิงในคน</span></td>
			</tr>
			<tr>
				<td><span class="">Date Authorise : <?=$arr3["authorisedate"];?> &nbsp;&nbsp;&nbsp;Date Printed : <?=date("Y-m-d H:i:s");?></span></td>
				<td align="right"><span class=""></span></td>
				<td><span class=""><strong>LL, HH</strong> หมายถึง ค่าที่อยู่ในช่วงวิกฤต</span></td>
			</tr>
		</table>
	</div>
</div>  
