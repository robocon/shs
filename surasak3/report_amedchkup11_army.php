...
<p align="center" style="font-weight:bold;">รายงานผลการตรวจสุขภาพกำลังพล ทบ. (ผสต.11)</p>
<form name="form1" method="post" action="report_amedchkup11_army.php" >
...
if($_POST["act"]=="show"){
$nPrefix=$_POST["year1"];
$subyear=substr($nPrefix,2);
if($_POST["camp"]=="all"){
/*$sql1="SELECT *
FROM `condxofyear_so`
WHERE `yearcheck` = '$nPrefix' and hn NOT IN ('47-14187','63-1834','59-8301') and camp NOT IN ('M10 กรม ทพ.33') group by hn ORDER BY row_id desc, substring(age,1,2) DESC";*/
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;" class="pdxpro">
  <tr>
    ...
    <td width="1%" rowspan="3" align="center" valign="top" bgcolor="#FFFFFF"><strong>คำนำหน้า</strong></td>
    ...
    ประชาชน</strong></td>
    <td width="1%" rowspan="3" align="center" valign="top" bgcolor="#FFFFFF"><strong>หน่วยต้นสังกัด</strong></td>
    ...
    <td width="2%" rowspan="3" align="center" valign="top" bgcolor="#FFFFFF"><strong>สิทธิเบิก</strong><br />
      ...
      (4) สปสช.<br />
      (5) อื่นๆ</div></td>
    <td colspan="4" align="center" valign="top" bgcolor="#FFFFFF"><strong>ดัชนีมวลกาย(BMI)</strong></td>
	...
	<td width="2%" rowspan="3" align="center" valign="top" bgcolor="#FFFFFF"><strong>ตรวจสุขภาพฟัน</strong><br />
      <div align="center">ระดับ<br />
  0=ไม่ได้ตรวจ <br />
  1=สุขภาพช่องปากดี <br />
    ควรตรวจสุขภาพช่องปากทุกปี<br />
  2=ควรรับการขูดหินน้ำลาย<br />
  3=ควรรับการรักษาภายใน 12 เดือน<br />
  4=ควรได้รับการรักษาเร่งด่วน</div></td>	
  </tr>
  <tr>
...
2=ออกกำลังกายตามเกณฑ์<br />
  ...
  while($arr1=mysql_fetch_array($query1)){
	...
	$densql=mysql_query("select den_health from survey_oral where hn='".$arr1[hn]."' and yearcheck='".$subyear."'");		
	list($den_health)=mysql_fetch_row($densql);	 
	
	
	
	$opsql=mysql_query("select yot,name,surname,idcard,sex from opcard where hn='$arr1[hn]'");		
	list($yot,$name,$surname,$idcard,$sex)=mysql_fetch_row($opsql);	
	...
	if($arr1["camp"]=="M03 มณฑลทหารบกที่32"){
		$camp="มทบ.32";
	}else if($arr1["camp"]=="M0301 บก.มทบ.32"){
		$camp="บก.มทบ.32";
	}else if($arr1["camp"]=="M0302 กกพ.มทบ.32"){
		$camp="กกพ.มทบ.32";
	}else if($arr1["camp"]=="M0303 กขว.,ฝผท.มทบ.32"){
		$camp="กขว.มทบ.32";
	}else if($arr1["camp"]=="M0304 กยก.มทบ.32"){
		$camp="กยก.มทบ.32";
	}else if($arr1["camp"]=="M0305 กกบ.มทบ.32"){
		$camp="กกบ.มทบ.32";
	}else if($arr1["camp"]=="M0306 กกร.มทบ.32"){
		$camp="กกร.มทบ.32";
	}else if($arr1["camp"]=="M0307 ฝคง.มทบ.32"){
		$camp="ฝคง.มทบ.32";
	}else if($arr1["camp"]=="M0308 ฝกง.มทบ.32"){
		$camp="ฝกง.มทบ.32";
	}else if($arr1["camp"]=="M0309 ฝสก.มทบ.32"){
		$camp="ฝสก.มทบ.32";
	}else if($arr1["camp"]=="M0311 ผพธ.มทบ.32"){
		$camp="ผพธ.มทบ.32";
	}else if($arr1["camp"]=="M0312 อก.ศาล มทบ.32" || $arr1["camp"]=="M0326 ศาล.มทบ.32"){
		$camp="ศาล และ อก.ศาล มทบ.32";
	}else if($arr1["camp"]=="M0313 รจ.มทบ.32"){
		//$camp="รจ.มทบ.32";
		$camp="มทบ.32";
	}else if($arr1["camp"]=="M0313 ฝสวส.มทบ.32"){
		$camp="ฝสวส.มทบ.32";
	}else if($arr1["camp"]=="M0316 ร้อย.มทบ.32"){
		$camp="ร้อย.มทบ.32";
	}else if($arr1["camp"]=="M0317 สขส.มทบ.32"){
		$camp="สขส.มทบ.32";
	}else if($arr1["camp"]=="M0318 ผยย.มทบ.32"){
		$camp="ผยย.มทบ.32";
	}else if($arr1["camp"]=="M0319 สส.มทบ.32"){
		//$camp="สส.มทบ.32";
		$camp="มทบ.32";
	}else if($arr1["camp"]=="M0320 ฝสห.มทบ.32"){
		$camp="ฝสห.มทบ.32";
	}else if($arr1["camp"]=="M0321 ร้อย.สห.มทบ.32"){
		$camp="ร้อย.สห.มทบ.32";
	}else if($arr1["camp"]=="M0322 มว.ดย.มทบ.32"){
		$camp="มว.ดย.มทบ.32";
	}else if($arr1["camp"]=="M0323 ผสพ.มทบ.32"){
		$camp="ผสพ.มทบ.32";
	}else if($arr1["camp"]=="M0324 สรรพกำลัง มทบ.32" || $arr1["camp"]=="M0315 อศจ.มทบ.32"){
		$camp="มทบ.32";
	}else if($arr1["camp"]=="M0325 ศฝ.นศท.มทบ.32"){
		$camp="ศฝ.นศท.มทบ.32";
	}else if($arr1["camp"]=="M0328 ผปบ.มทบ.32"){
		$camp="ผปบ.มทบ.32";
	}else if($arr1["camp"]=="M02 ร.17 พัน2"){
		$camp="ร.17 พัน 2";
	}else if($arr1["camp"]=="M04 ร.พ.ค่ายสุรศักดิ์มนตรี" || $arr1["camp"]=="M04  ร.พ.ค่ายสุรศักดิ์มนตรี"){
		$camp="รพ.ค่ายสุรศักดิ์มนตรี";
	}else if($arr1["camp"]=="M05 ช.พัน4"){
		$camp="ช.พัน.4 พล.ร.4";
	}else if($arr1["camp"]=="M06 ร้อยฝึกรบพิเศษประตูผา"){
		$camp="ร้อย.ฝรพ.3";		
	}else if($arr1["camp"]=="M08 สัสดีจังหวัดลำปาง"){
		$camp="สง.สด.จว.ล.ป.";	
	}else{
		$camp="มทบ.32";
	}
	...
	$chksql=mysql_query("select gender,position,ratchakarn,dxptright from chkup_solider where hn='$arr1[hn]' order by row_id desc limit 0,1");		
  ...
  <tr>
    ...
    <td><? if(!empty($hn)){ echo $hn;}else{ echo "&nbsp;";}?></td>
    <td><? if(!empty($idcard)){ echo $idcard;}else{ echo "&nbsp;";}?></td>
    <td><? echo $camp;?></td>
