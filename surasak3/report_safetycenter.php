...
<title>พิมพ์ใบตรวจสุขภาพบริษัท กลางคุ้มครองผู้ประสบภัยจากรถ จำกัด</title>
<style type="text/css">
<!--
.tet {
...
<center>
<span class="tet1">พิมพ์ใบตรวจสุขภาพบริษัท กลางคุ้มครองผู้ประสบภัยจากรถ จำกัด</span> <br />
  <br />
...
if(isset($_POST['hn'])){
	...
	$select = "select * from opd  where hn='".$result2['HN']."' and (thidate like '2557-12%')";
	//echo $select;
	$row = mysql_query($select)or die (mysql_error());	
	$numchk=mysql_num_rows($row);
	if(empty($numchk)){
	$select = "select * from  dxofyear_out  where hn='".$result2['HN']."' and (thidate like '2014-12%')";
	$row = mysql_query($select)or die (mysql_error());		
	}
	//echo $select;
...
<table width="100%" border="0">
  <tr>
    <td colspan="2"><table width="100%">
      <tr>
        ...
        <td width="77%" align="center" valign="top" class="texthead"><strong>แบบรายงานการตรวจสุขภาพบริษัท กลางคุ้มครองผู้ประสบภัยจากรถ จำกัด</strong></td>
      ...
      <tr>
        <td align="center" valign="top" class="text3"><span class="text"><span class="text1"><span class="text2">ตรวจเมื่อวันที่ 3-4 ธันวาคม 2557</span></span></span></td>
  ...
  <tr>
    <td colspan="2"  valign="top"><table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000"   class="text1" style="border-collapse:collapse;" >
      <tr>
        <td  valign="top" class="text2"><strong class="text" style="font-size:22px"><u>ข้อมูลผู้ตรวจสุขภาพ </u></strong> <strong>&nbsp;&nbsp;&nbsp;HN :
          <?=$result['hn']?>
          ...
          ซม. <strong>BMI: </strong> <u>
            <?=$bmi?>
              ...
              mmHg. </u></strong><strong>P: </strong> <u>
                <?=$result['pause']?>
                ครั้ง/นาที </u></span><br /><strong style="font-size:20px;">ผลตรวจ : </strong><span style="font-size:16px;"> ดัชนีมวลกาย
          <?  if($bmi == '0.00' ){
...
/////
 ...
 ?>         
          <tr>
            <td height="19" colspan="3"><strong>ผลตรวจ :</strong> <? 
																	if($result['hn']=="57-9844"){
																		 echo "ผิดปกติ  ค่า EOS สูงผิดปกติ ควรตรวจซ้ำ หรือ พบแพทย์เพื่อหาสาเหตุ";
																	}else{
																		while($objResult = mysql_fetch_array($objQuery)){
																			 if($objResult["labcode"]=="WBC"){
...
/////
		...
		?>
          ...
          <?  } ?>        
          <tr>             
            <td height="27" colspan="3"><strong>ผลตรวจ :</strong> <? if($result['hn']=="57-9844" || $result['hn']=="51-4507" || $result['hn']=="47-29830"){ echo "ผิดปกติ ควรพบแพทย์เพื่อตรวจซ้ำหรือรับการรักษา";}else{ echo "ปกติ";}?></td>
            </tr>
        </table></td>
      </tr>
    </table></td>
...
if($objResult["labcode"]=='TRIG'){
	if($objResult["result"]>=30 && $objResult["result"]<=150){
		$app="ระดับไขมันในเลือดอยู่ระดับปกติ ควรติดตามซ้ำทุก1ปี";	
	}else	if($objResult["result"]>150 && $objResult["result"]<=300){
...
if($objResult["labcode"]=='HIV'){
	if($objResult["result"]=="Positive"){
		...
		$app="";
			...
			if($objResult['result']=='Positive' || $objResult['result']=='positive'){
				$objResult["result"]="<strong>".$objResult["result"]."</strong>";
			}else{
				$objResult["result"]=$objResult["result"];
			}
...
<table width="100%" border="0" class="text4">
  <tr>
    ...
    ) </strong><strong>Doctor :พ.ต.เลอปรัชญ์ มังกรกนกพงศ์ (16-12-2014)</strong></td>
