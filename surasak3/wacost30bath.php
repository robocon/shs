...
<th bgcolor="6495ED"><font face='Angsana New'>OTHER</th>
...
<?php
    ...
    $query = "SELECT date_format(a.thidate,'%d/%m/%y'), a.ptname,a.hn,a.an,a.note,a.diag,a.patho,a.xray,a.phar,a.emer,a.surg,a.physi,a.denta,a.other,a.doctor,idcard,a.icd10,a.icd10_other
    FROM ( 
        Select * 
        From opday 
        WHERE thidate LIKE '$today%' 
        AND left(ptright,3) IN ('R09','R10','R11','R13','R17') 
    ) as a  
    ...
    $n=0;
 $totalpri=0;
    while (list ($thidate,$ptname,$hn,$an,$note,$diag,$patho,$xray,$phar,$emer,$surg,$physi,$denta,$other,$doctor,$idcard,$icd10,$icd10Other) = mysql_fetch_row ($result)) {
        $n++;	
...
//   $totalpri=$totalpri+$netprice;
        print (" <tr>\n".
                    ...
                    "  <td BGCOLOR=66CDAA><font face='Angsana New'>$icd10Other</td>\n".
