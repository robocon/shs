...
//    $chkdate=($chkdate).date(" H:i:s"); 
    ...
    $today="$d-$m-$yr";
    $repdate=$today;   
    print "บัญชีรายรับผู้ป่วยนอก จ่ายตรง ของวันที่ $repdate ";
...
session_register("credit_8");
    ...
    $query = "SELECT * FROM opacc WHERE txdate LIKE '$today%' and credit = 'จ่ายตรง' AND paidcscd > 0 ";
...
//   $x++;
   ...
   for ($n=$x; $n>=1; $n--){
        ...
        print("<tr>\n".
			...
			"<td bgcolor=F5DEB3><font face='Angsana New'>$time2</td>\n".
          ...
          //      "<td bgcolor=F5DEB3><font face='Angsana New'>$aPaid[$n]</td>\n".  
                "<td bgcolor=F5DEB3><font face='Angsana New'><A HREF=\"edit_cashcscd.php?id=$rowid[$n]\" target='_blank'>$aCredit[$n]</A></td>\n".  
		  //	   "<td bgcolor=F5DEB3><font face='Angsana New'>$aCredit_d[$n]</td>\n".
				  	   "<td bgcolor=F5DEB3><font face='Angsana New'>$aPaidcscd[$n]</td>\n".
