...
<center>
  ...
  <h1 align="center">โปรแกรมซักประวัติตรวจสุขภาพ <u>สำหรับใบรับรองแพทย์อิเล็กทรอนิกส์</u></h1>
</center>

<form action="Edx_ofyear_out2.php" method="post">
...
list($temperature,$pause,$rate,$weight,$height,$bp1,$bp2,$bp3,$bp4,$waist) = Mysql_fetch_row($result3);
if($cou=="0"){
	$sql3 = "Select  temperature,pause,rate,weight,height,bp1,bp2,bp3,bp4,doctor,clinic From dxofyear_out where hn = '".$arr_view["hn"]."' and thidate like '".date("Y-m-d")."%'";
	$result3 = Mysql_Query($sql3);
	list($temperature,$pause,$rate,$weight,$height,$bp1,$bp2,$bp3,$bp4,$dr,$cli) = Mysql_fetch_row($result3);
}
...
//ค้นหาข้อมูลเดิม
	...
	if($count > 0){
		...
		$sql = "Select congenital_disease, weight, height,cigarette,alcohol,exercise ,bp1,bp2,bp3,bp4,doctor  From opd where hn = '".$arr_view["hn"]."' AND type <> 'ญาติ' Order by row_id DESC limit 1";
		
		//echo "OPD";

		$result = Mysql_Query($sql);
		list($congenital_disease, $weight, $height, $cigarette, $alcohol, $exercise,$bp1,$bp2,$bp3,$bp4,$doctor) = Mysql_fetch_row($result);
	...
	<table width="528" border="0" class="tb_font">
		...
		<tr>
		  ...
		  <td colspan="3"><span class="pdxhead">
		    <select name='camp' id="camp" onchange='Show()'>
			<?php 
			$toborow_key = substr($toborow,0,4);
			$sql12 = "select * from chkcompany where status='Y' AND (row_id = 77 OR row_id = 78) order by row_id asc";
		...
		<tr>
		  ...
		  <td align="left">
		  		<?php
		  			...
		  			$sql_address = "select * from opcard where hn = '47-1' LIMIT 0,1 ";
...
ครั้ง/นาที</td>
		  ...
		  <td align="left">
		  	<input name="bp21" type="text" size="1" maxlength="3" value="<?php if($bp3 != ""){echo $bp3;}else{echo $bp21;} ?>" /> / <input name="bp22" type="text" size="1" maxlength="3" value="<?php if($bp4 != ""){echo $bp4;}else{echo $bp22;} ?>" /> mmHg<br>
...
<select name="select" onchange="if(this.value !=''){document.getElementById('organ').value = document.getElementById('organ').value+' '+this.value;}" style="position: absolute;">
	...
	<tr>
      ...
      <?php 
		...
		print " <option value='10 รังสีวิทยา'>รังสีวิทยา</option>";
