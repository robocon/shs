...
/////คำนวนวันสุดท้าย
		//echo $clastcal;
		$query = "SELECT lastcalroom,bedname FROM bed WHERE bedcode = '$cBedcode'";
    	$result = mysql_query($query) or die("Query failed");
		list($lastcalroom,$cBedname) = mysql_fetch_array($result);
	   ...
	   //echo "จำนวนวัน  $d วัน $h ชั่วโมง &nbsp;&nbsp;";
		...
		//echo $h;
		//echo $dayslast;
		$query3 = "Select my_food,doctor,diag,ptright From ipcard where an = '$cAn' limit 1";
		$result3 = Mysql_Query($query3) or die(mysql_error());
		list($myfood,$doctor,$diag,$ptright) = Mysql_fetch_row($result3);
		...
		if($oBedcode1 != '44'){
		 ...
		 elseif($oBedcode1=="46") $dcward = "หอผู้ป่วย Cohort Ward";
		 elseif($oBedcode1=="47") $dcward = "หอผู้ป่วย Home Isolation";
		 elseif($oBedcode1=="48") $dcward = "หอผู้ป่วย รพ.สนาม";
 		//echo "เบิกได้ $cYBedpri1<br>";
   		//echo "เบิกไม่ได้ $cNBedpri1<br>";
		  ...
		  /////////
			...
			if($oBedcode1=="46"){  //Cohort Ward
				$chkptright=substr($ptright,0,3);				
				if($chkptright=="R09" || $chkptright=="R10" || $chkptright=="R11" || $chkptright=="R12" || $chkptright=="R13" || $chkptright=="R14" || $chkptright=="R36"){ //สิทธิ 30 บาท
					if($cBedpri=="1500.00"){
						$claimcode="COVR07";
						$cBedname="ค่าห้องสำหรับผู้ป่วยโควิดสีเขียว";
					}else if($cBedpri=="7500.00"){
						$claimcode="COVR10";
						$cBedname="ค่าห้องสำหรับผู้ป่วยโควิดสีแดง";
					}else{
						$claimcode="COVR09";
						$cBedname="ค่าห้องสำหรับผู้ป่วยโควิดสีเหลือง";						
					}							
				}else if($chkptright=="R07"){  //สิทธิประกันสังคม
					if($cBedpri=="1500.00"){
						$claimcode="21421";
						$cBedname="ค่าห้องควบคุม/ดูแลสำหรับผู้ติดเชื้อมีอาการเล็กน้อย";
					}else if($cBedpri=="7500.00"){
						$claimcode="21423";
						$cBedname="ค่าห้องควบคุม/ดูแลสำหรับผู้ติดเชื้อมีอาการรุนแรง";					
					}else if($cBedpri=="3000.00"){
						$claimcode="21422";
						$cBedname="ค่าห้องควบคุม/ดูแลสำหรับผู้ติดเชื้อมีอาการปานกลาง";						
					}				
				}else{  //สิทธิอื่นๆ
					if($cBedpri=="1000.00"){
						$claimcode="21421";
...
//ค่าห้องที่เบิกได้ patdata
			$query = "INSERT INTO patdata(date,hn,an,ptname,doctor,item,code,detail,amount,price,depart,part,idno) VALUES('$Thidate','$cHn','$cAn','$cPtname','$cDoctor','2','$claimcode','$cBedname (เฉพาะที่เบิกได้) $stays','$dayslast','$cYBedfood','WARD','BFY','$idno');";
			//echo $query."<br>";
			$result = mysql_query($query) or die("Query failed,cannot insert into patdata");
//ค่าบริการทางพยาบาล
		if($oBedcode1!="47"){	 //Home Isolation
...
//ค่าห้องที่เบิกได้ ipacc	
			$query = "INSERT INTO ipacc(date,an,code,depart,detail,amount,price,idname,part,accno,idno)VALUES('$Thidate','$cAn','$claimcode','WARD','$cBedname ($cBedcode)(จำหน่าย) $stays','$dayslast','$cYBedfood','$sOfficer','BFY','$cAccno','$idno');";
			//echo $query."<br>";
