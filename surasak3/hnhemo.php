...
<form method="POST" name="formm" onsubmit="return checkType();" action="<?php echo $PHP_SELF ?>">
	...
	<p>&nbsp;&nbsp;HN&nbsp;&nbsp;<input type="text" name="hn" size="12">
		...
		<br />
		<input name="servicd" type="radio" id="servicd2" value="0" checked="checked" />
...
if (!empty($hn) && $confirm != true) { // <<==== เข้าฟอร์มนี้ก่อน

	$hn = $_POST['hn'];
	$service = $_POST['service'];
	$query = "SELECT hn, concat(yot,' ',name,' ',surname) as ptname, ptright FROM opcard WHERE hn = '$hn'  limit 1 ";
	$result = mysql_query($query) or die(Mysql_Error());
	list($opcardHn, $opcardPtname, $opcardPtright) = Mysql_fetch_row($result);
	?>
	<div>
		<table>
			<tr>
		...
		<div>
			<a href="hnhemo.php?hn=<?= $opcardHn ?>&confirm=true&service=<?= $servicd ?>">✅ ชื่อถูกต้อง ทำรายการต่อไป</a>
		</div>
	</div>
	<?php
	...
	if (mysql_num_rows($result)) { //กรณีลงทะเบียนแล้ว
		...
		$tvn = $row->vn;
		////////////คิดเงิน 50 บาท
		if ($_GET['service'] == "1") { // คิดค่าบริการ 50 บาท
