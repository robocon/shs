...
<head>
	...
	<style>
		...
		.container {
			...
			max-width: 100vw;
			margin: 0 auto;
			/* กึ่งกลางในแนวนอน (แม้จะกว้าง 100vw แต่ใช้เผื่อไว้) */
			padding: 2rem 0 1rem 0;
			/* ด้านบนห่าง 2rem, ล่าง 1rem */
			box-sizing: border-box;
		}
...
<body>
    ...
    <h2>รายชื่อแพทย์ไม่ออกตรวจวันนี้ (" . date("d-m-") . (date("Y") + 543) . ")</h2><p>";
		...
		while (list($row, $depart, $new, $datetime, $file, $start, $end) = mysql_fetch_row($result)) {
			...
			if ($file) {
				echo "<a href='surasak3/file_news/$file' target='_blank' style='color:#1D293D;'>ดาวน์โหลดไฟล์</a>";
			}
			echo "</p>";
		}
		echo "</div>";
		...
		if (mysql_num_rows($q) > 0) {
			?>
			<div class='section news-contain'><h2>คู่มือการปฏิบัติงานและการใช้งานระบบสารสนเทศ</h2>
				<ol>
			<?php
			...
			?>
				...
				<div style="margin-top:1em;">
					<a href="surasak3/news_list_show.php" style="color: #004cff;">อ่านทั้งหมด</a>
				</div>
