...
<?
	...
	//echo $query1;
 	...
 	while(list($bed,$date,$ptname,$an,$hn,$caldate,$bedname,$bedcode,$status)=Mysql_fetch_row($result1)){
		...
		$query3 = "Select my_food,doctor,diag,dcdate,ptright From ipcard where an = '$an' limit 1";
		$result3 = Mysql_Query($query3) or die(mysql_error());
		list($myfood,$doctor,$diag,$dcdate,$ptright) = Mysql_fetch_row($result3);
		...
		if($dcdate=="0000-00-00 00:00:00"){
			...
			if($oBedcode1=="46"){  //Cohort Ward
				...
				if($chkptright=="R09" || $chkptright=="R10" || $chkptright=="R11" || $chkptright=="R12" || $chkptright=="R13" || $chkptright=="R14" || $chkptright=="R36"){ //สิทธิ 30 บาท
					if($cBedpri=="1500.00"){
						$claimcode="COVR07";
						$bedname="ค่าห้องสำหรับผู้ป่วยโควิดสีเขียว";
					}else if($cBedpri=="7500.00"){
						$claimcode="COVR10";
						$bedname="ค่าห้องสำหรับผู้ป่วยโควิดสีแดง";
					}else{
						$claimcode="COVR09";
						$bedname="ค่าห้องสำหรับผู้ป่วยโควิดสีเหลือง";						
					}							
					...
					if($cBedpri=="1500.00"){
						...
						$claimcode="21423";
						$bedname="ค่าห้องควบคุม/ดูแลสำหรับผู้ติดเชื้อมีอาการรุนแรง";					
					}else if($cBedpri=="3000.00"){
						$claimcode="21422";
						$bedname="ค่าห้องควบคุม/ดูแลสำหรับผู้ติดเชื้อมีอาการปานกลาง";						
					}				
				}else{  //สิทธิอื่นๆ
					if($cBedpri=="1000.00"){
						$claimcode="21421";
	...
	//ค่าห้องที่เบิกได้ patdata
				$query = "INSERT INTO patdata(date,hn,an,ptname,doctor,item,code,detail,amount,price,depart,part,idno) VALUES('$Thidate','$hn','$an','$ptname','$doctor','2','$claimcode','$bedname (เฉพาะที่เบิกได้) $stays','$days','$cYBedfood','WARD','BFY','$idno');";
				//echo $query."<br>";
	...
	//ค่าห้องที่เบิกได้ ipacc	
				$query = "INSERT INTO ipacc(date,an,code,depart,detail,amount,price,idname,part,accno,idno)VALUES('$Thidate','$an','$claimcode','WARD','$bedname ($bedcode) $stays','$days','$cYBedfood','คอมพิวเตอร์','BFY','$cAccno','$idno');";
				//echo $query."<br>";
